...
class CacheServiceClient:
    def __init__(self, client: HTTPClient, enable_cache: bool = True):
        self._client = client
        self._url = "/cache-services"
        self._enable_cache = enable_cache
        self._cache: Dict[int, CacheServicePublic] = {}
    ...
    def list(
        ...
        if use_cache and self._enable_cache and not has_pagination:
            ...
            logger.debug(
                "cache-services cache not authoritative; "
        ...
        return CacheServicesPublic.model_validate(response.json())

    def _list_from_cache(
        self, params: Dict[str, Any] = None
    ) -> Optional[CacheServicesPublic]:
        """
        ...
        return CacheServicesPublic(items=all_items, total=total, pagination=pagination)
        ...
        try:
            ...
            if is_id_only_delete:
                ...
                if item is not None:
                    ...
                    event.data = item
                logger.debug(f"Cache: removed cacheservice {event.id}")
                return

            item = CacheServicePublic.model_validate(event.data)
            ...
            with self._cache_lock:
                if event.type == EventType.DELETED:
                    ...
                    logger.debug(f"Cache: removed cacheservice {item.id}")
                else:  # CREATED or UPDATED
                    self._cache[item.id] = item
                    logger.trace(f"Cache: updated cacheservice {item.id}")
        except Exception as e:
            logger.error(f"Failed to update cache-services cache from event: {e}")
        ...
        try:
            async with self._client.get_async_httpx_client().stream(
                ...
                if self._enable_cache:
                    ...
                    if first_start:
                        logger.debug(f"cache-services cache watch started")
                    ...
                    try:
                        ...
                        if line:
                            ...
                            if self._enable_cache:
                                ...
                                if (
                                    ...
                                    if cache_size > 0:
                                        ...
                                        logger.debug(
                                            f"cache-services cache populated with {cache_size} items"
                                        )
                            ...
                            if (
                                ...
                                logger.debug(
                                    f"Skipping callback for ID-only {event.type} event on cache-services {event.id}"
                                )
                            elif callback:
    ...
    def get(self, id: int, use_cache: bool = True) -> CacheServicePublic:
        """
        ...
        if use_cache and self._enable_cache:
            with self._cache_lock:
                if self._watch_started and id in self._cache:
                    logger.trace(f"Cache hit for cacheservice {id}")
        ...
        raise_if_response_error(response)
        return CacheServicePublic.model_validate(response.json())

    def create(self, model_create: CacheServiceCreate):
    ...
    def update(self, id: int, model_update: CacheServiceUpdate):
