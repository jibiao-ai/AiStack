...
write_files:
  - path: /var/lib/aistack/config.yaml
    permissions: '0600'
    content: |
  ...
  - path: /opt/aistack-run-worker.sh
    permissions: '0755'
    content: |-
      #!/bin/bash
      set -e
      ...
      echo "$(date): aistack worker container started" >> /var/log/post-reboot.log
"""

post_boot_service = """[Unit]
Description=bootstrap aistack worker container
...
Type=oneshot
RemainAfterExit=no
ExecStart=/bin/bash -c "/opt/aistack-run-worker.sh && systemctl disable post-reboot.service"
  ...
  libnvidia-container1={nvidia_toolkit_version}""",
    ...
    def format(self) -> str:
        ...
        if not should_restart:
            runcmds.append("/opt/aistack-run-worker.sh")
        else:
            write_files.append(
                {
