"""Shared helpers for CLI commands that authenticate to a local AiStack
...
``--jwt-secret-key`` / ``AISTACK_JWT_SECRET_KEY``), so the argument
...
def add_local_auth_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        "--data-dir",
        type=str,
        help="Directory of the local aistack data dir. When --api-key is not "
    ...
    parser.add_argument(
        ...
        "<data-dir>/jwt_secret_key. Prefer the AISTACK_JWT_SECRET_KEY "
        ...
        default=get_aistack_env("JWT_SECRET_KEY"),
    )
    parser.add_argument(
        "-u",
        "--admin-username",
        ...
        default=get_aistack_env("ADMIN_USERNAME") or "admin",
    )
