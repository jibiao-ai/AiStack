...
from aistack.envs import MIGRATION_DATA_DIR, DATA_MIGRATION
from aistack.logging import setup_logging
from aistack.cmd.start import (
    start_cmd_options,
    parse_args,
)
from aistack.utils.envs import get_aistack_env
from aistack.utils.ephemeral_ports import ensure_reserved_against_ephemeral
from aistack.utils.network import is_port_available, parse_port_range
from aistack.utils.s6_services import (
    gateway_services,
    postgres_services,
    migration_services,
    observability_services,
    all_services,
    aistack_service_name,
)
...
def setup_prerun_cmd(subparsers: argparse._SubParsersAction):
    parser_server: argparse.ArgumentParser = subparsers.add_parser(
        "prerun",
        description="Perform pre-run checks and setup s6-overlay configuration for AiStack.",
    )
    ...
    parser_server.add_argument(
        ...
        default=get_aistack_env("S6_BASE_PATH"),
    )
...
def run(args: argparse.Namespace):
    try:
        ...
        logger.info(
            "Starting pre-run checks and setup s6-overlay configuration for AiStack..."
        )
        ...
        if len(dependency_services) == 0 and len(enabled_services) == 0:
            logger.info("No extra s6 services for aistack to enable.")
        else:
            logger.info(
                f"Enabled s6 services: {enabled_services}, dependencies for aistack: {dependency_services}"
            )
...
def ports_for_services(cfg: Config) -> Dict[int, str]:
    ...
    if is_server:
        ports[cfg.port] = aistack_service_name
        ports[cfg.proxy_port] = aistack_service_name
        if enabled_tls:
            ports[cfg.tls_port] = aistack_service_name
        if not cfg.disable_metrics:
            ports[cfg.metrics_port] = aistack_service_name
    if is_worker:
        ports[cfg.worker_port] = aistack_service_name
        if not cfg.disable_worker_metrics:
            ports[cfg.worker_metrics_port] = aistack_service_name
    # when gateway is disabled, api port is not required
    if not gateway_disabled:
        ports[cfg.api_port] = aistack_service_name
...
def check_ports_availability(cfg: Config, *services: str):
    ...
    all_services = list(services) + [aistack_service_name]
...
def reserve_ports_against_ephemeral(cfg: Config):
    """
    Reserve aistack's service and Ray port ranges against the kernel's
    ephemeral port range so outbound connections (by aistack, higress, or
...
def migrate_hardcode_postgres_data_and_password(
    ...
    pair = {
        Path("/var/lib/aistack/postgres/data"): Path(cfg.postgres_base_dir()) / "data",
        Path("/var/lib/aistack/postgres_root_pass"): Path(cfg.data_dir)
        / "postgres_root_pass",
        Path("/var/lib/aistack/run/migration_done"): get_migration_done_file(cfg),
    }
...
def prepare_postgres_config(cfg: Config):
    ...
    config_path = prepare_env("AISTACK_POSTGRES_CONFIG", "postgresql")
...
def prepare_gateway_config(cfg: Config):
    ...
    config_path = prepare_env("AISTACK_GATEWAY_CONFIG", "gateway")
    ...
    if cfg.gateway_mode == GatewayModeEnum.embedded:
        with open(config_path, "w") as f:
            ...
            f.write(f"AISTACK_API_PORT={cfg.get_api_port()}\n")
...
def prepare_observability_config(cfg: Config):
    env_config_path = prepare_env("AISTACK_OBSERVABILITY_CONFIG", "observability")
    ...
    prometheus_config = f"""# Managed by AiStack
global:
  scrape_interval: 15s
  scrape_timeout: 10s
  ...
  - job_name: aistack-worker-discovery
    ...
    http_sd_configs:
      - url: "http://127.0.0.1:{cfg.metrics_port}/metrics/targets"
        refresh_interval: 1m
  - job_name: aistack-proxy-worker-discovery
    scrape_interval: 5s
...
"""
    ...
    if cfg.gateway_mode == GatewayModeEnum.embedded:
        ...
        prometheus_config += f"""  - job_name: aistack-gateway
...
def determine_dependency_services(cfg: Config) -> List[str]:
    ...
    if cfg.server_role() in [
        ...
        if should_migrate and MIGRATION_DATA_DIR is not None:
            logger.warning(
                f"The environment variable AISTACK_MIGRATION_DATA_DIR is deprecated. The migration target dir will be set to {cfg.data_dir} instead."
            )
...
def prepare_env(env_name: str, scope: str, env_file_name: str = ".env") -> Path:
    ...
    if config_path is None:
        base_dir = Path(os.getenv("AISTACK_RUN_DIR", "/run/aistack"))
