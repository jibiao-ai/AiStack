...
from aistack.api.auth import SESSION_COOKIE_NAME
from aistack.cmd.local_auth import (
    add_local_auth_arguments,
    mint_admin_jwt,
    read_local_jwt_secret,
)
from aistack.cmd.start import load_config_from_yaml
...
from aistack.utils.config import (
...
def setup_reload_config_cmd(subparsers: argparse._SubParsersAction):
    parser: argparse.ArgumentParser = subparsers.add_parser(
        "reload-config",
        help="Reload AiStack configuration.",
        description=("Reload AiStack configuration via --set, --file, or --list."),
    )
    ...
    parser.add_argument(
        "--file",
        type=str,
        help="Load configuration from YAML file: --file /path/to/aistack_config.yaml",
    )
    parser.add_argument(
        "--list",
    ...
    parser.add_argument(
        ...
        "worker token.",
        default=get_aistack_env("API_KEY"),
    )
    ...
    parser.add_argument(
        "--server-port",
        type=int,
        help="Port of the AiStack API server to target.",
        default=get_aistack_env("API_PORT"),
    )
    parser.add_argument(
        "--worker-port",
        type=int,
        help="Port of the AiStack worker to target.",
...
def apply_runtime_updates(
    ...
    if not applied:
        ...
        raise Exception(
            ...
            "aistack server or worker is running on this host, or target it "
