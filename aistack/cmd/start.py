...
from aistack import __version__, __git_commit__
from aistack.config.config import set_global_config
from aistack.extension import Plugin, iter_plugin_classes
from aistack.logging import setup_logging
from aistack.utils.envs import get_aistack_env, get_aistack_env_bool
from aistack.worker.worker import Worker
from aistack.config import Config
from aistack.server.server import Server
from aistack.gateway import initialize_gateway
...
def setup_start_cmd(subparsers: argparse._SubParsersAction):
    parser_server: argparse.ArgumentParser = subparsers.add_parser(
        "start",
        help="Run AiStack server or worker.",
        description="Run AiStack server or worker.",
    )
...
def start_cmd_options(parser_server: argparse.ArgumentParser):
    ...
    common_group.add_argument(
        ...
        default=get_aistack_env("ADVERTISE_ADDRESS"),
    )
    common_group.add_argument(
        "--port",
        type=int,
        help="Port to bind the server to.",
        default=get_aistack_env("PORT"),
    )
    common_group.add_argument(
        "--tls-port",
        type=int,
        help="Port to bind the TLS server to.",
        default=get_aistack_env("TLS_PORT"),
    )
    common_group.add_argument(
        "--api-port",
        type=int,
        help="Port to bind the AiStack API server to.",
        default=get_aistack_env("API_PORT"),
    )
    common_group.add_argument(
        "--proxy-port",
        type=int,
        ...
        default=get_aistack_env("PROXY_PORT"),
    )
    common_group.add_argument(
        "--config-file",
        type=str,
    ...
    common_group.add_argument(
        ...
        default=get_aistack_env_bool("DEBUG"),
    )
    common_group.add_argument(
        "--data-dir",
        type=str,
    ...
    common_group.add_argument(
        ...
        default=get_aistack_env("PIPX_PATH"),
    )
    common_group.add_argument(
        "--huggingface-token",
        type=str,
    ...
    common_group.add_argument(
        ...
        type=str,
        help="Default container registry for AiStack to pull system and inference images. The default is 'docker.io'. Keep all AiStack business images under the 'aistack' namespace; if a multi-level namespace is required, keep 'aistack' as the last level (e.g. 'awesome.com/amazing-group/amazing-subgroup/aistack') and set this to the parent path (e.g. 'awesome.com/amazing-group/amazing-subgroup'). If the images cannot stay under the 'aistack' namespace (e.g. 'aistack-ai'), adjust --image-repo and --benchmark-image-repo accordingly.",
        default=get_aistack_env("SYSTEM_DEFAULT_CONTAINER_REGISTRY"),
    )
    common_group.add_argument(
        "--image-name-override",
        type=str,
        help="Override the default image name for the AiStack container.",
        default=get_aistack_env("IMAGE_NAME_OVERRIDE"),
    )
    common_group.add_argument(
        "--image-repo",
        type=str,
        help="Override the default image repository aistack/aistack for the AiStack container (do not include the tag; AiStack appends the version tag automatically). If the image lives outside the 'aistack' namespace (e.g. 'awesome.com/amazing-group/amazing-subgroup/aistack-ai/aistack:<tag>'), set --system-default-container-registry to the parent path (e.g. 'awesome.com/amazing-group/amazing-subgroup') first, then set this to 'aistack-ai/aistack'.",
        default=get_aistack_env("IMAGE_REPO"),
    )
    common_group.add_argument(
        "--benchmark-image-repo",
        type=str,
        help="Override the default benchmark image repository aistack/benchmark-runner for the AiStack benchmark container. Unlike --image-repo, this value is used as-is, so include the tag. If the image lives outside the 'aistack' namespace (e.g. 'awesome.com/amazing-group/amazing-subgroup/aistack-ai/benchmark-runner:<tag>'), set --system-default-container-registry to the parent path (e.g. 'awesome.com/amazing-group/amazing-subgroup') first, then set this to 'aistack-ai/benchmark-runner:<tag>'.",
        default=get_aistack_env("BENCHMARK_IMAGE_REPO"),
    )
    common_group.add_argument(
        "--gateway-mode",
        type=str,
        help="Gateway running mode. Options: embedded, in-cluster, external, disabled, or auto (default).",
        default=get_aistack_env("GATEWAY_MODE"),
    )
    common_group.add_argument(
        "--gateway-kubeconfig",
        type=str,
        help="Path to the kubeconfig file for gatway. Only useful for external gateway-mode.",
        default=get_aistack_env("GATEWAY_KUBECONFIG"),
    )
    common_group.add_argument(
        "--gateway-namespace",
        type=str,
        help="The namespace where the gateway component is deployed.",
        default=get_aistack_env("GATEWAY_NAMESPACE"),
    )
    common_group.add_argument(
        "--service-discovery-name",
        type=str,
        help="the name of the service discovery service in DNS. Only useful when deployed in Kubernetes with service discovery.",
        default=get_aistack_env("SERVICE_DISCOVERY_NAME"),
    )
    common_group.add_argument(
        "--namespace",
        type=str,
        help="Kubernetes namespace for AiStack to deploy gateway routing rules and model instances.",
    ...
    server_group.add_argument(
        ...
        default=get_aistack_env("DATABASE_PORT"),
    )
    server_group.add_argument(
        "--metrics-port",
        type=int,
        help="Port to expose server metrics.",
        default=get_aistack_env("METRICS_PORT"),
    )
    server_group.add_argument(
        "--database-url",
        type=str,
    ...
    server_group.add_argument(
        "--disable-worker",
        action=OptionalBoolAction,
        help="(DEPRECATED) Disable the embedded worker for the AiStack server. New installations will not have the embedded worker by default. Use '--enable-worker' to enable the embedded worker if needed. If neither flag is set, for backward compatibility, the embedded worker will be enabled by default for legacy installations prior to v2.0.1.",
        default=get_aistack_env_bool("DISABLE_WORKER"),
    )
    server_group.add_argument(
        "--enable-worker",
        action=OptionalBoolAction,
        help="Enable the embedded worker for the AiStack server.",
        default=get_aistack_env_bool("ENABLE_WORKER"),
    )
    ...
    server_group.add_argument(
        ...
        default=get_aistack_env("BOOTSTRAP_PASSWORD"),
    )
    server_group.add_argument(
        "--ssl-keyfile",
        type=str,
    ...
    server_group.add_argument(
        ...
        default=get_aistack_env_bool("DISABLE_UPDATE_CHECK"),
    )
    server_group.add_argument(
        ...
        default=get_aistack_env_bool("DISABLE_OPENAPI_DOCS"),
    )
    server_group.add_argument(
        "--update-check-url",
        type=str,
        help=argparse.SUPPRESS,
        default=get_aistack_env("UPDATE_CHECK_URL"),
    )
    server_group.add_argument(
        "--ota-server-url",
        type=str,
        ...
        "https://ota.example.com/aistack. Defaults to the public OTA server; point "
        "it at a copy of your own when that one is unreachable.",
        default=get_aistack_env("OTA_SERVER_URL"),
    )
    server_group.add_argument(
        "--model-catalog-file",
        type=str,
        help="Path or URL to the model catalog file.",
        default=get_aistack_env("MODEL_CATALOG_FILE"),
    )
    server_group.add_argument(
        "--server-external-url",
        type=str,
        help="External URL of the server. Should be set if the server is behind a reverse proxy.",
        default=get_aistack_env("SERVER_EXTERNAL_URL"),
    )
    server_group.add_argument(
        "--gateway-concurrency",
        type=int,
        help="Number of concurrent connections for the embedded gateway. The default is 16.",
        default=get_aistack_env("GATEWAY_CONCURRENCY"),
    )
    server_group.add_argument(
        "--gateway-plugin-server-url",
        type=str,
        help=argparse.SUPPRESS,
        default=get_aistack_env("GATEWAY_PLUGIN_SERVER_URL"),
    )
    server_group.add_argument(
        "--gateway-ingress-class",
        type=str,
        help=argparse.SUPPRESS,
        default=get_aistack_env("GATEWAY_INGRESS_CLASS"),
    )
    server_group.add_argument(
        "--shuihua-api-base-url",
        type=str,
        help=argparse.SUPPRESS,
        default=get_aistack_env("SHUIHUA_API_BASE_URL"),
    )
    ...
    server_group.add_argument(
        ...
        default=get_aistack_env_bool("ENABLE_CORS"),
    )
    server_group.add_argument(
        "--allow-credentials",
        action=OptionalBoolAction,
        help="Allow cookies and credentials in cross-origin requests.",
        default=get_aistack_env_bool("ALLOW_CREDENTIALS"),
    )
    server_group.add_argument(
        "--allow-origins",
        action='append',
    ...
    server_group.add_argument(
        ...
        default=get_aistack_env("SAML_IDP_SERVER_URL"),
    )
    server_group.add_argument(
        "--saml-idp-logout-url",
        type=str,
        help="SAML IdP Single Logout endpoint URL.",
        default=get_aistack_env("SAML_IDP_LOGOUT_URL"),
    )
    server_group.add_argument(
        "--saml-idp-entity-id",
        type=str,
        help="SAML IdP entity ID.",
        default=get_aistack_env("SAML_IDP_ENTITY_ID"),
    )
    server_group.add_argument(
        "--saml-idp-x509-cert",
        type=str,
        help="SAML IdP X.509 certificate.",
        default=get_aistack_env("SAML_IDP_X509_CERT"),
    )
    server_group.add_argument(
        "--saml-sp-entity-id",
        type=str,
        help="SAML SP entity ID.",
        default=get_aistack_env("SAML_SP_ENTITY_ID"),
    )
    server_group.add_argument(
        "--saml-sp-acs-url",
        type=str,
        help="SAML SP Assertion Consumer Service(ACS) URL. It should be set to `<server-url>/auth/saml/callback`.",
        default=get_aistack_env("SAML_SP_ACS_URL"),
    )
    server_group.add_argument(
        "--saml-sp-slo-url",
        type=str,
        ...
        default=get_aistack_env("SAML_SP_SLO_URL"),
    )
    server_group.add_argument(
        "--saml-sp-x509-cert",
        type=str,
        help="SAML SP X.509 certificate.",
        default=get_aistack_env("SAML_SP_X509_CERT"),
    )
    server_group.add_argument(
        "--saml-sp-private-key",
        type=str,
        help="SAML SP private key.",
        default=get_aistack_env("SAML_SP_PRIVATE_KEY"),
    )
    server_group.add_argument(
        ...
        default=get_aistack_env("SAML_SP_ATTRIBUTE_PREFIX"),
    )
    server_group.add_argument(
        "--saml-security",
        type=str,
        help="SAML security settings in JSON.",
        default=get_aistack_env("SAML_SECURITY"),
    )
    ...
    server_group.add_argument(
        ...
        default=get_aistack_env("CAS_VALIDATE_ENDPOINT"),
    )
    server_group.add_argument(
        "--cas-username-attribute",
        type=str,
        help="CAS XML attribute name to use as the AiStack username. Defaults to the CAS `cas:user` element when unset.",
        default=get_aistack_env("CAS_USERNAME_ATTRIBUTE"),
    )
    server_group.add_argument(
        "--cas-full-name-attribute",
        type=str,
        help="CAS XML attribute name to use as the user's full name, e.g., 'displayName'.",
        default=get_aistack_env("CAS_FULL_NAME_ATTRIBUTE"),
    )
    server_group.add_argument(
        "--cas-avatar-attribute",
        type=str,
        help="CAS XML attribute name to use as the user's avatar URL.",
        default=get_aistack_env("CAS_AVATAR_ATTRIBUTE"),
    )
    ...
    server_group.add_argument(
        ...
        default=get_aistack_env("EXTERNAL_AUTH_NAME"),
    )
    server_group.add_argument(
        ...
        default=get_aistack_env("EXTERNAL_AUTH_FULL_NAME"),
    )
    server_group.add_argument(
        ...
        default=get_aistack_env("EXTERNAL_AUTH_AVATAR_URL"),
    )
    server_group.add_argument(
        "--external-auth-default-inactive",
        action=OptionalBoolAction,
        help="Set newly created externally authenticated users inactive by default.",
        default=get_aistack_env_bool("EXTERNAL_AUTH_DEFAULT_INACTIVE"),
    )
    server_group.add_argument(
        ...
        default=get_aistack_env_bool("EXTERNAL_AUTH_INSECURE_SKIP_TLS_VERIFY"),
    )
    server_group.add_argument(
        "--external-auth-post-logout-redirect-key",
        type=str,
        help="Generic key for post-logout redirection across IdPs.",
        default=get_aistack_env("EXTERNAL_AUTH_POST_LOGOUT_REDIRECT_KEY"),
    )
    ...
    worker_group.add_argument(
        ...
        default=get_aistack_env("SERVICE_PORT_RANGE"),
    )
    worker_group.add_argument(
        "--ray-port-range",
        type=str,
    ...
    worker_group.add_argument(
        ...
        default=get_aistack_env("WORKER_METRICS_PORT"),
    )
    worker_group.add_argument(
        "--log-dir",
        type=str,
        help="Directory to store logs.",
        default=get_aistack_env("LOG_DIR"),
    )
    worker_group.add_argument(
        "--benchmark-dir",
        type=str,
        help="Directory to store benchmark results.",
        default=get_aistack_env("BENCHMARK_DIR"),
    )
    worker_group.add_argument(
        "--system-reserved",
        type=json.loads,
        ...
        default=get_aistack_env("SYSTEM_RESERVED"),
    )
    worker_group.add_argument(
        "--tools-download-base-url",
        type=str,
        help=argparse.SUPPRESS,
        default=get_aistack_env("TOOLS_DOWNLOAD_BASE_URL"),
    )
    worker_group.add_argument(
        "--enable-hf-transfer",
        action=OptionalBoolAction,
...
def run(args: argparse.Namespace):
    try:
        ...
        logger.info(f"AiStack version: {__version__} ({__git_commit__})")
