...
from aistack.config.registration import (
...
class GatewayPluginEntry(BaseModel):
    ...
    that govern how the *server* maintains that CR stay in ``aistack.envs``,
...
class Config(WorkerConfig, BaseSettings):
    """A class used to define AiStack configuration.

    Attributes:
        port: Port to bind the server to. Default is 80.
        tls_port: Port to bind the TLS server to. Default is 443.
        api_port: Port to bind the aistack API server to. Default is 30080.
        ...
        system_default_container_registry: Default registry for container images (server and inference images). Images are expected under the 'aistack' namespace; for multi-level namespaces keep 'aistack' as the last level and set this to the parent path.
        image_name_override: Force override of the image name.
        image_repo: Repository for the container images. When the image lives outside the 'aistack' namespace (e.g. 'aistack-ai/aistack'), point system_default_container_registry to the parent namespace path and set this to the remaining repository path.
        ...
        namespace: Kubernetes namespace for AiStack to deploy gateway routing rules and model instances.
    ...
    # The api_port is used in aistack server/worker serving API requests.
    ...
    # under in the cluster (``transformer`` vs ``aistack-header-transformer``,
    # ``ai-statistics`` vs ``aistack-ai-statistics``, ...). See
    # ``aistack.gateway.plugins.plugin_spec_overrides``.
    ...
    grafana_worker_dashboard_uid: Optional[str] = "aistack-worker"
    grafana_model_dashboard_uid: Optional[str] = "aistack-model"
    grafana_cache_service_dashboard_uid: Optional[str] = "aistack-cache-service"

    # Server-wide default for the AiStack Operator image. Cluster-level
    # ``k8s_options.operator_image`` overrides this when set.
    operator_image: str = f"aistack/aistack-operator:{__operator_version__}"
    ...
    # (then the built-in ``aistack-system`` default) via
    # ``TemplateConfig.namespace``.
    namespace: str = "aistack-system"
    ...
    model_config = SettingsConfigDict(
        env_prefix="AISTACK_", protected_namespaces=('settings_',), extra="allow"
    )
    ...
    def __init__(self, **values):
        ...
        # Signing key for the ``x-aistack-auth-cache`` marker, which the gateway
    ...
    def get_database_url(self) -> str:
        ...
        return (
            f"postgresql://root@127.0.0.1:{self.database_port}/aistack?sslmode=disable"
        )
    ...
    def detect_gateway_mode(self):
        ...
        if (
            ...
            raise Exception("The k8s cluster for aistack does not support Higress.")

        if self.gateway_plugin_server_url is None:
            # for embedded gateway model, higress will fetch plugins from aistack server
    ...
    def get_tls_secret_name(self) -> Optional[str]:
        ...
        if hostname:
            return f"aistack-tls-{hostname.replace('.', '-')}"
        else:
            return "aistack-tls-default"
...
def get_image_name(
    ...
    image_repo: str = "aistack/aistack",
) -> str:
    if image_name_override:
        return image_name_override
    # Lazy import: aistack.extension imports from this module at top level.
    from aistack.extension import resolve_version_info
