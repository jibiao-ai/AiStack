"""Configurable environment variables for AiStack."""
...
DB_ECHO = os.getenv("AISTACK_DB_ECHO", "false").lower() == "true"
...
DB_TRACE_SQL_SUBSTR = os.getenv("AISTACK_DB_TRACE_SQL_SUBSTR", "")
DB_POOL_SIZE = int(os.getenv("AISTACK_DB_POOL_SIZE", 30))
DB_MAX_OVERFLOW = int(os.getenv("AISTACK_DB_MAX_OVERFLOW", 20))
DB_POOL_TIMEOUT = int(os.getenv("AISTACK_DB_POOL_TIMEOUT", 30))
...
DB_IDLE_IN_TRANSACTION_SESSION_TIMEOUT_SECONDS = int(
    os.getenv("AISTACK_DB_IDLE_IN_TRANSACTION_SESSION_TIMEOUT_SECONDS", 8 * 3600)
)

# Proxy configuration
PROXY_TIMEOUT = int(os.getenv("AISTACK_PROXY_TIMEOUT_SECONDS", 1800))
PROXY_UPSTREAM_IDLE_TIMEOUT = int(
    os.getenv("AISTACK_PROXY_UPSTREAM_IDLE_TIMEOUT_SECONDS", 3)
)

# HTTP client TCP connector configuration
TCP_CONNECTOR_LIMIT = int(os.getenv("AISTACK_TCP_CONNECTOR_LIMIT", 1000))

# JWT Expiration
JWT_TOKEN_EXPIRE_MINUTES = int(os.getenv("AISTACK_JWT_TOKEN_EXPIRE_MINUTES", 120))
...
def deprecated_ai_statistics_content_types() -> List[str]:
    ...
    return _deprecated_gateway_env(
        "AISTACK_GATEWAY_AI_STATISTICS_PLUGIN_CONTENT_TYPES",
...
GATEWAY_AUTH_RECONCILE_INTERVAL_SECONDS = int(
    os.getenv("AISTACK_GATEWAY_AUTH_RECONCILE_INTERVAL_SECONDS", 30)
)
...
GATEWAY_AUTH_ALLOW_CUSTOM_KEYS = (
    os.getenv("AISTACK_GATEWAY_AUTH_ALLOW_CUSTOM_KEYS", "true").lower() != "false"
)
...
GATEWAY_AUTH_MAX_CR_BYTES = int(
    os.getenv("AISTACK_GATEWAY_AUTH_MAX_CR_BYTES", 1_100_000)
)

# Server Cache
SERVER_CACHE_TTL_SECONDS = int(os.getenv("AISTACK_SERVER_CACHE_TTL_SECONDS", 600))
SERVER_CACHE_LOCKS_MAX_SIZE = int(
    os.getenv("AISTACK_SERVER_CACHE_LOCKS_MAX_SIZE", 10000)
)
...
EVENT_BUS_SUBSCRIBER_QUEUE_SIZE = int(
    os.getenv("AISTACK_EVENT_BUS_SUBSCRIBER_QUEUE_SIZE", 1024)
)

# Worker configuration
WORKER_HEARTBEAT_INTERVAL = int(
    os.getenv("AISTACK_WORKER_HEARTBEAT_INTERVAL", 30)
)  # in seconds
WORKER_STATUS_SYNC_INTERVAL = int(
    os.getenv("AISTACK_WORKER_STATUS_SYNC_INTERVAL", 30)
)  # in seconds
WORKER_HEARTBEAT_GRACE_PERIOD = int(
    os.getenv("AISTACK_WORKER_HEARTBEAT_GRACE_PERIOD", 150)
)  # 2.5 minutes in seconds
WORKER_ORPHAN_WORKLOAD_CLEANUP_GRACE_PERIOD = int(
    os.getenv("AISTACK_WORKER_ORPHAN_WORKLOAD_CLEANUP_GRACE_PERIOD", 300)
)  # 5 minutes in seconds
WORKER_ORPHAN_BENCHMARK_WORKLOAD_CLEANUP_GRACE_PERIOD = int(
    os.getenv("AISTACK_WORKER_ORPHAN_BENCHMARK_WORKLOAD_CLEANUP_GRACE_PERIOD", 300)
)  # 5 minutes in seconds
...
WORKER_UNREACHABLE_CHECK_MODE = os.getenv(
    "AISTACK_WORKER_UNREACHABLE_CHECK_MODE", "auto"
).lower()
...
ENABLE_CUDA_MINOR_VERSION_COMPATIBILITY_ENV = (
    "AISTACK_ENABLE_CUDA_MINOR_VERSION_COMPATIBILITY"
)
...
GPU_INSTANCE_TRANSITIONING_REQUEUE_INTERVAL = int(
    os.getenv("AISTACK_GPU_INSTANCE_TRANSITIONING_REQUEUE_INTERVAL", 15)
)  # in seconds
...
GPU_INSTANCE_READY_SWEEP_INTERVAL = int(
    os.getenv("AISTACK_GPU_INSTANCE_READY_SWEEP_INTERVAL", 0)
)  # in seconds
...
GPU_INSTANCE_UNREADABLE_CR_TOLERANCE = int(
    os.getenv("AISTACK_GPU_INSTANCE_UNREADABLE_CR_TOLERANCE", 1800)
)  # in seconds

# Model instance configuration
MODEL_INSTANCE_RESCHEDULE_GRACE_PERIOD = int(
    os.getenv("AISTACK_MODEL_INSTANCE_RESCHEDULE_GRACE_PERIOD", 300)
)  # 5 minutes in seconds
MODEL_INSTANCE_HEALTH_CHECK_INTERVAL = int(
    os.getenv("AISTACK_MODEL_INSTANCE_HEALTH_CHECK_INTERVAL", 3)
)
...
MODEL_INSTANCE_STATE_RECONCILE_INTERVAL = int(
    os.getenv("AISTACK_MODEL_INSTANCE_STATE_RECONCILE_INTERVAL", 0)
)
DISABLE_OS_FILELOCK = os.getenv("AISTACK_DISABLE_OS_FILELOCK", "false").lower() in [
    "true",
    "1",
]

# Opt out of automatically writing aistack's configured port ranges to
...
SKIP_RESERVE_EPHEMERAL_PORTS = os.getenv(
    "AISTACK_SKIP_RESERVE_EPHEMERAL_PORTS", "false"
).lower() in ["true", "1"]
# Add debug logs for slow worker status collection, default to 3 minutes
WORKER_STATUS_COLLECTION_LOG_SLOW_SECONDS = float(
    os.getenv("AISTACK_WORKER_STATUS_COLLECTION_LOG_SLOW_SECONDS", 180)
)

# Model evaluation cache configuration
MODEL_EVALUATION_CACHE_MAX_SIZE = int(
    os.getenv("AISTACK_MODEL_EVALUATION_CACHE_MAX_SIZE", 1000)
)
MODEL_EVALUATION_CACHE_TTL = int(os.getenv("AISTACK_MODEL_EVALUATION_CACHE_TTL", 3600))

# Scheduler configuration (server-side)
SCHEDULER_SCALE_UP_PLACEMENT_MAX_SCORE = float(
    os.getenv("AISTACK_SCHEDULER_SCALE_UP_PLACEMENT_MAX_SCORE", 100)
)
SCHEDULER_SCALE_UP_LOCALITY_MAX_SCORE = float(
    os.getenv("AISTACK_SCHEDULER_SCALE_UP_LOCALITY_MAX_SCORE", 5)
)
# Scale-down scoring weights (relative, normalized in score chain)
SCHEDULER_SCALE_DOWN_STATUS_MAX_SCORE = float(
    os.getenv("AISTACK_SCHEDULER_SCALE_DOWN_STATUS_MAX_SCORE", 100)
)
SCHEDULER_SCALE_DOWN_OFFLOAD_MAX_SCORE = float(
    os.getenv("AISTACK_SCHEDULER_SCALE_DOWN_OFFLOAD_MAX_SCORE", 10)
)
SCHEDULER_SCALE_DOWN_PLACEMENT_MAX_SCORE = float(
    os.getenv("AISTACK_SCHEDULER_SCALE_DOWN_PLACEMENT_MAX_SCORE", 1)
)

MIGRATION_DATA_DIR = os.getenv("AISTACK_MIGRATION_DATA_DIR", None)

DATA_MIGRATION = os.getenv("AISTACK_DATA_MIGRATION", "false").lower() == "true"

GATEWAY_PORT_CHECK_INTERVAL = int(
    os.getenv("AISTACK_GATEWAY_PORT_CHECK_INTERVAL", 2)
)  # in seconds

GATEWAY_PORT_CHECK_RETRY_COUNT = int(
    os.getenv("AISTACK_GATEWAY_PORT_CHECK_RETRY_COUNT", 300)
)  # number of retries

GATEWAY_MIRROR_INGRESS_NAME = os.getenv(
    "AISTACK_GATEWAY_MIRROR_INGRESS_NAME", "aistack"
)
...
USAGE_ESTIMATED_BYTES_PER_INPUT_TOKEN = max(
    1, int(os.getenv("AISTACK_USAGE_ESTIMATED_BYTES_PER_INPUT_TOKEN", 4))
)
USAGE_ESTIMATED_TOKENS_PER_OUTPUT_CHUNK = max(
    1, int(os.getenv("AISTACK_USAGE_ESTIMATED_TOKENS_PER_OUTPUT_CHUNK", 1))
)
...
#
# ``AISTACK_USAGE_ROLLUP_TIMEZONE`` is the pre-rename name, kept as a
# deprecated alias: ``AISTACK_TIMEZONE`` wins when both are set; otherwise the
...
_timezone = os.getenv("AISTACK_TIMEZONE", "")
_legacy_rollup_timezone = os.getenv("AISTACK_USAGE_ROLLUP_TIMEZONE", "")
...
USAGE_DETAILS_RETENTION_MONTHS = int(
    os.getenv("AISTACK_USAGE_DETAILS_RETENTION_MONTHS", 13)
)
USAGE_DETAILS_ARCHIVE_CRON = os.getenv(
    "AISTACK_USAGE_DETAILS_ARCHIVE_CRON", "0 3 * * *"
)
...
USAGE_DETAILS_ARCHIVE_BATCH_SIZE = int(
    os.getenv("AISTACK_USAGE_DETAILS_ARCHIVE_BATCH_SIZE", 1000)
)
...
USAGE_DETAILS_BUFFER_MAX_SIZE = int(
    os.getenv("AISTACK_USAGE_DETAILS_BUFFER_MAX_SIZE", 100000)
)
...
USAGE_EVENTS_RETENTION_MONTHS = int(
    os.getenv("AISTACK_USAGE_EVENTS_RETENTION_MONTHS", 13)
)
USAGE_EVENTS_ARCHIVE_CRON = os.getenv(
    "AISTACK_USAGE_EVENTS_ARCHIVE_CRON", "30 3 * * *"
)
USAGE_EVENTS_ARCHIVE_BATCH_SIZE = int(
    os.getenv("AISTACK_USAGE_EVENTS_ARCHIVE_BATCH_SIZE", 5000)
)
...
METERED_USAGE_RETENTION_MONTHS = int(
    os.getenv("AISTACK_METERED_USAGE_RETENTION_MONTHS", 13)
)
METERED_USAGE_ARCHIVE_CRON = os.getenv(
    "AISTACK_METERED_USAGE_ARCHIVE_CRON", "0 4 * * *"
)
METERED_USAGE_ARCHIVE_BATCH_SIZE = int(
    os.getenv("AISTACK_METERED_USAGE_ARCHIVE_BATCH_SIZE", 5000)
)
...
RESOURCE_USAGE_TICK_SECONDS = int(
    os.getenv("AISTACK_RESOURCE_USAGE_TICK_SECONDS", 300)
)
STORAGE_USAGE_TICK_SECONDS = int(os.getenv("AISTACK_STORAGE_USAGE_TICK_SECONDS", 300))
...
METERED_USAGE_SEAL_GRACE_SECONDS = int(
    os.getenv("AISTACK_METERED_USAGE_SEAL_GRACE_SECONDS", 900)
)

DEFAULT_CLUSTER_KUBERNETES = (
    os.getenv("AISTACK_DEFAULT_CLUSTER_KUBERNETES", "false").lower() == "true"
)

# Benchmark configuration
BENCHMARK_DATASET_SHAREGPT_PATH = os.getenv(
    "AISTACK_BENCHMARK_DATASET_SHAREGPT_PATH",
    "/workspace/benchmark-runner/sharegpt_data/ShareGPT_V3_unfiltered_cleaned_split.json",
)
BENCHMARK_REQUEST_TIMEOUT = int(
    os.getenv("AISTACK_BENCHMARK_REQUEST_TIMEOUT", 3600)  # 1 hour
)  # in seconds
...
USAGE_BREAKDOWN_MAX_NO_PAGINATION_ROWS = int(
    os.getenv("AISTACK_USAGE_BREAKDOWN_MAX_NO_PAGINATION_ROWS", 50000)
)
...
USAGE_EXPORT_MAX_ROWS = int(os.getenv("AISTACK_USAGE_EXPORT_MAX_ROWS", 100000))
...
USAGE_EXPORT_SOFT_ROWS = int(os.getenv("AISTACK_USAGE_EXPORT_SOFT_ROWS", 10000))
...
# a reason to move them, and exposing them as AISTACK_* would advertise knobs
...
SCALING_SCHEDULER_INTERVAL = max(
    1, int(os.getenv("AISTACK_SCALING_SCHEDULER_INTERVAL", 30))
)  # in seconds
