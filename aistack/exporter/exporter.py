...
from aistack.exporter.bus_metrics import BusMetricsCollector
...
from aistack.schemas.models import CategoryEnum, Model
...
from aistack.utils.name import metric_name
import logging
...
class MetricExporter(Collector):
    ...
    async def _collect_metrics(self, session: AsyncSession):
        ...
        for cluster in clusters:
            ...
            for model in models:
                ...
                # scalar. AiStack currently treats the first entry as the
                # primary category for metrics, so secondary categories are not
                # exposed in aistack:model_info. This keeps one model_info
    ...
    async def start(self):
        try:
            ...
            app = FastAPI(
                title="AiStack Metrics Exporter", response_model_exclude_unset=True
            )
