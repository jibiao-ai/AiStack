"""
Extension plugin interface for AiStack.

Third-party plugins can implement this interface
and register via the ``aistack.plugins`` entry-point group.
...
if TYPE_CHECKING:
    from aistack.server.coordinator import Coordinator
...
def resolve_version_info() -> Tuple[str, str]:
    ...
    Used wherever core needs the *user-facing* version — ``aistack version``,
...
def iter_plugin_classes() -> Generator[Tuple[str, type], None, None]:
    """Iterate over registered plugin classes via the ``aistack.plugins``
    entry-point group.
    ...
    try:
        ...
        for ep in entry_points(group="aistack.plugins"):
            try:
...
class Plugin:
    ...
    def setup_start_cmd(cls, parser) -> None:
        """Contribute arguments to the ``aistack start`` argparse parser.
    ...
    def extra_image_list(cls) -> List[str]:
        """Contribute image names to the global AiStack image list.
    ...
    def get_version_info(cls) -> Optional[Tuple[str, str]]:
        """Override the version reported by ``aistack version`` and the
        ``/version`` endpoint.
