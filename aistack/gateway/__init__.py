...
from aistack.gateway import client as gw_client
from aistack.gateway.client import (
    McpBridge,
    McpBridgeSpec,
    McpBridgeRegistry,
    WasmPluginSpec,
)
from aistack.gateway.labels_annotations import managed_labels, match_labels
from aistack.gateway.utils import (
    default_mcp_bridge_name,
    openai_model_prefixes,
    anthropic_model_exact,
    aistack_ai_proxy_name,
    aistack_model_mapper_name,
    aistack_generic_proxy_router_name,
    mcp_ingress_equal,
    get_default_mcpbridge_ref,
    ensure_wasm_plugin,
    router_header_key,
    aistack_original_path_header,
    aistack_fallback_path_header,
)
from aistack.gateway.ext_auth import (
    ext_auth_init_spec_diff,
    ext_auth_resource_name,
    ext_auth_spec,
)
from aistack.gateway.plugins import plugin_entry, plugin_spec_overrides
...
def get_aistack_higress_registry(cfg: Config) -> McpBridgeRegistry:
    registry_type = "dns"
...
async def ensure_mcp_resources(cfg: Config, api_client: k8s_client.ApiClient):
    ...
    try:
        ...
        if e.status == 404:
            ...
            raise
    target_registry = get_aistack_higress_registry(cfg=cfg)
    try:
...
async def ensure_ingress_resources(cfg: Config, api_client: k8s_client.ApiClient):
    ...
    try:
        ...
        if e.status == 404:
            ...
            raise
    registry = get_aistack_higress_registry(cfg=cfg)
    ...
    if not ingress:
        ...
        # only update ingress managed by aistack
...
def ext_auth_plugin(cfg: Config) -> Tuple[str, WasmPluginSpec]:
    ...
    Scoped to aistack's own inference routes by route-name prefix, so
    ...
    :class:`~aistack.server.gateway_auth_reconciler.GatewayAuthReconciler`;
    see :func:`aistack.gateway.ext_auth.ext_auth_init_spec_diff` for how they
    survive this rewrite on restart.
    """
    return ext_auth_resource_name, ext_auth_spec(
        cfg=cfg, registry=get_aistack_higress_registry(cfg=cfg)
    )


# Manifest name, which is also the ``gateway_plugin`` key -- distinct from the
# ``aistack-ai-statistics`` resource it is deployed as. Kept beside the model
...
class AiStatisticsOverride(BaseModel):
    ...
    #
    # Defaults to the deprecated AISTACK_GATEWAY_AI_STATISTICS_PLUGIN_CONTENT_TYPES
...
def ai_statistics_plugin(cfg: Config) -> Tuple[str, WasmPluginSpec]:
    resource_name = "aistack-ai-statistics"
...
def model_pre_route_plugin(cfg: Config) -> Tuple[str, WasmPluginSpec]:
    resource_name = "aistack-set-model-pre-route"
    ...
    expected_spec = WasmPluginSpec(
        defaultConfig={
            ...
            'routeNameHeader': 'X-AiStack-Route-Name',
        ...
        phase="AUTHN",
        priority=90,
        **plugin_spec_overrides("aistack-set-header-pre-route", cfg=cfg),
    )
    return resource_name, expected_spec


def model_mapper_plugin(cfg: Config) -> Tuple[str, WasmPluginSpec]:
    return aistack_model_mapper_name, WasmPluginSpec(
        phase="AUTHN",
        priority=800,
        **plugin_spec_overrides("aistack-model-mapper", cfg=cfg),
        defaultConfigDisable=False,
...
def transformer_plugin(cfg: Config) -> Tuple[str, WasmPluginSpec]:
    resource_name = "aistack-header-transformer"
    expected_spec = WasmPluginSpec(
        defaultConfig={
            "reqRules": [
                ...
                transform_header(
                    "rename",
                    HeaderRule(
                        oldKey="x-aistack-model",
                        newKey="x-higress-llm-model",
                    ),
                    HeaderRule(
                        oldKey=aistack_fallback_path_header,
                        newKey=":path",
                    ),
                ),
                transform_header(
                ...
                transform_header(
                    ...
                    HeaderRule(
                        fromKey=':path',
                        toKey=aistack_original_path_header,
                    ),
                ),
                transform_header(
                    "remove",
                    HeaderRule(
                        key=aistack_fallback_path_header,
                    ),
                ),
            ],
        },
...
def generic_proxy_router_plugin(cfg: Config) -> Tuple[str, WasmPluginSpec]:
    ...
    1. **Path-driven**: when ``:path`` matches ``prefix`` (``/model/proxy/``),
       ...
       multipart). Used by aistack's generic-proxy routes.
    ...
    # ``aistack-model-router`` slot previously occupied by Higress's built-in
    ...
    # ``aistack-generic-proxy-router`` (its identity in the plugin manifest).
    resource_name = aistack_generic_proxy_router_name
    enabled_path_suffixes = supported_openai_routes + supported_anthropic_routes
    expected_spec = WasmPluginSpec(
        defaultConfig={
            "prefix": "/model/proxy/",
        ...
        phase="AUTHN",
        priority=900,
        **plugin_spec_overrides("aistack-generic-proxy-router", cfg=cfg),
    )
    return resource_name, expected_spec
...
def token_usage_plugin(cfg: Config) -> Tuple[str, WasmPluginSpec]:
    registry = get_aistack_higress_registry(cfg=cfg)
    resource_name = "aistack-token-usage"
    expected_spec = WasmPluginSpec(
        defaultConfig={
            'endpoint': {
        ...
        phase="UNSPECIFIED_PHASE",
        priority=400,
        **plugin_spec_overrides("aistack-token-usage", cfg=cfg),
    )
    return resource_name, expected_spec


def ai_proxy_plugin(cfg: Config) -> Tuple[str, WasmPluginSpec]:
    resource_name = aistack_ai_proxy_name
    expected_spec = WasmPluginSpec(
        defaultConfig={},
        defaultConfigDisable=False,
        failStrategy="FAIL_OPEN",
        ...
        priority=100,
        phase="UNSPECIFIED_PHASE",
        **plugin_spec_overrides("aistack-ai-proxy", cfg=cfg),
    )
    return resource_name, expected_spec
    ...
    if cfg.gateway_mode in [
        ...
        async def prepare():
            ...
            for plugin_name, plugin_spec in plugin_list:
                if plugin_name == aistack_generic_proxy_router_name:
