...
:mod:`aistack.server.gateway_auth_reconciler`, which recomputes them from the
...
from aistack.api.auth import GATEWAY_AUTH_TOKEN_HEADER
...
from aistack.gateway.utils import model_route_ingress_prefix
from aistack.security import AUTH_CACHE_HEADER
...
class AuthzOverride(BaseModel):
    ...
    ``endpoint`` is not settable: it points at aistack's own ``/token-auth``
    ...
    #
    # Defaults to the deprecated AISTACK_HIGRESS_EXT_AUTH_TIMEOUT_MS when that
...
class ExtAuthOverride(BaseModel):
    ...
    * ``upstream_request.headers_to_remove`` -- dropping ``cookie`` or
    ...
    # fail closed. AiStack overrides it deliberately: the gateway is here to
...
def ext_auth_module_source(cfg: Config) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ValueError(
            f"{e} Upgrade the aistack-higress-plugins package, or point "
...
def model_route_prefix(cfg: Config) -> str:
    """Route-name prefix covering every aistack inference route.
...
def ext_auth_default_config(
    ...
    return {
        ...
        "authz": {
            ...
            # is passed back unconditionally, X-AiStack-Key-Ref is for the
        ...
        "upstream_request": {
            ...
            # authenticating with that header had its AiStack key forwarded
