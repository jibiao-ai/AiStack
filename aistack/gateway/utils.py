...
from aistack.gateway import ai_proxy_types
...
from aistack.server.services import ModelInstanceService, WorkerService
from aistack.schemas.config import ModelInstanceProxyModeEnum
from aistack.schemas.workers import Worker
from aistack.schemas.clusters import Cluster
from aistack.utils.network import is_ipaddress
from kubernetes_asyncio import client as k8s_client
from kubernetes_asyncio.client import ApiException, V1IngressTLS
from aistack.envs import GATEWAY_MIRROR_INGRESS_NAME
from aistack.api.exceptions import NotFoundException
from aistack.websocket_proxy.message import ServerInfo, RegisteredClientInfo

logger = logging.getLogger(__name__)

default_mcp_bridge_name = "default"
aistack_ai_proxy_name = "aistack-ai-proxy"
aistack_model_mapper_name = "aistack-model-mapper"
aistack_generic_proxy_router_name = "aistack-model-router"
...
model_ai_proxy_provider_prefix = "aistack-model-"
...
router_header_key = "X-AiStack-Model-Instance"
aistack_original_path_header = "x-aistack-original-path"
aistack_fallback_path_header = "x-aistack-fallback-path"
...
def generate_model_ingress(
    ...
    if included_proxy_route:
        ...
        # /model/proxy/<path> + X-AiStack-Model header form. The more specific rule
...
async def cleanup_model_mapper(
    ...
    await ensure_wasm_plugin(
        api=api,
        name=aistack_model_mapper_name,
        namespace=namespace,
        spec_diff=spec_diff,
...
async def ensure_fallback_filter(
    ...
    expected_filter = get_ingress_fallback_envoyfilter(
        ...
        labels={**managed_labels},
        extra_req_headers={
            aistack_fallback_path_header: f'%REQ({aistack_original_path_header.upper()})%'
        },
    )
...
async def cleanup_ai_proxy_config(
    ...
    *not* pruned here: each route retires its own when it reconciles, in the same
    ...
    try:
        ...
        logger.error(
            f"Failed to cleanup aistack AI proxy wasmplugin {aistack_ai_proxy_name}: {e}"
        )
        raise
...
async def cleanup_generic_proxy_router(
    ...
    await ensure_wasm_plugin(
        api=api,
        name=aistack_generic_proxy_router_name,
        namespace=namespace,
        spec_diff=partial(
...
def get_instance_id_from_header(headers: Mapping[str, str]) -> int:
    """Parse the model instance ID from the ``x-aistack-model-instance`` routing header.
...
async def resolve_instance_address_from_model_header(
    ...
    Parses the ``x-aistack-model-instance`` routing header injected by the API gateway
