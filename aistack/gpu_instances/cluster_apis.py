...
_DEFAULT_GROUP = "worker.aistack.ai"
_DEFAULT_VERSION = "v1"
...
class _Scope(Enum):
    ...
    lands in a namespace AiStack creates on demand, while a system-namespaced
    one lives beside the operator in the cluster's own system namespace, which
    AiStack does not own and must never create.
    """
...
class _CRDSpec:
    ...
    ``group`` / ``version`` default to ``worker.aistack.ai/v1`` because that
...
# ``worker.aistack.ai``: it lives in ``aistack.ai/v1``, namespaced beside the
...
class ClusterOps:
    ...
    group/version its :class:`_CRDSpec` names (``worker.aistack.ai/v1`` unless
    ...
    def __init__(
        ...
        ``aistack-system``, the same default the manifest renderer applies, so
    ...
    async def _create(
        ...
        # there and is not AiStack's to provision.
        if spec.scope is _Scope.ORG_NAMESPACED:
