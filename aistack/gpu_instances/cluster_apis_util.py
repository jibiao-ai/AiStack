...
def spec_persistent_volume(pv: GPUInstancePersistentVolume) -> dict:
    ...
    shape expected by the ``worker.aistack.ai/v1`` InstancePersistentVolume
...
def spec_persistent_volume_type(
    ...
    dict shape expected by the ``worker.aistack.ai/v1``
...
def get_k8s_client(
    ...
    api.user_agent = "aistack/aistack"
    return api
...
DEFAULT_SYSTEM_NAMESPACE = "aistack-system"
"""Namespace a cluster's own AiStack components (operator, workers) live in
...
def get_persistent_volume_type_name(
    ...
    """
    Get the AiStack-Operator InstancePersistentVolumeType name for
...
def parse_persistent_volume_type_name(
    ...
    AiStack-Operator InstancePersistentVolumeType name.
