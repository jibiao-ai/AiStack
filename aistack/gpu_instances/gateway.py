"""Subscribe Kubernetes-provider clusters to the aistack-operator worker gateway.
...
every probe is logged as a server error (aistack#5947).
"""
...
_SOURCE = "aistack_operator_subscription"

_GVK = [
    ("worker.aistack.ai", "v1", "InstanceType"),
    ("worker.aistack.ai", "v1", "Instance"),
]


async def reconcile_aistack_operator_subscription():
    ...
    async def _watch(self, name, stream, reconcile):
        async for event in stream:
            try:
                ...
                logger.exception(
                    f"Failed to reconcile aistack-operator subscription "
                    f"on a {name} event: {e}"
                )
