"""HTTP client for the ``aistack-operator`` worker-gateway.

The aistack-operator's worker gateway serves the ``/apis/*`` routes documented in
``github.com/aistack-operator/pkg/workergateway/service``.

It binds to a TLS-on-unix socket whose path is generated per server startup
(see ``aistack.server.server.Server._enqueue_operator_process``) and handed to
this module via :func:`set_unix_path`.

The certificate is self-signed by the aistack-operator at startup,
...
_BASE_URL = "https://aistack-operator/apis"
...
async def _stream(path: str, params: list[tuple[str, str]]) -> AsyncIterator[str]:
    ...
    framing produced by :meth:`aistack.mixins.active_record.streaming` so
