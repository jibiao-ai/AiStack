"""Converge the operator settings AiStack manages onto each GPU Service cluster.

The operator seeds every ``Setting`` from ``AISTACK_<UPPER_SNAKE_NAME>`` on its
...
from aistack.gpu_instances.gateway import count_ready_workers, deleted_cluster_id
from aistack.schemas.clusters import Cluster, ClusterProvider, is_gpu_service_cluster
from aistack.schemas.principals import PLATFORM_PRINCIPAL_NAME
from aistack.schemas.workers import Worker, WorkerStateEnum
from aistack.server.bus import Event, EventType, event_field

logger = logging.getLogger(__name__)

_SOURCE = "aistack_operator_settings"

# One entry per knob AiStack can manage: the ``GpuInstanceOptions`` field, its
...
silently disagreeing with what AiStack shows. It is a divisor on the cost of
that guarantee: each sweep is one read per managed knob per GPU Service cluster,
over the cluster proxy.
"""


async def reconcile_aistack_operator_settings(config: Config):
...
class OperatorSettingsReconciler:
    ...
    :class:`~aistack.gpu_instances.gateway.OperatorSubscriptionReconciler`:
    ...
    Only the settings AiStack manages are ever touched: an unset knob is not
    ...
    async def _reconcile_cluster(self, event: Event):
        ...
        # (see :func:`~aistack.gpu_instances.gateway.deleted_cluster_id`). The
    ...
    async def _reconcile_worker(self, event: Event):
        ...
        # (see :func:`~aistack.gpu_instances.gateway.deleted_cluster_id`).
...
def _target_of(cluster: Cluster) -> Optional[_Target]:
    ...
    return _Target(
        ...
        # ``aistack-system`` when the cluster did not name one.
...
def _desired_settings(cluster: Cluster) -> Dict[str, str]:
    """The operator settings AiStack manages for this cluster, on-the-wire.
