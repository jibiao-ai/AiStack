...
from aistack import __operator_version__
...
_DEFAULT_CONTAINER_NAMESPACE = "aistack"
_DEFAULT_CLUSTER_NAMESPACE = "aistack-system"


WORKER_DS_BASENAME = "aistack-worker"
IMAGE_PULL_SECRET_NAME_PREFIX = "aistack-image-pull-secret"
...
_CPU_NODE_LABEL = "feature.aistack.ai/acceleratable"
...
def _env_bool(value: Optional[bool]) -> Optional[str]:
    ...
    ``AISTACK_*`` environment variable carries.
...
class WorkerRenderSpec(BaseModel):
    ...
    (named ``aistack-worker``), plus one entry per requested GPU runtime
    (each named ``aistack-worker-<runtime>``).
...
class TemplateConfig(ClusterRegistrationTokenPublic):
    ...
    # "aistack-{cluster_owner_principal_identifier}", used to place the
    ...
    # image registry prefix and the AISTACK_CONTAINER_REGISTRY env var
    ...
    def multi_vendor_mode(self) -> bool:
        ...
        ``app.kubernetes.io/component``/``aistack.io/runtime`` labels +
    ...
    def namespace(self) -> str:
        ...
        it — and falls back to the built-in ``aistack-system`` default
    ...
    def operator_image(self) -> str:
        ...
        via ``AISTACK_OPERATOR_IMAGE``) when the cluster doesn't override it —
    ...
    def container_namespace(self) -> Optional[str]:
        """
        Namespace segment inferred from the resolved aistack image — used by
        the operator runtime (``AISTACK_CONTAINER_NAMESPACE``) to compose
        sibling image references. The operator image may live elsewhere, so the
        namespace must come from the aistack image (``self.image``) instead.
        ...
        like ``quay.io/aistack/aistack:dev``); the latter is detected with
        ...
        ``aistack`` default since the operator already knows that one.
        """
        image = self.image
        registry = (self.system_default_container_registry or "").strip().rstrip("/")
    ...
    def operator_instance_type_derived_from_node(self) -> Optional[str]:
        ...
        setting. The operator seeds each ``Setting`` from ``AISTACK_<NAME>``
    ...
    def render(self) -> str:
        ...
        with pkg_resources.path("aistack.k8s", "manifests.jinja") as manifest_path:
            with manifest_path.open(encoding="utf-8") as f:
                template_data = f.read()
        with pkg_resources.path("aistack.k8s", "image_pull_secrets.jinja") as ips_path:
            with ips_path.open(encoding="utf-8") as f:
                ips_data = f.read()
        with pkg_resources.path("aistack.k8s", "daemonset.jinja") as daemon_set_path:
            with daemon_set_path.open(encoding="utf-8") as f:
                daemon_set_data = f.read()
        with pkg_resources.path("aistack.k8s", "operator.jinja") as operator_path:
    ...
    def _build_image_pull_secrets(self) -> List[ImagePullSecretRenderSpec]:
        ...
        for i, cred in enumerate(self.k8s_options.image_credentials):
            ...
            # the aistack Helm chart's canonical pull-secret uses when no
...
def _cpu_node_selector_for(
    ...
    (``feature.aistack.ai/acceleratable: "false"``) so the DaemonSet only
