...
import aistack
import secrets
from aistack.schemas.stmt import (
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('created_at', aistack.schemas.common.UTCDateTime(), nullable=False, server_default=now_func()),
        sa.Column('updated_at', aistack.schemas.common.UTCDateTime(), nullable=False, server_default=now_func(), onupdate=now_func()),
    ...
    if should_create_cluster:
        ...
        op.execute(f"""
            ...
            VALUES ('Default Cluster', 'The default cluster for AiStack', 'Docker', 'READY', '{secrets.token_hex(6)}', '', {utc_now_sql_literal()}, {utc_now_sql_literal()})
        """)
