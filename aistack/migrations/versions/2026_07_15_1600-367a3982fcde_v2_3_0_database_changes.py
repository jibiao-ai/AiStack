...
1. GPU instance types: a ``gpu_instance_types`` table holding the per-cluster
   ...
   ``schedule.aistack.ai/derived-from-node`` label). It cannot be inferred from
...
3. ``api_keys.secret_key_digest``: a cheap second verifier
   (``sha256$<salt>$<hash>``) for the secrets AiStack generates itself, so
...
import sqlmodel
import aistack
from aistack.schemas.common import JSON, UTCDateTime
from aistack.migrations.utils import column_exists, table_exists
...
# ``system/worker-<suffix>`` — see ``aistack.schemas.users.system_name_prefix``
# and ``aistack.routes.workers.system_name_prefix``. Scoping the cleanup to
...
# Precision mirrors aistack.schemas.metered_usage.SKU_COUNT_{PRECISION,SCALE};
...
def upgrade() -> None:
    ...
    if not table_exists('gpu_instance_types'):
        op.create_table(
            'gpu_instance_types',
            sa.Column(
                'created_at', aistack.schemas.common.UTCDateTime(), nullable=False
            ),
            sa.Column(
                'updated_at', aistack.schemas.common.UTCDateTime(), nullable=False
            ),
            sa.Column(
                'deleted_at', aistack.schemas.common.UTCDateTime(), nullable=True
            ),
            sa.Column('id', sa.Integer(), nullable=False),
            sa.Column('cluster_id', sa.Integer(), nullable=False),
            sa.Column('name', sqlmodel.sql.sqltypes.AutoString(), nullable=False),
            sa.Column('spec', aistack.schemas.common.JSON(), nullable=False),
            sa.Column('status', aistack.schemas.common.JSON(), nullable=True),
