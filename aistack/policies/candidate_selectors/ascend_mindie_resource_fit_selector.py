...
from aistack.policies.candidate_selectors.base_candidate_selector import (
    ModelAttentionTypeEnum,
    RequestEstimateUsage,
    ScheduleCandidatesSelector,
)
from aistack.policies.event_recorder.recorder import EventCollector
...
from aistack.worker.backends.ascend_mindie import AscendMindIEParameters
...
class AscendMindIEResourceFitSelector(ScheduleCandidatesSelector):
    ...
    async def _estimate_usage(  # noqa: C901
        ...
        if self._model.env:
            reqeust_ram = safe_int(self._model.env.get("AISTACK_MODEL_RAM_CLAIM", 0))
            request_vram = safe_int(self._model.env.get("AISTACK_MODEL_VRAM_CLAIM", 0))
            if request_vram > 0:
