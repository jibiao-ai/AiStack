...
from aistack.api.exceptions import BadRequestException, NotFoundException
from aistack.api.tenant import TenantContext, cluster_visibility_conditions
from aistack.gpu_instances import gateway_client
from aistack.gpu_instances.cluster_apis import ClusterOps
from aistack.gpu_instances.gateway import count_ready_workers
from aistack.routes.gpu_instances_helper import (
...
from aistack.schemas.clusters import ClusterProvider, is_gpu_service_cluster
from aistack.schemas.common import Pagination
from aistack.schemas.gpu_instance_types import DERIVED_FROM_NODE_LABEL
...
async def get_gpu_aggregated_instance_types(
    ...
    if watch:
        ...
        # DELETED); wrap them into AiStack events so a caller sees the same
        # event-type contract as every other AiStack stream.
...
def _to_instance_type_public(item: dict) -> GPUInstanceTypePublic:
    """Map a raw ``worker.aistack.ai/v1`` InstanceType dict into the public
