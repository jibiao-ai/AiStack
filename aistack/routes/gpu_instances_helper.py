...
Most helpers sit on top of :class:`ClusterOps` (the raw ``worker.aistack.ai/v1``
...
watch routes, framing a normalized watch source as AiStack SSE.
"""
...
from aistack.gpu_instances.cluster_apis_util import principal_namespace_identifier
from aistack.schemas import Cluster
from aistack.schemas.clusters import is_gpu_service_cluster
from aistack.schemas.principals import PLATFORM_PRINCIPAL_NAME, Principal
from aistack.server.bus import Event, EventType

logger = logging.getLogger(__name__)

# Kubernetes watch verbs → AiStack EventType. Any other verb (notably
# BOOKMARK / ERROR) is absent here and dropped from the stream, so a AiStack
...
async def watch_event_stream(
    ...
    """Frame a ``(verb, object)`` watch source as AiStack SSE.

    A background task drains ``events`` into a bounded queue; the consumer maps
    each Kubernetes watch verb (ADDED/MODIFIED/DELETED) to a AiStack EventType,
    ...
    try:
        while True:
            ...
            if item is _DONE:
                return
            verb, obj = item
            mapped = _K8S_TO_AISTACK.get(verb)
            if mapped is None:
                continue
