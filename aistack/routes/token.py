...
from aistack.server.services import ModelRouteService, UserService
from aistack.schemas.api_keys import ApiKey
from aistack.schemas.users import User
from aistack.schemas.models import AccessPolicyEnum
...
from aistack.security import JWTManager, AUTH_CACHE_HEADER
from aistack.server.gateway_auth_reconciler import gateway_ref_indexable
...
def _build_consumer(access_key: Optional[str], user: User) -> str:
    ...
    dot, ``.aistack-<id>``.
    """
    return ".".join([part for part in [access_key, f"aistack-{user.id}"] if part])
