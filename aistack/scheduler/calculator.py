...
from aistack.client.worker_filesystem_client import WorkerFilesystemClient
...
from aistack.utils.convert import parse_duration, safe_int
from aistack.utils.hub import (
    filter_filename,
    list_repo,
...
async def _gguf_parser_command(
    ...
    bin_path = pkg_resources.files("aistack.third_party.bin.gguf-parser").joinpath(
...
async def get_pretrained_config_with_workers(
    ...
    try:
        ...
        if model.env and model.env.get("AISTACK_SKIP_MODEL_EVALUATION"):
...
async def _calculate_from_workers(  # noqa: C901
    ...
    **kwargs,
    ...
    # Apply GPUMatchingFilter
    if model.gpu_selector:
        from aistack.policies.worker_filters.gpu_matching_filter import (
            GPUMatchingFilter,
        )
    ...
    # Apply LabelMatchingFilter
    if model.worker_selector:
        from aistack.policies.worker_filters.label_matching_filter import (
            LabelMatchingFilter,
        )
