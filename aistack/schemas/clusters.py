...
from aistack.schemas.common import (
    PublicFields,
    ListParams,
    PaginatedList,
...
if TYPE_CHECKING:
    ...
    from aistack.schemas.principals import Principal
...
class ImageCredential(BaseModel):
    ...
    ``aistack-image-pull-secret-<index>`` in the worker namespace, which
    ...
    payload when either is missing, matching the aistack Helm chart
    convention.
    """
...
class GpuInstanceOptions(BaseModel):
    ...
    Each knob mirrors a aistack-operator ``Setting`` of the same name and is
    **tri-state**: ``None`` means AiStack does not manage that setting and the
    ...
    gpu_instance_type_derived_from_node: Optional[bool] = PydanticField(
        ...
        description=(
            ...
            "true). Unset means AiStack does not manage it and the cluster's "
            "own value stands."
        ),
    )
    gpu_instance_type_mixed_on_node: Optional[bool] = PydanticField(
        ...
        description=(
            ...
            "Unset means AiStack does not manage it and the cluster's own "
            "value stands."
        ),
    )
...
class K8sOptions(BaseModel):
    ...
    image_credentials: Optional[List[ImageCredential]] = PydanticField(
        ...
        description=(
            ...
            "``aistack-image-pull-secret-<index>`` in the worker namespace "
    ...
    volume_mounts: Optional[List[K8sVolumeMount]] = PydanticField(
        ...
        description=(
            ...
            "DaemonSet. The first entry is reserved for the aistack data "
    ...
    operator_image: Optional[str] = PydanticField(
        ...
        alias="operatorImage",
        description=(
            "Override for the aistack-operator container image. Falls back "
            "to the server's AISTACK_OPERATOR_IMAGE / built-in default when unset."
        ),
    )
    ...
    namespace: Optional[str] = PydanticField(
        ...
        description=(
            ...
            "Falls back to ``aistack-system`` at render time when unset."
        ),
    )
...
class ClusterUpdate(SQLModel):
    ...
    # AISTACK_SYSTEM_DEFAULT_CONTAINER_REGISTRY when unset.
