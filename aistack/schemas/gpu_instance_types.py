...
class GPUInstanceType(SQLModel, BaseModelMixin, table=True):
    """
    Server-side projection of a cluster's ``worker.aistack.ai/v1`` InstanceType.
    ...
    """
    Whether the AiStack Operator derived this type from a node's resource
...
DERIVED_FROM_NODE_LABEL = "schedule.aistack.ai/derived-from-node"
"""
Label the AiStack Operator stamps on the GPU instance types it derives from a
...
class GPUInstanceTypePublic(GPUInstanceTypeBase):
    ...
    """
    Whether the AiStack Operator derived this GPU instance type from a node's
