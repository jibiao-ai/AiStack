...
class GPUInstanceResources(BaseModel):
    ...
    e.g. "3g.40gb". A non-empty value makes this a request for one hardware
    partition of that shape, which is mutually exclusive with the two slice
    percentages above: hardware partitioning and software slicing cannot both
    apply to one card. It is ignored by InstanceTypes offering no partition.
    """
...
class GPUInstanceImagePullSecretReference(BaseModel):
    ...
    name: str
    """
    Name of the AiStack Operator InstanceImagePullSecret to use for pulling container images.
    """
...
class GPUInstanceSpec(BaseModel):
    ...
    """
    Optional reference to a AiStack Operator InstanceImagePullSecret for pulling container images.
    """
...
# The AiStack-defined failure phases, enumerated for user-action gating / UX
...
# Label stamped on the worker CR's ``metadata.labels`` carrying the AiStack
...
KUBERES_INSTANCE_ID_LABEL = "aistack.ai/instance-id"
...
class GPUInstance(GPUInstanceBase, BaseModelMixin, table=True):
    ...
    def is_failed(self) -> bool:
        ...
        The worker CR may report failure phases AiStack does not define, so
        this uses the broad ``endswith("Failed")`` rather than membership in
        :data:`FAILED_PHASES` (which enumerates only the AiStack-defined
    ...
    def convert_to_kuberes(self) -> dict:
        """Full ``worker.aistack.ai/v1`` Instance CR body: ``metadata`` + ``spec``.
