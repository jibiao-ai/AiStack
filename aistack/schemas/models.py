...
from aistack.utils.command import find_parameter, find_bool_parameter
from aistack.schemas.model_routes import (
    ModelRoute,
    ModelRouteTarget,
    AccessPolicyEnum,
)
from aistack.schemas.principals import _platform_principal_id
...
class GPUTypeSelector(BaseModel):
    """
    Selects a sliced GPU from a aistack-operator InstanceType pool.
...
class DistributedServerCoordinateModeEnum(Enum):
    ...
    DELEGATED = "delegated"
    # INITIALIZE_LATER means that the subordinate workers' coordinate is handled by AiStack,
    ...
    # RUN_FIRST means that the subordinate workers' coordinate is handled by AiStack,
