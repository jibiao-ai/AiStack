...
from aistack.policies.scorers.offload_layer_scorer import OffloadLayerScorer
from aistack.policies.scorers.placement_scorer import PlacementScorer, ScaleTypeEnum
from aistack.policies.scorers.score_chain import (
    ModelInstanceScoreChain,
)
from aistack.policies.base import ModelInstanceScore
from aistack.policies.worker_filters.label_matching_filter import label_matching
from aistack.policies.scorers.status_scorer import StatusScorer
from aistack.schemas.inference_backend import (
    InferenceBackend,
    get_built_in_backend,
)
from aistack.schemas.links import ModelRoutePrincipalLink
from aistack.schemas.model_files import ModelFile, ModelFileStateEnum
...
from aistack.schemas.links import (
    ModelInstanceModelFileLink,
    ModelInstanceDraftModelFileLink,
)
from aistack.utils.lora_model_source import (
    lora_entry_to_model_source,
    lora_route_name_for,
    normalized_lora_list,
    model_base_descriptor,
)
from aistack.schemas.config import (
    GatewayModeEnum,
    SensitivePredefinedConfig,
)
from aistack.schemas.cache_services import (
...
from aistack.server.cache_provider_catalog import get_cache_provider
from aistack.server.cache_services import resolve_instance_cache_config_safe
...
from aistack.schemas.runner_source import (
    InferenceRunnerSource,
    reconcile_runner_overrides,
)
from aistack.schemas.catalog_source import (
    BUILTIN_CATALOG_SOURCE_NAME,
    CatalogSource,
    normalize_catalog_yaml,
    reconcile_catalog,
)
from aistack.schemas.inference_backend_source import (
...
from aistack.schemas.source import SourceTypeEnum
from aistack.server.sources.core import gather_and_merge, sha256_of
from aistack.server.bus import (
    Event,
    EventType,
    event_bus,
    event_field,
...
from aistack.server.cache import delete_cache_by_key
from aistack.utils.model_source import get_draft_model_source
from aistack import envs
from aistack.server.db import async_session
from aistack.server.services import (
    ModelFileService,
...
from aistack.server.lora_model_routes import cleanup_orphan_lora_routes
from aistack.utils.model_instance_workers import get_model_instance_worker_match
from aistack.cloud_providers.common import (
    get_client_from_provider,
    construct_cloud_instance,
    generate_ssh_key_pair,
)
from aistack.cloud_providers.abstract import (
    ProviderClientBase,
    CloudInstance,
...
from aistack.gateway.client.networking_istio_io_v1alpha3_api import (
    NetworkingIstioIoV1Alpha3Api,
)
from aistack.gateway import utils as mcp_handler
...
async def sync_model_route_mapper(
    ...
    await mcp_handler.ensure_wasm_plugin(
        api=extensions_api,
        name=mcp_handler.aistack_model_mapper_name,
...
async def ensure_route_generic_proxy_router_config(
    ...
    await mcp_handler.ensure_wasm_plugin(
        api=extensions_api,
        name=mcp_handler.aistack_generic_proxy_router_name,
        namespace=cfg.gateway_namespace,
        spec_diff=partial(
...
async def ensure_route_ai_proxy_config(
    ...
    await mcp_handler.ensure_wasm_plugin(
        api=extensions_api,
        name=mcp_handler.aistack_ai_proxy_name,
        namespace=cfg.gateway_namespace,
        spec_diff=partial(
...
class InferenceBackendController:
    ...
    async def _seed_builtin_source(self):
        ...
        (so a AiStack upgrade ships it), while the user's ``enabled`` toggle on
        the built-in source is left untouched.
        """
        try:
            yaml_file = files("aistack.assets").joinpath(
                "community-inference-backends.yaml"
            )
