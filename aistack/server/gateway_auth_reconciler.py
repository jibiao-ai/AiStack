...
from aistack.gateway import get_async_k8s_config, get_aistack_higress_registry
from aistack.gateway.client.extensions_higress_io_v1_api import (
    ExtensionsHigressIoV1Api,
)
from aistack.gateway.client.networking_higress_io_v1_api import McpBridgeRegistry
from aistack.gateway.ext_auth import (
    ext_auth_reconcile_spec_diff,
    ext_auth_resource_name,
)
from aistack.gateway.utils import ensure_wasm_plugin, route_ingress_names_for_plugins
from aistack.schemas.api_keys import ApiKey
from aistack.schemas.config import GatewayModeEnum
from aistack.schemas.model_routes import AccessPolicyEnum, ModelRoute
from aistack.schemas.principals import Principal, PrincipalType
from aistack.security import gateway_digest
from aistack.server.bus import EventType, event_field
...
def gateway_ref_indexable(api_key: ApiKey, principal: Principal) -> bool:
    ...
    ``X-AiStack-Key-Ref`` for it, and the two must agree. A ref the gateway
    ...
    * **keys that have already expired.** Not for correctness -- ``exp`` rides
    ...
    for key_id, access_key, digest, expires_at, user_id, is_custom in rows:
        ...
        if publishable:
            ...
            # Just the id: the plugin rebuilds "<access_key>.aistack-<id>"
...
class GatewayAuthReconciler:
    ...
    async def start(self):
        ...
        self._registry = get_aistack_higress_registry(cfg=self._config)
