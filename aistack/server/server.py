...
from aistack.gpu_instances import sync_builtin_templates_to_db
from aistack.logging import setup_logging
from aistack.schemas.users import (
    User,
...
from aistack.schemas.models import Model, ModelInstance
from aistack.schemas.api_keys import ApiKey
from aistack.schemas.workers import Worker
from aistack.schemas.clusters import Cluster, ClusterProvider, ClusterStateEnum
from aistack.schemas.model_routes import ModelRoute, ModelRouteTarget
from aistack.schemas.model_provider import ModelProvider
from aistack.security import (
    generate_access_key,
    generate_secret_key,
...
from aistack.server.app import create_app
from aistack.server.passwords import set_password
from aistack.server.services import provision_bootstrap_admin_orgs
from aistack.config.config import Config
from aistack.schemas.config import GatewayModeEnum
from aistack.config import registration
from aistack.server.controllers import (
...
from aistack.gpu_instances.controllers import (
...
from aistack.schemas.catalog_source import normalize_catalog_yaml
from aistack.server.catalog import read_builtin_catalog_text
from aistack.server.db import async_session
from aistack.server.gateway_auth_reconciler import GatewayAuthReconciler
from aistack.server.lora_model_routes import (
    cleanup_orphan_lora_routes,
    create_lora_model_routes,
)
from aistack.utils.lora_model_source import normalized_lora_list
from aistack.server.init_db import init_db, get_query_count
from aistack.scheduler.scheduler import Scheduler
from aistack.server.system_load import SystemLoadCollector
from aistack.server.sources.probe import SourceRefresher
from aistack.server.update_check import UpdateChecker
from aistack.server.worker_status_buffer import flush_worker_status_to_db
from aistack.server.metrics_collector import flush_gateway_metrics_to_db
from aistack.server.usage_details_archiver import UsageDetailsArchiver
from aistack.server.resource_event_logger import ResourceEventLogger
from aistack.server.resource_usage_collector import ResourceUsageCollector
from aistack.server.storage_usage_collector import StorageUsageCollector
from aistack import envs
from aistack.server.usage_archiver import TableArchiver
from aistack.schemas.metered_usage import MeteredUsage, MeteredUsageArchive
from aistack.schemas.resource_events import ResourceEvent, ResourceEventArchive
from aistack.server.worker_instance_cleaner import WorkerInstanceCleaner
from aistack.server.cache_services import CacheServiceHealthChecker
from aistack.server.worker_syncer import WorkerSyncer
from aistack.server.scaling_scheduler import ScalingScheduler
from aistack.utils.platform import is_inside_kubernetes
from aistack.utils.process import add_signal_handlers_in_loop
from aistack.config.registration import write_registration_token
from aistack.exporter.exporter import MetricExporter
from aistack.gateway.utils import (
...
from aistack.gateway import get_async_k8s_config
from aistack.gateway.plugins import verify_published_plugin_modules
from aistack.envs import (
    GATEWAY_PORT_CHECK_INTERVAL,
    GATEWAY_PORT_CHECK_RETRY_COUNT,
    DEFAULT_CLUSTER_KUBERNETES,
)
from aistack.server.coordinator import LocalCoordinator
from aistack.server.coordinator.cache import preload_cache
from aistack.server.coordinator.models import get_model_for_topic
from aistack.server import bus
from aistack.server import cache as cache_module
from alembic import command
from alembic.config import Config as AlembicConfig

from aistack.websocket_proxy.proxy_server import HTTPSProxyServer
from aistack.api.auth import (
    authenticate_worker_by_request_headers,
)

from aistack.gpu_instances import gateway_client
from aistack.gpu_instances.gateway import (
    reconcile_aistack_operator_subscription,
)
from aistack.gpu_instances.settings import (
    reconcile_aistack_operator_settings,
)
...
class Server:
    ...
    async def start(self):
        logger.info("Starting AiStack server.")
        ...
        self._start_aistack_operator_subscription()
        ...
        serving_api_message = f"Serving AiStack API on {config.host}:{config.port}."
        if self._config.gateway_mode == GatewayModeEnum.embedded:
            logger.debug(serving_api_message)
            logger.info(
                f"AiStack Server will serve on 0.0.0.0:{self._config.get_gateway_port()}."
            )
            if self._config.get_tls_secret_name() is not None:
                logger.info(
                    f"AiStack Server will serve TLS on 0.0.0.0:{self._config.tls_port}."
                )
        else:
    ...
    def _run_migrations(self):
        ...
        spec = importlib.util.find_spec("aistack")
        if spec is None:
            raise ImportError("The 'aistack' package is not found.")
    ...
    def _start_aistack_operator_subscription(self):
        """Keep the in-process aistack-operator gateway in sync with Cluster events.

        Runs on every server instance — each server spawns its own
        ``aistack-operator`` subprocess and must feed it the current cluster
        set, so this cannot be a leader-only task.
        """
        if shutil.which("aistack-operator") is None:
            return
        self._create_async_task(reconcile_aistack_operator_subscription())

        logger.debug("AiStack operator subscription started.")

    def _start_aistack_operator_settings(self):
        ...
        writer is enough.
        """
        self._create_async_task(reconcile_aistack_operator_settings(self._config))

        logger.debug("AiStack operator settings reconciler started.")
    ...
    def _enqueue_operator_process(self):
        """Queue the ``aistack-operator wg`` subprocess for startup.
        ...
        """
        if shutil.which("aistack-operator") is None:
            logger.debug("aistack-operator binary not found on PATH, skipping start.")
            return

        socket_dir = tempfile.mkdtemp(prefix="aistack-operator-")
        ...
        self._before_healthy_sub_processes.append(
            _ExternalSubprocess(
                args, name="aistack-operator", on_exit=_cleanup_socket_dir
            )
        )
    ...
    async def _wait_for_gateway_ready(self):
        ...
        logger.info(f"Waiting for ports {ports} of AiStack to be ready...")
        try:
            ...
            return
        logger.info("AiStack Server is ready.")
    ...
    async def _init_default_cluster(self, session: AsyncSession):
        ...
        default_cluster = Cluster(
            ...
            description="The default cluster for AiStack",
            provider=provider,
    ...
    async def _prepare_jwt_secret_key(self):
        ...
        AISTACK_JWT_SECRET_KEY / config file) rather than the current value,
        ...
        raise RuntimeError(
            ...
            "or set the AISTACK_JWT_SECRET_KEY environment variable."
        )
    ...
    def _start_leader_tasks(self):
        ...
        self._start_aistack_operator_settings()
