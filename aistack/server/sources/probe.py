...
from aistack.schemas.runner_source import InferenceRunnerSource, normalize_runner_json
from aistack.server.catalog import get_builtin_model_catalog_file
from aistack.server.db import async_session

from aistack.schemas.source import SourceMixin, SourceTypeEnum
...
_HEADERS = {"User-Agent": f"aistack/{__version__}"}
...
# ``ota_server_url`` (config / CLI / ``AISTACK_OTA_SERVER_URL``)
# replaces the whole URL, so an OTA server of your own can live at any path.
OTA_SERVER_URL = "https://ota.aistack.ai/latest"
