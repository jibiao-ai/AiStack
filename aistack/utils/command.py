...
# Flags whose value is a credential AiStack injects itself, rather than
...
def find_bool_parameter(parameters: List[str], param_names: List[str]) -> bool:
    ...
    - any other value -> True (the key was explicitly declared; the backend
      validates the value itself, AiStack should not silently treat unrecognized
      values as "off")
...
def should_default_to_ray(backend_version: Optional[str]) -> bool:
    """
    Whether AiStack should default to the Ray executor backend when the user
    does not explicitly choose one.

    aistack-runner images bundle Ray themselves regardless of vLLM version,
...
def resolve_executor_backend(
    ...
    Returns ``"ray"`` if AiStack should take the Ray sidecar path (legacy),
    ``"mp"`` for the MP multi-node headless path (current).

    Precedence:
    1. User-supplied ``--distributed-executor-backend`` wins. Any explicit value
       other than ``"mp"`` is routed to the ray branch — AiStack does not inject
