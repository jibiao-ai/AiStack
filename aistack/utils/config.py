...
def apply_registry_override_to_image(
    ...
    3) If the image does not have an explicit registry and no system default registry is configured,
       using docker.io as default if image without "aistack" prefix.
    4) If the image does not have an explicit registry and no system default registry is configured,
       and with "aistack" prefix, using docker.io as default if docker.io is reachable.
    ...
    # 3) no explicit or configured, and not start with "aistack" using "docker.io" as default.
    if not image.startswith("aistack"):
        logger.info(
            f"Using Docker Hub for non-aistack image; image resolved to: {image}"
        )
        return image
