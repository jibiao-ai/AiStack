...
# Kueue queue names look like ``aistack-nvidia-geforce-rtx-4090-c9bjn`` —
# a stable ``aistack-{manufacturer}-{model}`` prefix plus a short random
...
def parse_gpu_type(queue_name: Optional[str]) -> Tuple[str, Optional[str]]:
    ...
    .. deprecated::
        ...
        This regex was written for kueue queue names (``aistack-<vendor>-<model>``
        ...
        InstanceType name, which uses a ``--`` separator and no random suffix::
            ...
            >>> parse_gpu_type("aistack--generic--ascend-910b2-linux-arm64")
    ...
    Examples
    --------
    >>> parse_gpu_type("aistack-nvidia-geforce-rtx-4090-c9bjn")
    ('nvidia-geforce-rtx-4090', 'nvidia')
    >>> parse_gpu_type("aistack-amd-mi300x-ab12c")
    ('amd-mi300x', 'amd')
    >>> parse_gpu_type(None)
    ('unknown', None)
    """
...
# Mirror of ``aistack.schemas.metered_usage.RESOURCE_TYPE_*`` — kept as
...
def instance_sku(
    ...
    ``aistack--generic--nvidia-tesla-t4-linux-amd64`` on operator v0.7,
    ``aistack--generic-ln-x64-4c-16g-98g--nvidia-tesla-t4-1d`` on v0.5)
    ...
    Examples
    --------
    >>> instance_sku("aistack--generic--nvidia-tesla-t4-linux-amd64", "x", 2, 8000, 1)
    'aistack--generic--nvidia-tesla-t4-linux-amd64'
...
def volume_sku(category: str, type_name: str) -> str:
    ...
    Mirrors the instance ``aistack--...`` flavor convention with a ``volume--``
