...
from aistack.schemas.workers import GPUDevicesStatus
from aistack.server.bus import Event
from aistack.utils.command import flatten_to_argv, is_parameter_key
from aistack.utils.config import apply_registry_override_to_image
from aistack.utils.envs import filter_env_vars
from aistack.utils.hub import get_hf_text_config, get_max_model_len
from aistack.utils.hub import get_pretrained_config, safe_pretrained_config_from_dict
from aistack.utils.profiling import time_decorator
from aistack.utils import platform
from aistack.utils.version import pick_runtime_version
from aistack.utils.runtime import transform_workload_plan
...
#
# Tracked in https://github.com/aistack/aistack/issues/5189.
#
# vLLM
# ----
...
#   - AiStack auto-injects `--enable-prompt-tokens-details` for vLLM
#     >= 0.9.0.1. See `aistack.worker.backends.vllm.get_cache_report_arguments`.
#
# SGLang
# ------
...
#   - AiStack auto-injects `--enable-cache-report` for SGLang >= 0.3.4. See
#     `aistack.worker.backends.sglang.get_cache_report_arguments`.
#
# Ascend MindIE
# -------------
...
#   - Cache token details require MindIE >= 2.3.0, AiStack does not auto-inject any cache-related flag for MindIE.
...
class InferenceServer(ABC):
    ...
    def _host_ipc_enabled(self) -> bool:
        ...
        ``AISTACK_HOST_IPC`` env (per-model, or global on the worker)
    ...
    def _cuda_minor_version_compatibility_enabled(self) -> bool:
        ...
        ``AISTACK_ENABLE_CUDA_MINOR_VERSION_COMPATIBILITY`` in the model's env
        overrides the global default (``aistack.envs``); off by default."""
        model_env = self._model.env or {}
    ...
    def _get_vgpu_configured_resources(
        ...
        operator's resource families (aistack-operator ``pkg/nodefeature``);
    ...
    def _get_container_env_config(env: Dict[str, str]) -> ContainerEnvConfig:
        ...
        # Read user ID
        uid_str = env.get("AISTACK_MODEL_RUNTIME_UID")
        if uid_str:
            try:
                config.user = int(uid_str)
            except ValueError:
                logger.warning(
                    f"Invalid AISTACK_MODEL_RUNTIME_UID value: {uid_str}, ignoring"
                )

        # Read group ID
        gid_str = env.get("AISTACK_MODEL_RUNTIME_GID")
        if gid_str:
            try:
                config.group = int(gid_str)
            except ValueError:
                logger.warning(
                    f"Invalid AISTACK_MODEL_RUNTIME_GID value: {gid_str}, ignoring"
                )

        # Read shared memory size in GiB
        shm_str = env.get("AISTACK_MODEL_RUNTIME_SHM_SIZE_GIB", "10")
        try:
            config.shm_size_gib = float(shm_str)
        except ValueError:
            logger.warning(
                f"Invalid AISTACK_MODEL_RUNTIME_SHM_SIZE_GIB value: {shm_str}, using default 10.0"
            )
    ...
    def _get_serving_command_script(self, env: dict[str, str]) -> Optional[str]:
        ...
        Return None if `AISTACK_MODEL_SERVING_COMMAND_SCRIPT_DISABLED` is set, or
        ...
        if env and to_bool(
            env.get("AISTACK_MODEL_SERVING_COMMAND_SCRIPT_DISABLED", "0")
        ):
            return None
        ...
        for path, content in sorted(cache_files.items()):
            ...
            cache_files_step += (
                ...
                f"cat > '{path}' <<'AISTACK_CACHE_FILE_EOF'\n"
                f"{content}\n"
                "AISTACK_CACHE_FILE_EOF\n\n"
            )
...
"""
    ...
    def _resolve_image(  # noqa: C901
        ...
        3) Auto-detected image from aistack-runner based on device vendor/arch and backend
        ...
        if not backend:
            ...
            # AiStack-Runner does not provide CPU-only platform images.
...
def _parse_image_cuda_version(image: Optional[str]) -> Optional[str]:
    ...
    (``aistack/runner:cuda12.9-...`` -> ``"12.9"``); None if the tag has none."""
    if not image:
        return None
