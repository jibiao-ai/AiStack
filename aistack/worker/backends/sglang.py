...
def extend_sglang_mounted_lora_arguments(
    ...
    in AiStack, but sglang's OpenAI handler splits model="<base>:<lora>" on
...
class SGLangServer(InferenceServer):
    """
    Containerized SGLang inference server backend using aistack-runtime.

    This backend runs SGLang in a Docker container managed by aistack-runtime,
    ...
    def _build_command_args(
        ...
        Returns:
            ...
            added by AiStack, excluding the entrypoint and user-specified
            backend parameters.
        """
        arguments = [
            "--model-path",
...
def get_metrics_arguments(
    ...
    if env and env.get("AISTACK_DISABLE_METRICS"):
        return []
