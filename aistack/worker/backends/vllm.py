...
from aistack.utils.command import (
    ExecutorBackend,
    find_parameter,
    find_bool_parameter,
    find_int_parameter,
...
from aistack.utils.unit import byte_to_gib
from aistack.utils.vllm_topology import (
    MultinodeShape,
...
class VLLMServer(InferenceServer):
    """
    Containerized vLLM inference server backend using aistack-runtime.

    This backend runs vLLM in a Docker container managed by aistack-runtime,
    ...
    def _get_configured_env(
        ...
        # Apply AiStack's inference environment setup
    ...
    def _set_cache_env(self, env: Dict[str, str]):
        """
        Point VLLM_CACHE_ROOT at a persistent directory under aistack's data dir
        so the torch compile cache survives container restarts. The directory is
        inherited by the inference container via aistack-runtime's mirrored
    ...
    def _build_command_args(
        ...
        Returns:
            ...
            added by AiStack, excluding the entrypoint/model path and
    ...
    def _build_ray_distributed_arguments(self, ctx: _VLLMArgsContext) -> List[str]:
        ...
        if dps and dps > 1:
            ...
            # ports[1] is reserved for DP RPC, see aistack/worker/serve_manager.py.
            arguments.extend(
    ...
    def _build_ray_configuration(
        ...
        # 2.  Client port (reserved for compatibility, not used anymore, see https://github.com/aistack/aistack/issues/4171)
        # 3.  Dashboard port
