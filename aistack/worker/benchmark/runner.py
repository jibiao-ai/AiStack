...
from aistack.envs import BENCHMARK_DATASET_SHAREGPT_PATH, BENCHMARK_REQUEST_TIMEOUT
from aistack.logging import setup_logging
from aistack.ssl_context import resolve_ca_bundle
...
PROGRESS_CA_BUNDLE_PATH = "/etc/aistack/progress-ca-bundle.crt"


def resolve_progress_insecure_tls() -> bool:
    """Whether the benchmark container should skip TLS verification on progress.

    Driven by ``AISTACK_INSECURE_TLS`` alone, which a worker started with the
    ...
    have, and ``filter_env_vars`` strips ``AISTACK_*`` before the container env
    is built. So this process (a spawn child, which does inherit the variable) is
    the last place that can act on it.
    """
    return bool(get_aistack_env_bool("INSECURE_TLS"))
...
class BenchmarkRunner:
    ...
    def _progress_ca_file(self) -> Optional[ContainerFile]:
        ...
        ``aistack/benchmark-runner`` image whose entrypoint is the runner CLI, so
        it never runs ``aistack-prerun.sh`` and never sees the CAs an operator
    ...
    def _build_command_args(  # noqa: C901
        ...
        if self._progress_insecure_skip_tls_verify and self._progress_is_https:
            logger.warning(
                "AISTACK_INSECURE_TLS is set: TLS verification is disabled for "
