...
from aistack.utils import network
from aistack.utils.convert import safe_int
from aistack.utils.attrs import set_attr
from aistack.utils.command import find_int_parameter
from aistack.utils.process import terminate_process_tree, add_signal_handlers
from aistack.worker.backends.ascend_mindie import AscendMindIEServer
from aistack.worker.backends.sglang import SGLangServer
from aistack.utils.command import resolve_executor_backend
from aistack.worker.backends.vllm import VLLMServer
from aistack.worker.backends.vox_box import VoxBoxServer
from aistack.worker.backends.custom import CustomServer
from aistack.routes.worker.logs import (
    extract_container_restart_count,
    extract_restart_count,
)
from aistack.worker.model_meta import get_meta_from_running_instance
...
from aistack.worker.inference_backend_manager import InferenceBackendManager
...
# EOF; beyond that aistack marks it ERROR and takes over recovery.
...
_ALLOCATED_ACCELERATORS_ANNOTATION = "device.aistack.ai/accelerator.allocated"
...
def _describe_workload_failure(workload) -> str:
    ...
    behind it (aistack/aistack#5869) -- but it never carries an exit code, and
...
class ServeManager:
    ...
    def sync_model_instances_state(self):  # noqa: C901
        ...
        # AISTACK_MODEL_INSTANCE_STATE_RECONCILE_INTERVAL env var when a
        ...
        for model_instance in model_instances:
            ...
            if model.env and 'AISTACK_MODEL_HEALTH_CHECK_PATH' in model.env:
                ...
                health_check_path = model.env['AISTACK_MODEL_HEALTH_CHECK_PATH']
    ...
    def _serve_model_instance(
        ...
        setproctitle.setproctitle(f"aistack_model_instance_{mi.id}")
        add_signal_handlers()
    ...
    def sync_model_instances_inference_health(self):
        ...
        - AISTACK_MODEL_INFERENCE_HEALTH_CHECK_ENABLED: "true"/"false" (default: false)
        - AISTACK_MODEL_INFERENCE_HEALTH_CHECK_INTERVAL: seconds (default: global env)
        - AISTACK_MODEL_INFERENCE_HEALTH_CHECK_TIMEOUT: seconds (default: 15)
        - AISTACK_MODEL_INFERENCE_HEALTH_CHECK_FAILURE_THRESHOLD: count (default: global env)
...
def _get_inference_health_check_config(model: Model) -> dict:
    ...
    env = model.env or {}
    enabled = env.get(
        "AISTACK_MODEL_INFERENCE_HEALTH_CHECK_ENABLED", "false"
    ).lower() in (
        "true",
        "1",
    )
    interval = safe_int(
        env.get("AISTACK_MODEL_INFERENCE_HEALTH_CHECK_INTERVAL"),
        300,
    )
    timeout = safe_int(
        env.get("AISTACK_MODEL_INFERENCE_HEALTH_CHECK_TIMEOUT"),
        15,
    )
    threshold = safe_int(
        env.get("AISTACK_MODEL_INFERENCE_HEALTH_CHECK_FAILURE_THRESHOLD"),
        3,
    )
    return {
        "enabled": enabled,
