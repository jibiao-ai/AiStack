...
from aistack.api import exceptions
...
from aistack.routes import config as route_config, debug, probes
from aistack.routes.worker import logs, proxy, filesystem, cluster_proxy
from aistack.routes.token import worker_auth
from aistack.api.auth import worker_auth as worker_request_auth
from aistack.server import catalog
from aistack.utils.network import (
...
from aistack.utils.system_check import check_glibc_version
from aistack.utils.task import run_periodically_in_thread
from aistack.worker.benchmark_manager import BenchmarkManager
from aistack.worker.cache_service_manager import CacheServiceManager
...
from aistack.worker.runtime_metrics_aggregator import RuntimeMetricsAggregator
from aistack.worker.serve_manager import ServeManager
from aistack.worker.exporter import MetricExporter
from aistack.worker.tools_manager import ToolsManager
from aistack.worker.worker_manager import WorkerManager
from aistack.worker.collector import WorkerStatusCollector
from aistack.config.registration import read_worker_token
from aistack.config import registration
from aistack.gateway import init_async_k8s_config
from aistack.client.generated_http_client import default_versioned_prefix
from aistack.worker.workload_cleaner import WorkloadCleaner
from aistack.utils.uuid import get_worker_name, get_legacy_uuid
from aistack.websocket_proxy.message_client import MessageClient
from aistack.api.auth import BearerTokenAuthenticator
...
class Worker:
    ...
    def start(self):
        ...
        if self._is_embedded:
            setproctitle.setproctitle("aistack_worker")
    ...
    async def start_async(self):
        ...
        logger.info("Starting AiStack worker.")
        ...
        logger.info("AiStack worker startup completed.")
...
def migrate_worker_name(cfg: Config):
    ...
    to file if the worker is started with `--worker-name` argument or `AISTACK_WORKER_NAME` environment variable.
