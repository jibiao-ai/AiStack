...
*/}}
{{- define "aistack.multiVendorMode" -}}
{{- $vendors := include "aistack.workerVendors" . | fromJsonArray -}}
...
*/}}
{{- define "aistack.serverNodeSelector" -}}
...
PCI labels.
*/}}
{{- define "aistack.workerBaseNodeSelectorJson" -}}
...
aistack/k8s/manifest_template.py. Used to derive deterministic
nodeSelector labels for each vendor DaemonSet.
*/}}
{{- define "aistack.pciVendorIds" -}}
...
*/}}
{{- define "aistack.canonicalVendorOrder" -}}
...
output only.
*/}}
{{- define "aistack.sortedVendors" -}}
{{- $vendors := include "aistack.workerVendors" . | fromJsonArray -}}
{{- $canonical := include "aistack.canonicalVendorOrder" . | fromJsonArray -}}
{{- $sorted := list -}}
{{- range $canonical -}}
  {{- if has . $vendors -}}
...
{{ printf "%s/%s" (include "aistack.hub" .) .Values.image.repository -}}
{{ end -}}


{{ define "server_config" -}}
{{ include "server_external_url" . }}
AISTACK_DEBUG: "{{ .Values.debug }}"
AISTACK_API_PORT: "{{ .Values.server.apiPort }}"
AISTACK_METRICS_PORT: "{{ .Values.server.metricsPort }}"
{{ if .Values.server.externalDatabaseURL -}}
AISTACK_DATABASE_URL: "{{ .Values.server.externalDatabaseURL }}"
{{- end }}
...
AISTACK_DEBUG: "{{ .Values.debug }}"
AISTACK_WORKER_PORT: "{{ .Values.worker.port }}"
AISTACK_WORKER_METRICS_PORT: "{{ .Values.worker.metricsPort }}"
...
{{ printf "%s/%s:%s" (include "aistack.hub" .) .Values.higressPlugins.image.repository (required "higressPlugins.image.tag is required" .Values.higressPlugins.image.tag) -}}
{{- end -}}
...
{{- if not .Values.server.environmentConfig.AISTACK_SERVER_EXTERNAL_URL -}}
{{- $schema := "http" -}}
{{- if and .Values.server.ingress.tls.cert .Values.server.ingress.tls.key }}
{{- $schema = "https" -}}
{{- end }}
{{- if .Values.server.ingress.hostname -}}
AISTACK_SERVER_EXTERNAL_URL: {{ printf "%s://%s" $schema .Values.server.ingress.hostname }}
{{- end -}}
{{- end -}}
{{- end -}}
...
automatically by CI from aistack/__init__.py __operator_version__).
Fails explicitly when the tag is unset instead of falling back to a
stale default.
*/}}
{{- define "aistack.operatorImageTag" -}}
...
*/}}
{{- define "aistack.operatorImage" -}}
{{ printf "%s/%s:%s" (include "aistack.hub" .) .Values.operator.image.repository (include "aistack.operatorImageTag" .) -}}
{{- end -}}
...
*/}}
{{- define "aistack.operatorNodeSelector" -}}
