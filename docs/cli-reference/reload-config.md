...
# aistack reload-config

Reload runtime-safe configuration values.

```bash
aistack reload-config [OPTIONS]
```
...
| `--server-port` value               | `30080`                                | Target port of the AiStack API server for applying or listing runtime config. When omitted, defaults to `AISTACK_API_PORT` if set, otherwise `30080`.                                                                                                                                                                                        |
| `--worker-port` value               | `10150`                                | Target port of the AiStack worker for applying or listing runtime config. When omitted, defaults to `AISTACK_WORKER_PORT` if set, otherwise `10150`.                                                                                                                                                                                         |
