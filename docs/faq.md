...
AiStack supports heterogeneous clusters spanning NVIDIA, AMD, Ascend NPUs, Hygon DCUs, Moore Threads, Iluvatar, MetaX, Cambricon and T-head PPUs, and works across both AMD64 and ARM64 architectures.
...
You can set it to a custom name using the `--worker-name` flag when running AiStack:
...
You can set it to a custom IP using the `--worker-ip` flag when running AiStack:
...
### Where are AiStack's data stored?

When running the AiStack container, the Docker volume is mounted using `--volume/-v` parameter. The default data path is under the Docker data directory, specifically in the volumes subdirectory, and the default path is:

```bash
/var/lib/docker/volumes/aistack-data/_data
```
...
```bash
docker volume ls
docker volume inspect aistack-data
```

If you need to change it to a custom path, modify the mount configuration when running container. For example, to mount the host directory `/data/aistack`:
...
+    --volume /data/aistack:/var/lib/aistack \
    ...
    aistack/aistack
```
...
When running the AiStack container, the Docker volume is mounted using `--volume/-v` parameter. The default cache path is under the Docker data directory, specifically in the volumes subdirectory, and the default path is:

```bash
/var/lib/docker/volumes/aistack-data/_data/cache
```
...
sudo docker run -d --name aistack \
    ...
    --volume aistack-data:/var/lib/aistack \
+    --volume /data/model-cache:/var/lib/aistack/cache \
    ...
    aistack/aistack
```
...
The context size for the model also affects the required GPU memory. You can adjust the `--max-model-len` parameter to set a smaller context. In AiStack, if this parameter is not set, its default value is 8192. If it is specified in the backend parameters, the actual setting will take effect.
...
Try restarting the AiStack container where the model is scheduled. If the issue persists, check the worker logs [here](troubleshooting.md#view-aistack-logs) to analyze the cause.
...
```bash
/aistack-command-xxxxxxxx: 40: /path/to/your_model: Permission denied
```
...
### How do I use AiStack behind a proxy?

We recommend passing standard proxy environment variables when running AiStack.

The following case demonstrates how to configure AiStack to forward all requests to the target proxy, except for requests to addresses specified in the NO_PROXY environment variable.
