...
const CONFIG = {
    ...
    registries: {
        'docker-hub': { 
            name: 'Docker Hub', prefix: 'aistack/', registry: 'docker.io',
        ...
        'quay': { name: 'Quay.io', prefix: 'quay.io/aistack/', registry: 'quay.io' },
        'china': { name: '国内镜像源', prefix: 'swr.cn-south-1.myhuaweicloud.com/aistack/', registry: 'swr.cn-south-1.myhuaweicloud.com' }
    },
    ...
    i18n: {
        'zh': {
            ...
            'optional_postgres_images_tooltip': '使用外部数据库：<a href="https://docs.aistack.ai/latest/installation/installation/#using-an-external-database" target="_blank">https://docs.aistack.ai/latest/installation/installation/#using-an-external-database</a>',
            'optional_monitoring_images_tooltip': '配置外部可观测性：<a href="https://docs.aistack.ai/latest/user-guide/observability/#external-observability-optional" target="_blank">https://docs.aistack.ai/latest/user-guide/observability/#external-observability-optional</a>',
            'postgres': 'PostgreSQL',
            ...
            'guide_save_content': `
                ...
                <ol>
                    ...
                    <li>导出镜像为打包文件：
                        <pre><code>docker save -o aistack-server-images.tar {{server_images}}\n\ndocker save -o aistack-worker-images.tar {{worker_images}}</code></pre>
                    </li>
                    <li>将镜像文件拷贝至离线机器。</li>
                    <li>按节点角色导入镜像：
                        <div style="margin-top:10px"><strong>Server 节点：</strong></div>
                        <pre><code>docker load -i aistack-server-images.tar</code></pre>
                        <div style="margin-top:10px"><strong>Worker 节点：</strong></div>
                        <pre><code>docker load -i aistack-worker-images.tar</code></pre>
                        <div style="margin-top:10px"><strong>Server + Worker 节点：</strong></div>
                        <pre><code>docker load -i aistack-server-images.tar\ndocker load -i aistack-worker-images.tar</code></pre>
                    </li>
                    <li>
                        运行 AiStack Server 和 Worker 容器时通过 <code class="inline-code">--system-default-container-registry</code> 参数指定镜像源：
                        <pre><code>sudo docker run -d --name aistack \\\n    --restart unless-stopped \\\n    -p 80:80 \\\n    -p 10161:10161 \\\n    --volume aistack-data:/var/lib/aistack \\\n    {{registry}}/aistack/aistack:{{version}} \\\n    --system-default-container-registry {{registry}}</code></pre>
                    </li>
                </ol>`,
            'guide_tag_content': `
                ...
                <ol>
                    ...
                    <li>运行 AiStack Server 和 Worker 容器时，通过启动参数指定镜像源：
                        <pre><code>sudo docker run -d --name aistack \\\n    --restart unless-stopped \\\n    -p 80:80 \\\n    -p 10161:10161 \\\n    --volume aistack-data:/var/lib/aistack \\\n    $PrivateRegistry/aistack/aistack:{{version}} \\\n    --system-default-container-registry $PrivateRegistry</code></pre>
                    </li>
                </ol>`,
            'guide_auto_content': `
                <p>若需要更自动化的镜像同步手段，AiStack 提供镜像管理命令，用于同步与管理所需镜像：</p>
                <ul style="list-style:none; padding-left:0">
                    <li style="margin-bottom:8px"><code>aistack copy-images</code>：从源仓库同步镜像到目标仓库</li>
                    <li style="margin-bottom:8px"><code>aistack save-images</code>：下载并保存镜像到本地路径</li>
                    <li style="margin-bottom:8px"><code>aistack load-images</code>：导入本地镜像包</li>
                    <li style="margin-bottom:8px"><code>aistack list-images</code>：列出当前版本镜像清单</li>
                </ul>
                <div style="margin-top:15px; font-size:13px; color:var(--text-color)">
                    以上命令均支持镜像过滤与自定义配置，具体用法请参考：
                    <a href="https://docs.aistack.ai/latest/installation/air-gapped/#container-images" target="_blank" style="color:var(--primary-color); font-weight:600">准备容器镜像 &rarr;</a>
                </div>`,
            'view_full_docs': '查看完整离线部署文档 &rarr;',
            'copied': '已复制到剪贴板',
            'no_images': '# 未找到匹配的镜像',
            'comments': {
                'main': 'AiStack 镜像 - AiStack 核心服务，Server 和 Worker 节点均需此镜像',
                'runner': '推理后端镜像',
        ...
        'en': {
            ...
            'optional_postgres_images_tooltip': 'Using an external database: <a href="https://docs.aistack.ai/latest/installation/installation/#using-an-external-database" target="_blank">https://docs.aistack.ai/latest/installation/installation/#using-an-external-database</a>',
            'optional_monitoring_images_tooltip': 'Configuring external observability: <a href="https://docs.aistack.ai/latest/user-guide/observability/#external-observability-optional" target="_blank">https://docs.aistack.ai/latest/user-guide/observability/#external-observability-optional</a>',
            'postgres': 'PostgreSQL',
            ...
            'guide_save_content': `
                ...
                <ol>
                    ...
                    <li>
                        When running AiStack Server and Worker containers, specify the container registry using the <code class="inline-code">--system-default-container-registry</code> parameter:
                        <pre><code>sudo docker run -d --name aistack \\\n    --restart unless-stopped \\\n    -p 80:80 \\\n    -p 10161:10161 \\\n    --volume aistack-data:/var/lib/aistack \\\n    {{registry}}/aistack/aistack:{{version}} \\\n    --system-default-container-registry {{registry}}</code></pre>
                    </li>
                </ol>`,
            'guide_tag_content': `
            ...
            'guide_auto_content': `
                <p>For more automated sync methods, AiStack provides image management commands:</p>
                <ul style="list-style:none; padding-left:0">
                    <li style="margin-bottom:8px"><code>aistack copy-images</code>: Sync images from source to destination registry</li>
                    <li style="margin-bottom:8px"><code>aistack save-images</code>: Download and save images to local path</li>
                    <li style="margin-bottom:8px"><code>aistack load-images</code>: Import images from local packages</li>
                    <li style="margin-bottom:8px"><code>aistack list-images</code>: List image manifest for current version</li>
                </ul>
                <div style="margin-top:15px; font-size:13px; color:var(--text-color)">
                    All commands support filtering and custom config. For details, refer to:
                    <a href="https://docs.aistack.ai/latest/installation/air-gapped/#container-images" target="_blank" style="color:var(--primary-color); font-weight:600">Prepare Container Images &rarr;</a>
                </div>`,
            'view_full_docs': 'View full air-gapped docs &rarr;',
            'copied': 'Copied to clipboard',
            'no_images': '# No matching images found',
            'comments': {
                'main': 'AiStack Image - AiStack core service, required for both Server and Worker nodes',
...
let state = {
    ...
    selectedBackends: [], availableVersions: [], selectedAiStackVersion: null,
...
function initElements() {
    ...
    elements.aistackVersionSelect = document.getElementById('aistack-version-select');
...
function bindEvents() {
    ...
    elements.aistackVersionSelect.addEventListener('change', (e) => selectAiStackVersion(e.target.value));
...
function getCurrentImages(component) {
    const imgs = [];
    if (state.selectedAiStackVersion) imgs.push(getFullImageName('aistack', state.selectedAiStackVersion, state.selectedRegistry));
    ...
    if (component === 'worker' || component === 'all') {
        state.runnerImages.forEach(img => {
            const tag = img.replace('aistack/runner:', '');
    ...
    if (component === 'server' || component === 'all') {
        ...
        state.images.filter(img => {
            ...
            return name.startsWith('aistack/mirrored-') || ['aistack/higress-plugins', 'aistack/aistack-operator', 'aistack/ssh-server'].includes(name);
        }).forEach(img => {
...
function updateLanguage() {
    ...
    const version = state.selectedAiStackVersion || 'latest';
    ...
    // Generate tag & push commands under 'aistack' namespace
    const tagPushCmds = allImgs.map(img => {
        ...
        const destImg = `aistack/${shortName}:${tagPart}`;
...
async function loadData() {
    try {
        ...
        state.selectedAiStackVersion = state.availableVersions[0];
        renderVersionSelector();
        await loadVersionData(state.selectedAiStackVersion);
    } catch (error) {
...
async function loadVersionData(version) {
    ...
    const data = await response.json();
    state.images = data;
    state.runnerImages = data.filter(img => img.startsWith('aistack/runner:'));
    parseSupportMatrix();
    renderCardSelector();
...
function renderVersionSelector() {
    elements.aistackVersionSelect.innerHTML = '';
    state.availableVersions.forEach(v => {
        const opt = document.createElement('option');
        opt.value = opt.textContent = v;
        opt.selected = (v === state.selectedAiStackVersion);
        elements.aistackVersionSelect.appendChild(opt);
    });
}

// Handle version change
async function selectAiStackVersion(v) {
    state.selectedAiStackVersion = v;
...
function parseSupportMatrix() {
    ...
    state.runnerImages.forEach(image => {
        const tag = image.replace('aistack/runner:', '');
        let parts = tag.split('-');
...
function generateImageList() {
    ...
    if (state.selectedAiStackVersion) {
        cmds.push(`# ${t.comments.main}`);
        cmds.push(`docker pull${plat} ${getFullImageName('aistack', state.selectedAiStackVersion, state.selectedRegistry)}`);
    }
    ...
    if (isServer) {
        ...
        const gatewayImgs = state.images.filter(img => {
            const name = img.split(':')[0];
            return name.startsWith('aistack/mirrored-higress-') || name === 'aistack/higress-plugins';
        });
        ...
        const gpuServiceImgs = state.images.filter(img => {
            const name = img.split(':')[0];
            return ['aistack/aistack-operator', 'aistack/ssh-server'].includes(name) ||
                   (name.startsWith('aistack/mirrored-') && !name.startsWith('aistack/mirrored-higress-'));
        });
