...
Since v2.2.0, AiStack can be deployed on Kubernetes with an in-cluster [Higress](https://higress.io/) API gateway using the official Helm chart.

!!! note

    Deploying AiStack on Kubernetes with Higress is currently considered **experimental**. Review the [limitations](#limitations) before proceeding.
...
- The AiStack server is deployed as a `StatefulSet` and currently does **not** support more than one replica.
...
- Higress plugins are served by a dedicated `aistack/higress-plugins` Deployment installed alongside AiStack. The Higress gateway downloads plugins from this service on restart; if the service is unavailable, gateway startup is blocked until the plugins become accessible.
...
The chart is published as an OCI artifact on Docker Hub at `oci://registry-1.docker.io/aistack/aistack-chart`. The chart version tracks the AiStack release (e.g. chart `2.2.0` ships AiStack `v2.2.0`).
...
```bash
helm install aistack oci://registry-1.docker.io/aistack/aistack-chart \
  --namespace aistack-system --create-namespace
```

To pin a specific version, append `--version <chart-version>` (the chart version matches the AiStack release, e.g. `2.2.0`):
...
By default, the `higress-core` sub-chart is enabled and deployed alongside AiStack. If you already have Higress installed in your cluster, disable the bundled Higress and point AiStack at your existing instance:
...
cd aistack/charts
helm dependency update ./aistack-chart
helm install aistack ./aistack-chart \
  --namespace aistack-system --create-namespace
```
...
# Install higress-core (match the version pinned by the AiStack chart)
...
```bash
kubectl get pods -n aistack-system -w
```

Retrieve the initial admin password:

```bash
kubectl exec -it -n aistack-system aistack-server-0 -- \
  cat /var/lib/aistack/initial_admin_password
```

If you did not set `server.ingress.hostname`, obtain the AiStack UI address from the ingress:

```bash
kubectl get ingress -n aistack-system aistack \
...
By default AiStack uses the embedded PostgreSQL database. To use an external PostgreSQL or MySQL database, set `server.externalDatabaseURL`:
...
To pull all images (AiStack server/worker, higress-plugins, and the bundled higress-core gateway/controller/pilot) from a mirrored private registry, override `global.hub`. This relies on Helm's global-values propagation, so a single setting covers every image:
...
`global.hub` is also passed to the server as `AISTACK_SYSTEM_DEFAULT_CONTAINER_REGISTRY`, ensuring inference engine images (e.g. vLLM, llama.cpp) are pulled from the same registry.

To supply pull credentials, the chart can create a `docker-registry` Secret named `aistack-image-pull-secret` and wire it into all pods:
...
The most commonly used parameters are listed below. For the complete and authoritative list, see the chart's [`values.yaml`](https://github.com/aistack/aistack/blob/main/charts/aistack-chart/values.yaml) and [README](https://github.com/aistack/aistack/blob/main/charts/aistack-chart/README.md).
...
| `global.imagePullSecrets`              | `[aistack-image-pull-secret]` | Image pull Secrets attached to all pods and propagated to sub-charts. |
| `global.nodeSelector`                  | `{}`                     | Default nodeSelector for every component; replaced by component-level value. |
| `image.repository`                     | `aistack/aistack`      | Image repo with namespace; final ref is `{global.hub}/{repository}:{tag}`. |
...
| `server.environmentConfig`             | `{}`                     | Extra environment variables for the AiStack server.                      |
...
| `worker.dataDir`                       | `/var/lib/aistack`      | Host path mounted at `/var/lib/aistack` inside each worker pod.           |
| `worker.environmentConfig`             | `{}`                     | Extra environment variables for the AiStack worker.                      |
...
```bash
helm uninstall aistack -n aistack-system
```

Helm does not remove PVCs created by the `StatefulSet`. The PVC is named `aistack-data-dir-aistack-server-0` (`<volumeClaimTemplate>-<statefulset>-<ordinal>`). To delete the persisted data, remove the leftover PVC (and optionally the namespace):

```bash
kubectl delete pvc aistack-data-dir-aistack-server-0 -n aistack-system
kubectl delete namespace aistack-system
```
