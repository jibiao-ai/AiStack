...
AiStack provides [OpenAI-compatible APIs](https://platform.openai.com/docs/api-reference) at the `/v1` endpoint.

You can integrate and use models deployed on AiStack with any application or framework that supports the OpenAI-compatible API, simply by pointing it to AiStack's OpenAI-compatible endpoint.
...
AiStack provides the Anthropic-compatible [`/v1/messages` API](https://platform.claude.com/docs/en/api/messages/create).

### Usage

```bash
export AISTACK_API_KEY=your_api_key
curl http://your_aistack_server_url/v1/messages \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $AISTACK_API_KEY" \
  -d '{
    "model": "qwen3",
...
Note that the OpenAI-compatible APIs **do not** provide a `rerank` endpoint. To fill this gap, AiStack provides a [Jina-compatible Rerank API](https://jina.ai/reranker/) at the `/v1/rerank` path.
...
```bash
export AISTACK_API_KEY=your_api_key
curl http://your_aistack_server_url/v1/rerank \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer $AISTACK_API_KEY" \
    -d '{
...
For other API types, AiStack allows you to enable the **Generic Proxy** feature when deploying a model.
...
curl http://your_aistack_server_url/model/proxy/42/embed \
  -X POST \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $AISTACK_API_KEY" \
...
**Header-based form (deprecated)** — send to `/model/proxy/<upstream-path>` and specify the target model via the `X-AiStack-Model` header or the `model` field in the JSON body:

```bash
curl http://your_aistack_server_url/model/proxy/embed \
  -X POST \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $AISTACK_API_KEY" \
