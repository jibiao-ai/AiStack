...
Claude Code is an agentic coding tool from Anthropic. Since model deployments on AiStack are compatible with the Anthropic API, you can easily connect Claude Code to your AiStack deployment and use it for code generation tasks. In this guide, we will walk through the steps to integrate Claude Code with AiStack and test the integration by asking Claude to create a Flappy Bird game.

## Prerequisites

- One or more GPUs with at least 100 GB of VRAM in total
- AiStack installed and running
...
1. In the AiStack UI, navigate to the **Model Catalog** page.
...
## Configure Claude Code with AiStack

1. Open CC-Switch and add a custom provider with the following settings:

   - **Provider Name**: `AiStack`
   - **API Endpoint**: Your AiStack server URL
...
In this guide, we successfully integrated Claude Code with AiStack and used it to generate a Flappy Bird game. You can now explore more complex coding tasks with Claude Code and leverage the power of AiStack for efficient model serving.
