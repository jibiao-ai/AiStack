...
Dify can integrate with AiStack to leverage locally deployed LLMs, embeddings, reranking, image generation, Speech-to-Text and Text-to-Speech capabilities.
...
## Integrating AiStack into Dify

1. Access the Dify UI, go to the top right corner and click on `PLUGINS`, select `Install from Marketplace`, search for the AiStack plugin, and choose to install it.

![dify-install-aistack-plugin](../assets/integrations/integration-dify-install-aistack-plugin.png)

2. After installed, go to `Settings > Model Provider > AiStack`, then select `Add Model` and fill in:

- Model Type: Select the model type based on the model.

- Model Name: The name must match the model name deployed on AiStack.

- Server URL: `http://your-aistack-url`, do not use `localhost`, as it refers to the container’s internal network. If you’re using a custom port, make sure to include it. Also, ensure the URL is accessible from inside the Dify container (you can test this with `curl`).
