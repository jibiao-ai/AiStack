...
If there are any missing points or updates reflecting new changes, please [let us know](https://github.com/aistack/aistack/issues/new/choose).
...
This project uses AiStack's one-click benchmark capability for serving workloads. The benchmark tests in this document were executed with that workflow.

AiStack's benchmark implementation is built on top of [guidellm](https://github.com/vllm-project/guidellm) via the wrapper project [benchmark-runner](https://github.com/aistack/benchmark-runner).

AiStack handles model deployment, benchmark job submission, and result collection for the benchmark configurations listed below.
...
If you do not use AiStack, you can replace the AiStack benchmark workflow with direct `guidellm benchmark` commands.
