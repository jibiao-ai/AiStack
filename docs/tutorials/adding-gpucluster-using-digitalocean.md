...
When creating a cluster, AiStack can leverage DigitalOcean to create workers and add them to the AiStack cluster.
...
Labels and volumes are supported to set in worker pool to provision worker. The volumes created and attached for droplet won't be mount into worker container automatically. You can modify the run worker script in `/opt/aistack-run-worker.sh` to mount the volumes as needed.
