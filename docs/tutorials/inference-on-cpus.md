...
AiStack supports inference on CPUs, offering flexibility when GPU resources are limited or when model sizes exceed allocatable GPU memory. The following CPU inference modes are available:
...
Execution Command: `--model-id BAAI/bge-large-en-v1.5 --huggingface-hub-cache /var/lib/aistack/cache/huggingface --port {{port}}`
...
!!! note
    ...
    `--huggingface-hub-cache /var/lib/aistack/cache/huggingface` sets the location of the HuggingFace Hub cache for TEI to the path where AiStack stores downloaded HuggingFace models. The default path is `/var/lib/aistack/cache/huggingface`. See: [TEI CLI Arguments](https://huggingface.co/docs/text-embeddings-inference/cli_arguments).

    `{{port}}` is a placeholder that represents the port automatically assigned by AiStack.
