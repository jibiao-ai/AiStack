...
In this tutorial, you’ll learn how to set up and use tool calling within AiStack to extend your AI’s capabilities.
...
1. Navigate to the `Deployments` page in the AiStack UI and click the `Deploy Model` button. In the dropdown, select `Hugging Face` as the source for your model.
...
We will use the AiStack API to interact with the model. To do this, you need to generate an API key:
...
With the model deployed and an API key, you can call the model via the AiStack API. Here is an example script using `curl` (replace `<your-server-url>` with your AiStack server URL and `<your-api-key>` with the API key generated in the previous step):

```bash
export AISTACK_SERVER_URL=<your-server-url>
export AISTACK_API_KEY=<your-api-key>
curl $AISTACK_SERVER_URL/v1-openai/chat/completions \
