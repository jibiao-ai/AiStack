...
1. Add enough GPU resources for the new version of the model into AiStack.
...
5. Both models from AiStack `Deployments` and models from `Providers` can be selected as targets in the same route.
...
2. Edit this route and configure the `Fallback Route Target`. Like other route targets, it can be a model from AiStack `Deployments` or from `Providers`.
...
If a running inference service (such as ollama or lm-studio) wants to use AiStack for proxying, access control, and token usage statistics, you can create a custom-path `OpenAI` Model Provider for hosting.
...
7. Click the `Save` button to apply the model route. Your model is now proxied by AiStack.
