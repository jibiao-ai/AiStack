...
AiStack supports authentication using API keys. Each AiStack user can generate and manage their own API keys.
...
4. Select the `Type` of the API key:
   ...
   - **Auto-generated**: AiStack generates the API key for you.
...
!!! warning "Custom keys carry the strength you give them"
    ...
    AiStack accepts any string as a custom key and cannot tell a random one
    ...
    the API gateway rather than at the server, AiStack publishes a fast hash of
    ...
    choose them by hand, set `AISTACK_GATEWAY_AUTH_ALLOW_CUSTOM_KEYS=false`.
...
key must stop working **now** and the AiStack server is down — during an
...
```bash
kubectl -n higress-system edit wasmplugin aistack-llm-ext-auth
```
...
`aistack_<access key>_<secret>` for an auto-generated key. A custom key has no
...
Where `AISTACK_GATEWAY_AUTH_ALLOW_CUSTOM_KEYS` is set to `false`, custom keys
...
!!! note
    ...
    it was changed. Modifying the AiStack database directly is not a supported
...
AiStack supports using the API key as a bearer token. The following is an example using curl:
