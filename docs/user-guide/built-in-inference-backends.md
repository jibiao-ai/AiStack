...
AiStack supports the following inference backends:
...
!!! Note
    ...
    For all supported inference backends, we have pre-built Docker images available at [DockerHub](https://hub.docker.com/r/aistack/runner). When users deploy models, the system will automatically pull and run the corresponding images.
...
By default, AiStack estimates the VRAM requirement for the model instance based on the model's metadata.
...
- `--max-model-len`: Model context length. For large-context models, AiStack automatically sets this parameter to `8192` to simplify model deployment, especially in resource constrained environments. You can customize this parameter to fit your needs.
- `--tensor-parallel-size`: Number of tensor parallel replicas. By default, AiStack sets this parameter given the GPU resources available and the estimation of the model's memory requirement. You can customize this parameter to fit your needs.
...
AiStack integrates with vLLM-Omni to deliver a seamless experience for deploying and managing omni-modal models. When a model is deployed via the vLLM backend, AiStack automatically detects whether it is omni-modal based on its metadata and sets the required parameters for vLLM-Omni.

#### Distributed Inference Across Workers (Experimental)

When a single worker lacks enough VRAM or compute to host a model, vLLM can split one model instance across multiple workers and run inference cooperatively. AiStack orchestrates these cross-node processes as a single model instance and still exposes only one OpenAI-compatible endpoint externally.

Check `Allow Distributed Inference Across Workers` when deploying a model to enable it.

AiStack provides two execution paths for vLLM distributed inference, selected by vLLM's `--distributed-executor-backend` parameter. You generally do not need to set it manually — AiStack picks one automatically based on the container image and vLLM version.

##### Ray Backend

- **When used**: The default path on AiStack's official runner images — runner images always ship with Ray bundled, regardless of vLLM version. Also used when `--distributed-executor-backend` is explicitly set to `ray` or any non-`mp` value (e.g. `external_launcher`) in the backend parameters.
- **How it works**: AiStack starts a [Ray](https://ray.io) sidecar process on every participating node to form a cluster. The leader node runs the main vLLM process and exposes the API server, while Ray coordinates task scheduling across nodes.

##### MultiProcessing Backend

- **When used**: When `--distributed-executor-backend=mp` is set explicitly in the backend parameters; or when running a custom image (backend version with a `-custom` suffix) on vLLM ≥ `0.18.0` — vLLM dropped Ray from its default dependencies in `0.18.0`, so a custom image may not include Ray, and AiStack falls back to `mp` to avoid startup failures.
- **How it works**: Uses vLLM's multiprocessing-based multi-node execution directly, without a Ray sidecar. AiStack injects topology arguments such as `--nnodes`, `--node-rank`, `--master-addr`, and `--master-port` based on the cluster layout.
...
- **Topology shape**: AiStack automatically picks one of `dp_only`, `mp_only`, or `nested` parameter layouts based on the parallelism configuration (TP / PP / DP).
...
By default, AiStack estimates the VRAM requirement for the model instance based on model metadata.

When needed, AiStack also sets several parameters automatically for large-context models. Common SGLang parameters include:
...
- `--context-length`: Model context length. For large-context models, if the automatically estimated context length exceeds device capability, AiStack sets this parameter to `8192` to simplify deployment in resource-constrained environments. You can customize this parameter as needed.
- `--tp-size`: Tensor parallel size. When not explicitly provided, AiStack infers and sets this parameter based on the selected GPUs.
- `--pp-size`: Pipeline parallel size. In multi-node deployments, AiStack determines a combination of `--tp-size` and `--pp-size` according to the model and cluster configuration.
- Multi-node arguments: `--nnodes`, `--node-rank`, `--dist-init-addr`. When distributed inference is enabled, AiStack injects these arguments to initialize multi-node communication.
...
You can enable distributed SGLang inference across multiple workers in AiStack.
...
Within AiStack, support [large language models (LLMs)](https://www.hiascend.com/software/mindie/modellist) and [multimodal language models (VLMs)](https://www.hiascend.com/software/mindie/modellist).
...
At present, AiStack supports a subset of these capabilities, including
...
To avoid directly configuring JSON, AiStack provides a set of command line parameters as below.
...
| `--openai-support`                                   | vllm    |                          | Server Config                          | The compatibility mode for OpenAI API.                                                                                                                                                                                                                                          |
...
!!! note
    ...
    AiStack allows users to inject custom environment variables during model deployment, however, some variables may be conflicted with AiStack managment.

    Hence, AiStack will override/prevent those variables. Please compare the model instance logs' output with your expectations.

## VoxBox

[VoxBox](https://github.com/aistack/vox-box) is an inference engine designed for deploying Text-to-Speech and Speech-to-Text models. It also provides an API that is fully compatible with the OpenAI audio API.

### Supported Models

| Model                           | Type           | Link                                                                                                                                                                | Supported Platforms |
| ------------------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- |
| Faster-whisper-large-v3         | Speech-to-Text | [Hugging Face](https://huggingface.co/Systran/faster-whisper-large-v3), [ModelScope](https://modelscope.cn/models/aistack/faster-whisper-large-v3)                 | AMD64,ARM64         |
| Faster-whisper-large-v2         | Speech-to-Text | [Hugging Face](https://huggingface.co/Systran/faster-whisper-large-v2), [ModelScope](https://modelscope.cn/models/aistack/faster-whisper-large-v2)                 | AMD64,ARM64         |
| Faster-whisper-large-v1         | Speech-to-Text | [Hugging Face](https://huggingface.co/Systran/faster-whisper-large-v1), [ModelScope](https://modelscope.cn/models/aistack/faster-whisper-large-v1)                 | AMD64,ARM64         |
| Faster-whisper-medium           | Speech-to-Text | [Hugging Face](https://huggingface.co/Systran/faster-whisper-medium), [ModelScope](https://modelscope.cn/models/aistack/faster-whisper-medium)                     | AMD64,ARM64         |
| Faster-whisper-medium.en        | Speech-to-Text | [Hugging Face](https://huggingface.co/Systran/faster-whisper-medium.en), [ModelScope](https://modelscope.cn/models/aistack/faster-whisper-medium.en)               | AMD64,ARM64         |
| Faster-whisper-small            | Speech-to-Text | [Hugging Face](https://huggingface.co/Systran/faster-whisper-small), [ModelScope](https://modelscope.cn/models/aistack/faster-whisper-small)                       | AMD64,ARM64         |
| Faster-whisper-small.en         | Speech-to-Text | [Hugging Face](https://huggingface.co/Systran/faster-whisper-small.en), [ModelScope](https://modelscope.cn/models/aistack/faster-whisper-small.en)                 | AMD64,ARM64         |
| Faster-distil-whisper-large-v3  | Speech-to-Text | [Hugging Face](https://huggingface.co/Systran/faster-distil-whisper-large-v3), [ModelScope](https://modelscope.cn/models/aistack/faster-distil-whisper-large-v3)   | AMD64,ARM64         |
| Faster-distil-whisper-large-v2  | Speech-to-Text | [Hugging Face](https://huggingface.co/Systran/faster-distil-whisper-large-v2), [ModelScope](https://modelscope.cn/models/aistack/faster-distil-whisper-large-v2)   | AMD64,ARM64         |
| Faster-distil-whisper-medium.en | Speech-to-Text | [Hugging Face](https://huggingface.co/Systran/faster-distil-whisper-medium.en), [ModelScope](https://modelscope.cn/models/aistack/faster-distil-whisper-medium.en) | AMD64,ARM64         |
| Faster-whisper-tiny             | Speech-to-Text | [Hugging Face](https://huggingface.co/Systran/faster-whisper-tiny), [ModelScope](https://modelscope.cn/models/aistack/faster-whisper-tiny)                         | AMD64,ARM64         |
| Faster-whisper-tiny.en          | Speech-to-Text | [Hugging Face](https://huggingface.co/Systran/faster-whisper-tiny.en), [ModelScope](https://modelscope.cn/models/aistack/faster-whisper-tiny.en)                   | AMD64,ARM64         |
| CosyVoice-300M-Instruct         | Text-to-Speech | [Hugging Face](https://huggingface.co/aistack/CosyVoice-300M-Instruct), [ModelScope](https://modelscope.cn/models/aistack/CosyVoice-300M-Instruct)                | AMD64               |
| CosyVoice-300M-SFT              | Text-to-Speech | [Hugging Face](https://huggingface.co/aistack/CosyVoice-300M-SFT), [ModelScope](https://modelscope.cn/models/iic/CosyVoice-300M-SFT)                               | AMD64               |
| CosyVoice-300M                  | Text-to-Speech | [Hugging Face](https://huggingface.co/aistack/CosyVoice-300M), [ModelScope](https://modelscope.cn/models/aistack/CosyVoice-300M)                                  | AMD64               |
| CosyVoice-300M-25Hz             | Text-to-Speech | [ModelScope](https://modelscope.cn/models/iic/CosyVoice-300M-25Hz)                                                                                                  | AMD64               |
| CosyVoice2-0.5B                 | Text-to-Speech | [Hugging Face](https://huggingface.co/aistack/CosyVoice2-0.5B), [ModelScope](https://modelscope.cn/models/iic/CosyVoice2-0.5B)                                     | AMD64               |
