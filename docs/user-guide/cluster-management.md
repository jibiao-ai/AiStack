...
AiStack supports cluster-based worker management and provides multiple cluster types. You can provision a cluster through a `Cloud Provider` such as `DigitalOcean`, or create a self-hosted cluster and add workers using `Docker` run commands. Alternatively, you can register all nodes in a self-hosted `Kubernetes` cluster as AiStack workers.
...
- `Namespace` — the Kubernetes namespace the cluster's manifests render into. Leave empty to use `aistack-system`.
...
- `Image Credentials` — image pull secrets used to pull AiStack images from a private registry. For each entry, specify a `Registry`, `Username`, and `Password`.
...
- `Default Container Registry` — the default registry used to resolve AiStack images for this cluster. Falls back to the server default when unset (placeholder `docker.io`).
- `Operator Image` — override for the AiStack Operator container image. Leave empty to use the server default.
...
cache_dir: "/var/lib/aistack/cache"
log_dir: "/var/lib/aistack/log"
bin_dir: "/var/lib/aistack/bin"
# ========= container & image ===========
image_name_override: "aistack/aistack:main"
image_repo: "aistack/aistack"
# ========= service & networking ===========
worker_ifname: en0
worker_port: 10150
...
The above YAML lists all currently supported options for the `Worker Configuration YAML`. For the meaning of each option, refer to the full AiStack [config file documentation](../cli-reference/start.md#config-file). The `proxy_mode` option controls how the server reaches the worker — see [Worker Connection Modes](#worker-connection-modes).
