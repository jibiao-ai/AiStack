...
AiStack performs a compatibility check prior to model deployment. This check provides detailed information about the model’s compatibility with the current AiStack environment. The following compatibility checks are performed:
...
The resource check ensures that sufficient system resources are available to deploy the model. AiStack estimates the required resources and compares them with available resources in the environment. Estimations are performed using the following methods:

1. **For GGUF models**: AiStack uses the [GGUF parser](https://github.com/aistack/gguf-parser-go) to estimate the model's resource requirements.
2. **For other models**: AiStack estimates VRAM usage using the following formula:
