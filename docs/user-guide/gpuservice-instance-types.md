...
The [AiStack Operator](https://github.com/aistack/aistack-operator) supports accelerators from multiple manufacturers — AMD, Ascend, Cambricon, Hygon, Iluvatar, MetaX, Moore Threads, NVIDIA, and T-Head. For each manufacturer, an accelerator can be allocated **exclusively** (a whole device), **shared**, or **logically sliced** (software splitting with independent VRAM and compute budgets). NVIDIA and T-Head additionally support **physical partitioning** (MIG), where the device is split by hardware. See the operator's [accelerator support matrix](https://github.com/aistack/aistack-operator#accelerator-support) for the per-manufacturer details.
...
aistack--generic-linux-amd64                 aistack-fnv64-3b93966fd73eb9ec   1/2Gi/100Gi             0/0 0/0 0/0 0/0            16/40   Active
aistack--nvidia-h100-80gb-hbm3-linux-amd64   aistack-fnv64-e4768a65ca0ce96b   12/192Gi/100Gi          1/1 10/10 100/100 1/7      0/0     Active
aistack--nvidia-l40s-linux-amd64             aistack-fnv64-a730f1dca9e26fca   12/128Gi/100Gi          1/1 10/10 100/100 0/0      0/0     Active
```
...
kubectl -n aistack-system rollout restart ds/aistack-operator-device-manager-nvidia
```

See the operator's [NVIDIA MIG operations runbook](https://github.com/aistack/aistack-operator/blob/main/docs/operation/nvidia-mig.md#what-aistack-operator-does-not-do) for the full procedure, prerequisites, and limitations — including that MIG instances never survive a node reboot, and that on Hopper and newer the mode itself does not persist across reboots either.
