...
AiStack manages multiple Kubernetes clusters and provides a unified interface for launching GPU Service Instances on any of them.

!!! note "Upgrading from AiStack 2.2?"

    AiStack 2.3 ships AiStack Operator v0.8.5, which converges the in-place upgrade itself: the worker Deployment is replaced rather than overlapped (`Recreate`), the worker gets a 900s startup budget, and the worker adopts the legacy per-application Helm releases left behind by v0.5.x.

    After the upgraded worker is healthy, remove the orphaned v0.5.x scheduling objects with the operator's cleanup script:

    ```bash
    curl -sSLO https://raw.githubusercontent.com/aistack/aistack-operator/v0.8.5/docs/migration/cleanup-v0.5-orphans.sh
    bash cleanup-v0.5-orphans.sh --dry-run   # preview, changes nothing
    bash cleanup-v0.5-orphans.sh             # delete the orphans
    ```

    If an earlier upgrade attempt already wedged (Kueue CRDs stuck `Terminating`, the worker never becoming Ready), see the operator's [migration troubleshooting](https://github.com/aistack/aistack-operator/blob/main/docs/migration/troubleshooting.md). For the full procedure, follow the operator's [migration guide](https://github.com/aistack/aistack-operator/blob/main/docs/migration/from-v0.5.md).
...
#### Ensure the AiStack Worker Is Reachable
...
If the AiStack server is deployed outside the Kubernetes cluster, set `proxy_mode=tunnel` in the `Worker Configuration` to enable the AiStack Worker **Tunnel** mode. This keeps a long-lived connection from the worker to the AiStack server and provides a tunnel for GPU Service to reach the cluster.
...
You can verify connectivity between the AiStack server and worker on the `Resources` > `Workers` page by checking the worker's `Status` column.
...
After you add a `GPU Service`-enabled Kubernetes cluster, the [AiStack Operator](https://github.com/aistack/aistack-operator) automatically discovers the GPU devices in the cluster, gathers their information, and generates the corresponding instance types for GPU Service.
...
Initially, the AiStack Operator provides a fixed profile for the CPU-only instance type: `1 CPU + 2 GB RAM`. To customize it, see [GPU Service Instance Types](gpuservice-instance-types.md).
...
The figures come from the operator's instance metrics subresource and node exporter; see the [Instance Metrics reference](https://github.com/aistack/aistack-operator/blob/main/docs/reference/instance-metrics.md) for every field, its source, and its limits.
