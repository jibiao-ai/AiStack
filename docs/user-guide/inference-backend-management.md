...
AiStack allows admins to configure inference backends and backend versions.
...
AiStack supports three types of inference backends:

- **Built-in**: Pre-configured backends maintained by AiStack (e.g., vLLM, MindIE, VoxBox). These cannot be deleted.
...
The Built-in and Community lists are updated over the air, without upgrading AiStack. You can also point either list at a document of your own. See [OTA Content Updates](ota-content-updates.md).
...
`Flag Format` controls how AiStack renders each `--key value` pair when assembling the backend's launch command. It is configured at the backend level, applies to all model deployments that use this backend, and takes effect when the worker assembles the container launch command.
