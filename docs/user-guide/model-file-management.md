...
AiStack allows admins to download and manage model files.

## Add Model File

AiStack currently supports models from [Hugging Face](https://huggingface.co), [ModelScope](https://modelscope.cn), and local paths. To add model files, navigate to the `Model Files` page.
...
5. _(Optional)_ Specify a `Local Directory` to download the model to a custom path instead of the AiStack cache directory.
...
4. AiStack will attempt to download the model file again from the specified source.

## Deploy Model

Models can be deployed from model files. Since the model is stored on a specific worker, AiStack will add a worker selector using the `worker-name` key to ensure proper scheduling.
