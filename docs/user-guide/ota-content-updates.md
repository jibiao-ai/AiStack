...
AiStack fetches its model catalog, Community Backend Marketplace and built-in backend versions from an official OTA server, so new models and backend versions reach a running AiStack service without an upgrade or a restart.

Updates are on by default, and the content comes from a single host, `https://ota.aistack.ai/latest`. If your AiStack service runs in a restricted network, allowing that one host is enough. You can also switch to a file you maintain yourself that is reachable from that network. A third option is to host your own copy of the OTA server and point AiStack at it at startup with `--ota-server-url`. All three kinds of content still update automatically, they just read from your copy. See [start](../cli-reference/start.md).
