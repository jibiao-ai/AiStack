...
AiStack supports Single Sign-On (SSO) authentication methods such as OIDC, SAML, and CAS. This allows users to log in using their existing credentials from an external identity provider.
...
If your OIDC provider uses a certificate issued by a private or corporate CA, see [Additional Trusted CAs](../installation/installation.md#additional-trusted-cas) for how to mount CA certificates into the AiStack container.
...
```bash
AISTACK_OIDC_ISSUER="your-oidc-issuer-url"
AISTACK_OIDC_CLIENT_ID="your-client-id"
AISTACK_OIDC_CLIENT_SECRET="your-client-secret"
AISTACK_OIDC_REDIRECT_URI="{your-server-url}/auth/oidc/callback"
# Optional
AISTACK_EXTERNAL_AUTH_NAME="email"
AISTACK_EXTERNAL_AUTH_FULL_NAME="name"
AISTACK_EXTERNAL_AUTH_AVATAR_URL="picture"
AISTACK_EXTERNAL_AUTH_DEFAULT_INACTIVE="true"
AISTACK_EXTERNAL_AUTH_POST_LOGOUT_REDIRECT_KEY="returnTo"  # e.g., for Auth0
```

### Example: Integrate with Auth0 OIDC

To configure AiStack with Auth0 as the OIDC provider:
...
Then, run AiStack with relevant OIDC configuration. The following example uses Docker with CUDA:
...
sudo docker run -d --name aistack \
    ...
    --runtime nvidia \
    -e AISTACK_OIDC_ISSUER="https://<your-auth0-domain>" \
    -e AISTACK_OIDC_CLIENT_ID="<your-client-id>" \
    -e AISTACK_OIDC_CLIENT_SECRET="<your-client-secret>" \
    -e AISTACK_OIDC_REDIRECT_URI="<your-server-url>/auth/oidc/callback" \
    aistack/aistack
```
...
AiStack supports SAML authentication for Single Sign-On (SSO). This allows users to log in using their existing credentials from an external identity provider that supports SAML.
...
| `--saml-sp-acs-url`                           | SAML Service Provider Assertion Consumer Service URL. It should be set to `<aistack-server-url>/auth/saml/callback`.                                                                                                                                          |
...
```bash
AISTACK_SAML_IDP_SERVER_URL="https://idp.example.com"
AISTACK_SAML_IDP_ENTITY_ID="your-idp-entity-id"
AISTACK_SAML_IDP_X509_CERT="your-idp-x509-cert"
AISTACK_SAML_SP_ENTITY_ID="your-sp-entity-id"
AISTACK_SAML_SP_ACS_URL="{your-server-url}/auth/saml/callback"
AISTACK_SAML_SP_X509_CERT="your-sp-x509-cert"
AISTACK_SAML_SP_PRIVATE_KEY="your-sp-private-key"
# Optional
AISTACK_SAML_SP_ATTRIBUTE_PREFIX="http://schemas.auth0.com/"
AISTACK_SAML_SECURITY="{}"
AISTACK_EXTERNAL_AUTH_NAME="emailaddress"
AISTACK_EXTERNAL_AUTH_FULL_NAME="name"
AISTACK_EXTERNAL_AUTH_AVATAR_URL="picture"
AISTACK_EXTERNAL_AUTH_DEFAULT_INACTIVE="true"
AISTACK_SAML_IDP_LOGOUT_URL="https://idp.example.com/saml/slo"  # if IdP supports SLO
AISTACK_SAML_SP_SLO_URL="{your-server-url}/auth/saml/logout/callback"
AISTACK_EXTERNAL_AUTH_POST_LOGOUT_REDIRECT_KEY="returnTo"  # optional, adds a post-logout parameter for compatible IdPs
```

### Example: Integrate with Auth0 SAML

To configure AiStack with Auth0 as the SAML provider:
...
6. Run AiStack with relevant SAML configuration. The following example uses Docker with CUDA:
...
sudo docker run -d --name aistack \
    ...
    --volume aistack-data:/var/lib/aistack \
    --runtime nvidia \
    -e AISTACK_SAML_IDP_SERVER_URL="<auth0-saml-protocol-url>" \
    -e AISTACK_SAML_IDP_ENTITY_ID="urn:<auth0-domain>" \
    -e AISTACK_SAML_IDP_X509_CERT="<auth0-x509-cert>" \
    -e AISTACK_SAML_SP_ENTITY_ID="urn:aistack:sp" \
    -e AISTACK_SAML_SP_ACS_URL="<your-aistack-server-url>/auth/saml/callback" \
    -e AISTACK_SAML_SP_X509_CERT="$SP_CERT" \
    -e AISTACK_SAML_SP_PRIVATE_KEY="$SP_PRIVATE_KEY" \
    -e AISTACK_SAML_SP_ATTRIBUTE_PREFIX="$SP_ATTRIBUTE_PREFIX" \
    -e AISTACK_SAML_IDP_LOGOUT_URL="<idp-slo-url-if-available>" \
    -e AISTACK_SAML_SP_SLO_URL="<your-aistack-server-url>/auth/saml/logout/callback" \
    -e AISTACK_EXTERNAL_AUTH_POST_LOGOUT_REDIRECT_KEY="returnTo" \
    aistack/aistack
```

!!! note

    Not all IdPs provide standard SAML Single Logout (SLO). Auth0 SAML connections commonly do not expose `singleLogoutService`. If unavailable, AiStack will still clear local sessions on logout; for full browser sign-out with Auth0, consider using its OIDC `v2/logout` with `client_id` and `returnTo` allowed.
...
| `--cas-username-attribute` (Optional)         | CAS XML attribute name to use as the AiStack username. Defaults to the CAS `cas:user` element when unset.                                                                                 |
...
```bash
AISTACK_CAS_SERVER_URL="https://cas.example.com/cas"
# Optional
AISTACK_CAS_CALLBACK_URL="{your-server-url}/auth/cas/callback"
AISTACK_CAS_VALIDATE_ENDPOINT="/p3/serviceValidate"
AISTACK_CAS_USERNAME_ATTRIBUTE="uid"
AISTACK_CAS_FULL_NAME_ATTRIBUTE="displayName"
AISTACK_CAS_AVATAR_ATTRIBUTE=""
AISTACK_EXTERNAL_AUTH_DEFAULT_INACTIVE="true"
```

### Example: Integrate with an Apereo CAS server

1. Register AiStack as a service on the CAS server with `serviceId` matching `<your-server-url>/auth/cas/callback`. Allow the attributes you intend to map (e.g., `uid`, `displayName`).

2. Run AiStack with the CAS configuration. The following example uses Docker with CUDA:
...
sudo docker run -d --name aistack \
    ...
    --volume aistack-data:/var/lib/aistack \
    --runtime nvidia \
    -e AISTACK_SERVER_EXTERNAL_URL="https://aistack.example.com" \
    -e AISTACK_CAS_SERVER_URL="https://cas.example.com/cas" \
    -e AISTACK_CAS_USERNAME_ATTRIBUTE="uid" \
    -e AISTACK_CAS_FULL_NAME_ATTRIBUTE="displayName" \
    aistack/aistack
```

!!! note

    If your CAS server uses a certificate issued by a private or corporate CA, see [Additional Trusted CAs](../installation/installation.md#additional-trusted-cas) for how to mount CA certificates into the AiStack container.
