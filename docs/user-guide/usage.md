...
The **Usage** page reports how your AiStack resources are consumed over time — LLM
...
AiStack has two roles — **Admin** and **User** (see [User Management](user-management.md)):
...
calendar boundaries. It is controlled by the `AISTACK_TIMEZONE` server environment
variable (the legacy `AISTACK_USAGE_ROLLUP_TIMEZONE` still works as a deprecated alias) and
...
via the `AISTACK_*_RETENTION_MONTHS` and `AISTACK_*_ARCHIVE_CRON` environment variables.
