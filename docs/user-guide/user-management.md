...
AiStack has two user roles: `Admin` and `User`.
...
On bootstrap, AiStack creates a default admin user.

The initial password is saved in `<data-dir>/initial_admin_password`.

In the default setup, this file is located at `/var/lib/aistack/initial_admin_password` inside the server container.

You can set a custom password for the default admin by using the `--bootstrap-password` flag when starting `AiStack`.
