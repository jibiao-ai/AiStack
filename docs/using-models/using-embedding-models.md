...
In this guide, we will demonstrate how to deploy embedding models in AiStack and generate text embeddings using the deployed models.
...
We will use the AiStack API to generate text embeddings, and an API key is required:
...
With the model deployed and an API key, you can generate text embeddings via the AiStack API. Here is an example script using `curl`:
...
Replace `<your-server-url>` with the URL of your AiStack server and `<your-api-key>` with the API key you generated in the previous step.
