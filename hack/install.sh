...
function download_ui() {
  if [[ "${UI_DOWNLOAD:-true}" != "true" ]]; then
    aistack::log::info "skipping UI assets download"
    return
  fi
  ...
  if ! curl --retry 3 --retry-connrefused --retry-delay 3 -sSfL "https://gpustack-ui-1303613262.cos.accelerate.myqcloud.com/releases/${tag}.tar.gz" 2>/dev/null |
    ...
    if [[ "${tag:-}" =~ ^v([0-9]+)\.([0-9]+)(\.[0-9]+)?(-[0-9A-Za-z.-]+)?(\+[0-9A-Za-z.-]+)?$ ]]; then
      aistack::log::fatal "failed to download '${tag}' ui archive"
    fi
...
function copy_extra_static() {
  ...
  local ui_static_path="${ROOT_DIR}/aistack/ui/static"
...
function make_community_backends() {
  local tmp_dir
  tmp_dir=$(mktemp -d -t aistack-community-backends.XXXXXX)

  # shellcheck disable=SC2064
  trap "rm -rf \"${tmp_dir}\"" EXIT

  local target_dir="${ROOT_DIR}/aistack/assets/"
