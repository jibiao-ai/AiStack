#!/usr/bin/env python3
"""
Redump complete benchmark detail JSON from AiStack into local benchmark result files.

Examples:
  python3 hack/perf/fill_benchmark_raw_metrics.py \
    --dir $RESULTS_DIR \
    --aistack-url $AISTACK_URL \
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "detail from the AiStack server using each file's benchmark id."
        )
    )
    parser.add_argument(
        "--dir",
    ...
    parser.add_argument("--aistack-url", required=True, help="AiStack base URL.")
    parser.add_argument(
        "--aistack-token", required=True, help="AiStack API token for authentication."
    )
    parser.add_argument(
        "--force",
        action="store_true",
...
def main() -> None:
    ...
    for file_path in json_files:
        try:
            result = process_file(
                file_path,
                base_url=args.aistack_url,
                token=args.aistack_token,
