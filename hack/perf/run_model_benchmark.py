#!/usr/bin/env python3
"""
Automated AiStack serving benchmark runner.

This script reads:
1. A model/run configuration YAML
2. A benchmark profile YAML

Then it drives the full benchmark lifecycle through AiStack's HTTP API:
...
python3 hack/perf/run_model_benchmark.py \
  ...
  --profile aistack/assets/profiles_config/profiles_config.yaml \
  --aistack-url https://YOUR_AISTACK \
  --aistack-token $AISTACK_TOKEN \
  --cluster-id 1 \
...
class EngineManager:
    """
    Translate one benchmark run into concrete AiStack API operations.
    ...
    def __init__(
        ...
        self.source = source
        self.aistack_url = aistack_url.rstrip("/")
        self.aistack_token = aistack_token
    ...
    def _headers(self) -> Dict[str, str]:
        return {
            ...
            "Authorization": f"Bearer {self.aistack_token}",
        }
    ...
    def setup_model(self, model: Model):
        """
        Create a AiStack model deployment for one run definition.

        The YAML `backend_parameters`, `envs`, and backend/version fields are passed
        through almost directly to AiStack's `/v2/models` API.
        """
    ...
    def create_benchmark(
        ...
        """
        Create one AiStack benchmark job from one profile definition.
    ...
    def monitor_benchmark(
        ...
        it suitable for later offline analysis without re-querying AiStack.
        """
...
def main():
    ...
    parser.add_argument("--aistack-url", required=True, help="AiStack URL")
    parser.add_argument("--aistack-token", required=True, help="AiStack token")
    parser.add_argument(
        "--cluster-id",
        "--aistack-cluster-id",
        dest="cluster_id",
