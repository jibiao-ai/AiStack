...
function Build {
    ...
    if ($LASTEXITCODE -ne 0) {
        AiStack.Log.Fatal "failed to run uv build."
    }

    $whlFiles = Get-ChildItem -Path $distDir -Filter "*.whl" -File
    if ($whlFiles.Count -eq 0) {
        AiStack.Log.Fatal "no wheel files found in $distDir"
    }
    ...
    foreach ($whlFile in $whlFiles) {
        ...
        AiStack.Log.Info "renamed $orginalName to $newName"
    }
}
...
function Set-Version {
    $versionFile = Join-Path -Path $ROOT_DIR -ChildPath "aistack\__init__.py"
    ...
    AiStack.Log.Info "setting version to $version"
    AiStack.Log.Info "setting git commit to $gitCommitShort"
...
function Restore-Version-File {
    ...
    if ($LASTEXITCODE -ne 0) {
        AiStack.Log.Fatal "failed restore version files."
    }
}
...
try {
    ...
    AiStack.Log.Fatal "failed to build: $($_.Exception.Message)"
}
AiStack.Log.Info "--- BUILD ---"
