...
function Install-Dependency {
    ...
    if ($LASTEXITCODE -ne 0) {
        AiStack.Log.Fatal "failed to install uv."
    }

    uv sync
    if ($LASTEXITCODE -ne 0) {
        AiStack.Log.Fatal "failed run uv sync."
    }

    uv run pre-commit install
    if ($LASTEXITCODE -ne 0) {
        AiStack.Log.Fatal "failed run pre-commint install."
    }
}

function Get-UI {
    $defaultTag = "latest"
    $uiPath = Join-Path -Path $ROOT_DIR -ChildPath "aistack/ui"
    ...
    try {
        ...
        $url = "https://aistack-ui-1303613262.cos.accelerate.myqcloud.com/releases/$tag.tar.gz"
        ...
        AiStack.Log.Fatal "failed to download '$tag' UI archive: $($_.Exception.Message)"

        if (-eq $tag $defaultTag) {
            return
        }

        AiStack.Log.Warn "failed to download '$tag' UI archive, fallback to '$defaultTag' UI archive"

        try {
            $tmpFile = "$tmpPath/ui.tar.gz"
            $url = "https://aistack-ui-1303613262.cos.accelerate.myqcloud.com/releases/$defaultTag.tar.gz"
            DownloadWithRetries -url $url -outFile $tmpFile -maxRetries 3
            tar -xzf $tmpFile -C "$tmpUIPath"
        }
        catch {
            AiStack.Log.Fatal "failed to download '$defaultTag' UI archive: : $($_.Exception.Message)"
        }
    }
...
function DownloadWithRetries {
    ...
    for ($i = 1; $i -le $maxRetries; $i++) {
        try {
            AiStack.Log.Info "Attempting to download from $url (Attempt $i of $maxRetries)"
            Invoke-WebRequest -Uri $url -OutFile $outFile -ErrorAction Stop
            return
        }
        catch {
            AiStack.Log.Warn "Download attempt $i failed: $($_.Exception.Message)"
            if ($i -eq $maxRetries) {
                throw $_
            }
        }
...
try {
    ...
    AiStack.Log.Fatal "failed to download dependencies: $($_.Exception.Message)"
}
AiStack.Log.Info "-- DEPENDENCIES ---"
