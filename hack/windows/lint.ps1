...
function Lint {
    ...
    if ($result.Length -ne 0) {
        AiStack.Log.Fatal "failed with Invoke-ScriptAnalyzer lint."
    }

    uv run pre-commit run flake8 --all-files
    if ($LASTEXITCODE -ne 0) {
        AiStack.Log.Fatal "failed with flake8 lint."
    }
    uv run pre-commit run black --all-files
    if ($LASTEXITCODE -ne 0) {
        AiStack.Log.Fatal "failed with black lint."
    }
    uv run pre-commit run check-yaml --all-files
    if ($LASTEXITCODE -ne 0) {
        AiStack.Log.Fatal "failed with check-yaml lint."
    }
    uv run pre-commit run debug-statements --all-files
    if ($LASTEXITCODE -ne 0) {
        AiStack.Log.Fatal "failed with debug-statements lint."
    }
    uv run pre-commit run end-of-file-fixer --all-files
    if ($LASTEXITCODE -ne 0) {
        AiStack.Log.Fatal "failed with end-of-file-fixer lint."
    }
}
...
try {
    ...
    AiStack.Log.Fatal "failed to lint: $($_.Exception.Message)"
}
AiStack.Log.Info "--- LINT ---"
