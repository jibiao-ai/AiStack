...
function Validate {
    ...
    if ($currentChecksum.Hash -eq $expectedChecksum.Trim()) {
        AiStack.Log.Info "Checksums match."
        Remove-Item -Path $generatedChecksumFile
    }
    else {
        AiStack.Log.Fatal "Checksums do not match!`nPlease run 'Get-FileHash -Path install.ps1 -Algorithm SHA256 | Select -ExpandProperty Hash > install.ps1.sha256sum' to update the checksum."
    }
}
