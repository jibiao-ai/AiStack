...
# ================================
# AiStack migration oneshot service
# ================================

SCRIPT_ROOT=/etc/s6-overlay/scripts
source "$SCRIPT_ROOT/base.sh"
# The config should be ready before starting
source "$AISTACK_POSTGRES_CONFIG"
source "$SCRIPT_ROOT/default-variables.sh"


if [ "${AISTACK_DATA_MIGRATION}" = "true" ]; then
    if [ -f "$STATE_MIGRATION_DONE_FILE" ]; then
        echo "[INFO] Migration already completed previously. Skipping."
        exit 0
    fi

    echo "[INFO] Using AISTACK_MIGRATION_DATA_DIR: ${DATA_DIR}."
    if aistack migrate --migration-data-dir "${DATA_DIR}" \
        --database-url "postgresql://root@localhost:${EMBEDDED_DATABASE_PORT}/aistack"; then
