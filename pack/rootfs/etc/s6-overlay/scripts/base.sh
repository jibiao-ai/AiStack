        ...
        ((count++))
            if [ $count -le 10 ]; then
                if [ "$silent_mode" != "true" ]; then
                    echo "Waiting for $name configuration from AiStack..."
                fi
                sleep_time=1
        else
            if [ "$silent_mode" != "true" ]; then
...
export AISTACK_RUN_DIR="${AISTACK_RUN_DIR:-/run/aistack}"
createDir "$AISTACK_RUN_DIR"

# shellcheck disable=SC2034
export AISTACK_GATEWAY_CONFIG="${AISTACK_RUN_DIR}/gateway/.env"

# shellcheck disable=SC2034
export AISTACK_POSTGRES_CONFIG="${AISTACK_RUN_DIR}/postgresql/.env"

# shellcheck disable=SC2034
export AISTACK_OBSERVABILITY_CONFIG="${AISTACK_RUN_DIR}/observability/.env"
