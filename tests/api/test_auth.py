...
from aistack.api.exceptions import register_handlers
...
from aistack.api.exceptions import BadRequestException, UnauthorizedException
from aistack.schemas.config import GatewayModeEnum
from aistack.schemas.users import AuthProviderEnum
from aistack.routes import auth as auth_route
from aistack.routes.auth import oidc_callback
...
async def test_get_current_user_accepts_x_api_key(monkeypatch):
    ...
    monkeypatch.setattr("aistack.api.auth.get_user_from_api_token", auth_mock)

    @asynccontextmanager
    async def fake_async_session():
        yield session

    monkeypatch.setattr("aistack.api.auth.async_session", fake_async_session)
...
async def test_authenticate_request_reuses_provided_session(monkeypatch):
    ...
    from aistack.api.auth import authenticate_request
    ...
    monkeypatch.setattr(
        "aistack.api.auth.get_user_from_api_token", fake_get_user_from_api_token
    )
    ...
    monkeypatch.setattr("aistack.api.auth.async_session", _no_fresh_session)
...
def _principal_double(**overrides):
    from aistack.schemas.principals import PrincipalType
...
async def test_get_user_from_api_token_skips_verify_for_expired_key(monkeypatch):
    ...
    from aistack.api.auth import get_user_from_api_token
    ...
    monkeypatch.setattr(
        "aistack.api.auth.APIKeyService.get_by_access_key", fake_get_by_access_key
    )
    monkeypatch.setattr("aistack.api.auth.verify_hashed_secret", fake_verify)

    session = _RollbackRecordingSession([])
    user, api_key = await get_user_from_api_token(
        session, "aistack_abcd1234_c11c75ed6334ea9505da4ad9"
    )
...
async def test_get_user_from_api_token_rollback_before_verify_via_thread(monkeypatch):
    ...
    ``asyncio.to_thread``."""
    import aistack.api.auth as auth_module
    from aistack.api.auth import get_user_from_api_token
    ...
    monkeypatch.setattr("aistack.api.auth.UserService.get_by_id", fake_get_by_id)
    monkeypatch.setattr("aistack.api.auth.verify_hashed_secret", fake_verify)
...
async def test_get_user_from_api_token_prefers_the_digest(monkeypatch):
    ...
    import aistack.api.auth as auth_module
    from aistack.api.auth import get_user_from_api_token
    from aistack.security import generate_access_key, new_secret_key_digest
    ...
    monkeypatch.setattr("aistack.api.auth.UserService.get_by_id", fake_get_by_id)
    monkeypatch.setattr("aistack.api.auth.verify_hashed_secret", fail_verify)
    ...
    user, api_key = await get_user_from_api_token(
        _RollbackRecordingSession([]), f"aistack_{access_key}_{secret_key}"
    )
...
async def test_digest_mismatch_is_rejected_without_falling_back_to_argon2(monkeypatch):
    ...
    monkeypatch.setattr(
        "aistack.api.auth.verify_hashed_secret",
    ...
    user, api_key = await get_user_from_api_token(
        ...
        f"aistack_{access_key}_ffffffffffffffffffffffffffffffff",
    )
...
async def test_unusable_digest_falls_back_to_argon2(monkeypatch, stored_digest):
    ...
    """
    from aistack.api.auth import get_user_from_api_token
    from aistack.security import generate_access_key
    ...
    scheduled = {"n": 0}
    monkeypatch.setattr(
        "aistack.api.auth._schedule_secret_key_digest_backfill",
        lambda api_key, secret: scheduled.__setitem__("n", scheduled["n"] + 1),
    )

    user, api_key = await get_user_from_api_token(
        _RollbackRecordingSession([]),
        f"aistack_{access_key}_c11c75ed6334ea9505da4ad9c11c75ed",
    )
...
async def test_successful_argon2_verify_backfills_the_digest(monkeypatch):
    ...
    monkeypatch.setattr("aistack.api.auth.UserService.get_by_id", fake_get_by_id)
    monkeypatch.setattr(
        "aistack.api.auth.verify_hashed_secret", lambda hashed, secret: True
    )
    ...
    from aistack.security import verify_secret_key_digest
...
async def test_backfill_invalidates_the_key_the_read_path_reads(monkeypatch):
    ...
    """
    import aistack.api.auth as auth_module
    from aistack.server.cache import build_cache_key
    from aistack.server.services import APIKeyService
...
async def test_unusable_digest_warns_once_per_key(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="aistack.api.auth"):
        for _ in range(3):
...
async def test_get_user_from_api_token_rejects_wrong_secret(monkeypatch):
    ...
    monkeypatch.setattr("aistack.api.auth.UserService.get_by_id", fake_get_by_id)
    monkeypatch.setattr(
        "aistack.api.auth.verify_hashed_secret", lambda hashed, secret: False
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "127.0.0.1",
            {"host": "aistack.example.com", "x-forwarded-for": "8.8.8.8"},
        ),
        # IPv6 loopback.
        ("::1", {"host": "[::1]:30080"}),
        # External IP.
        ("10.0.0.1", {"host": "aistack.example.com"}),
    ],
)
async def test_get_current_user_requires_credentials(monkeypatch, client_host, headers):
    ...
    monkeypatch.setattr("aistack.api.auth.User.first_by_field", first_by_field)
    monkeypatch.setattr(
        "aistack.api.auth.UserService.get_by_username", get_by_username
    )
...
async def test_oidc_callback_uses_system_trust_store(monkeypatch):
    ...
    request.app.state.server_config = type(
        ...
        (),
        {
            ...
            "oidc_redirect_uri": "https://aistack.example.com/auth/oidc/callback",
    ...
    request.cookies = {"aistack_oidc_state": "test-state"}
    ...
    monkeypatch.setattr("aistack.routes.auth.httpx.AsyncClient", FakeAsyncClient)
    monkeypatch.setattr("aistack.routes.auth.use_proxy_env_for_url", lambda url: False)
    monkeypatch.setattr(
        "aistack.routes.auth.get_oidc_user_data",
    ...
    monkeypatch.setattr(
        "aistack.routes.auth.User.first_by_fields",
...
async def test_persisted_system_principal_without_links_hits_admin_gate():
    ...
    from aistack.api.auth import get_worker_principal, is_server_token_principal
    from aistack.api.exceptions import ForbiddenException
    from aistack.schemas.principals import Principal, PrincipalType
...
async def test_saml_callback_derives_current_url_from_configured_acs(monkeypatch):
    ...
    request = _saml_callback_request(
        "dummy-b64",
        saml_sp_acs_url="https://aistack.example.com:8443/auth/saml/callback",
    )
    ...
    assert captured["req"]["http_host"] == "aistack.example.com"
...
async def test_saml_callback_unsigned_escape_hatch_skips_toolkit(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="aistack.routes.auth"):
        await auth_route.saml_callback.__wrapped__(request=request, session=MagicMock())
...
def _install_key_lookups(monkeypatch, api_key=None, user=None):
    ...
    monkeypatch.setattr("aistack.api.auth.APIKeyService", _APIKeyService)
    monkeypatch.setattr("aistack.api.auth.UserService", _UserService)
    monkeypatch.setattr("aistack.api.auth.ApiKey.one_by_id", _one_by_id)
    return calls
...
async def test_no_assertion_headers_is_a_no_op(monkeypatch):
    from aistack.api.auth import authenticate_gateway_asserted_identity
