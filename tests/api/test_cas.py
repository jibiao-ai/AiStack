...
:mod:`aistack.routes.auth`. Mock-based; no live CAS server required."""
...
from aistack.api.exceptions import ConflictException, UnauthorizedException
...
async def test_validate_cas_ticket_parses_namespaced_success():
    data = await auth_route.validate_cas_ticket(
        ...
        ticket="ST-123",
        service="https://aistack.example.com/auth/cas/callback",
        config=_config(),
    )
...
async def test_validate_cas_ticket_strips_trailing_slash_and_uses_endpoint():
    ...
    # query value.
    assert "service=https%3A%2F%2Faistack.example.com" in url
...
async def test_cas_callback_rejects_existing_user_from_other_source(monkeypatch):
    ...
    server hijacks the existing AiStack admin / OIDC user. The
    ...
    request.app.state.server_config.server_external_url = "https://aistack.example.com"
...
def _auth_config(
    *,
    ...
    data_dir: str = "/tmp/__aistack_test_nonexistent__",
) -> MagicMock:
    cfg = MagicMock()
...
def test_cas_service_url_uses_only_starlette_request_surface():
    ...
    config.server_external_url = "https://aistack.example.com"
    request = _StrictRequest(_App(), config)

    result = auth_route._cas_service_url(request, config)
    assert result == "https://aistack.example.com/auth/cas/callback"


def test_cas_service_url_prefers_explicit_callback_override():
    request = _cas_request(
        cas_callback_url="https://aistack.example.com/cas-cb",
        server_external_url="https://aistack.example.com",
    )
    assert (
        auth_route._cas_service_url(request, request.app.state.server_config)
        == "https://aistack.example.com/cas-cb"
    )
...
def test_cas_service_url_uses_server_external_url_when_no_override():
    ...
    assert (
        auth_route._cas_service_url(request, request.app.state.server_config)
        == "https://aistack.example.com/auth/cas/callback"
    )
