...
from aistack.api.tenant import TenantContext
from aistack.routes import cache_services as cache_services_route
from aistack.routes import clusters as clusters_route
from aistack.routes import cloud_credentials as cloud_credentials_route
from aistack.routes import gpu_instances as gpu_instances_route
from aistack.routes import model_instances as model_instances_route
...
from aistack.server import services as services_module
...
async def test_apiserver_proxy_denies_cross_tenant(monkeypatch):
    ...
    with pytest.raises(NotFoundException):
        await clusters_route.cluster_apiserver_proxy(
            ...
            ctx=_user_ctx(),
            id=2,
            path="apis/worker.aistack.ai/v1/namespaces/aistack-user-7/instances/x/log",
        )
...
async def test_apiserver_proxy_denies_other_namespace(monkeypatch):
    ...
    with pytest.raises(ForbiddenException):
        await clusters_route.cluster_apiserver_proxy(
            ...
            ctx=_personal_ctx(6),
            id=1,
            path="apis/worker.aistack.ai/v1/namespaces/aistack-user-999/instances/x/log",
        )
...
async def test_apiserver_proxy_denies_member_write(monkeypatch):
    ...
    with pytest.raises(ForbiddenException):
        await clusters_route.cluster_apiserver_proxy(
            ...
            ctx=_personal_ctx(6),
            id=1,
            path="apis/worker.aistack.ai/v1/namespaces/aistack-user-6/instances/x",
        )
...
async def test_apiserver_proxy_allows_own_namespace_workload(monkeypatch):
    ...
    with pytest.raises(ServiceUnavailableException):
        await clusters_route.cluster_apiserver_proxy(
            ...
            ctx=_personal_ctx(6),
            id=1,
            path="apis/worker.aistack.ai/v1/namespaces/aistack-user-6/instances/x/events",
        )
...
async def test_apiserver_proxy_allows_org_member_namespace(monkeypatch):
    ...
    with pytest.raises(ServiceUnavailableException):
        await clusters_route.cluster_apiserver_proxy(
            ...
            ctx=member_ctx,
            id=1,
            path="apis/worker.aistack.ai/v1/namespaces/aistack-acme/instances/x/events",
        )
