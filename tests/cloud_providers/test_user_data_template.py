from aistack.cloud_providers.user_data import UserDataTemplate, ManufacturerEnum
import yaml
...
def test_userdata_template_basic():
    template = UserDataTemplate(
        ...
        token="test-token",
        image_name="aistack/test:latest",
    ...
    assert any(
        f["path"] == "/var/lib/aistack/config.yaml" for f in data["write_files"]
    )
    assert any(f["path"] == "/opt/aistack-run-worker.sh" for f in data["write_files"])
...
def test_userdata_template_no_driver():
    ...
    assert "/opt/aistack-run-worker.sh" in data["runcmd"]
...
def test_userdata_template_env_in_worker_script():
    ...
    worker_script_file = next(
        f for f in data["write_files"] if f["path"] == "/opt/aistack-run-worker.sh"
    )
    assert (
        "--config-file=/var/lib/aistack/config.yaml" in worker_script_file["content"]
    )
...
def test_userdata_template_secret_configs():
    ...
    data = yaml.safe_load(result)
    config_file = next(
        f for f in data["write_files"] if f["path"] == "/var/lib/aistack/config.yaml"
    )
