...
from aistack.cmd.prerun import prepare_observability_config
from aistack.config.config import Config
from aistack.routes import prometheus as prometheus_routes
...
def test_prepare_prometheus_config_writes_observability_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AISTACK_RUN_DIR", str(tmp_run))
...
def test_cache_service_dashboard_json_matches_route_contract(tmp_path):
    ...
    assert dashboard["title"] == "AiStack Cache Service"
...
def test_provider_dashboard_json_matches_declaration():
    ...
    query string sets."""
    from aistack.server.cache_provider_catalog import load_cache_providers
