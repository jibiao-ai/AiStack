...
``aistack.ai/instance-id`` label first, else namespace reverse-resolution) and
...
from aistack.gpu_instances.controllers import GPUInstanceController
from aistack.schemas.gpu_instances import KUBERES_INSTANCE_ID_LABEL
from aistack.schemas.principals import platform_principal_id
from aistack.server.workqueue import WorkEventType
...
def _line(
    etype, *, name="gi-1", namespace="aistack-user-1", labels=None, deleting=False
):
...
async def test_resolve_owner_from_user_namespace(controller):
    ...
    owner = await controller._resolve_owner_principal_id(None, "aistack-user-9")
    assert owner == 9


@pytest.mark.asyncio
async def test_resolve_owner_from_default_namespace_is_platform(controller):
    owner = await controller._resolve_owner_principal_id(None, "aistack-default")
