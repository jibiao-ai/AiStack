...
from aistack.server.controllers import WorkerProvisioningController
from aistack.cloud_providers.abstract import CloudInstance, InstanceState, Volume
...
async def test_provisioning_flow(monkeypatch):
    ...
    monkeypatch.setattr("aistack.config.config.get_global_config", lambda: cfg)
    mock_sshkey = MagicMock()
    mock_sshkey.id = "ssh-key-id"
    monkeypatch.setattr(
        "aistack.schemas.clusters.Credential.create",
        AsyncMock(return_value=mock_sshkey),
    )
    monkeypatch.setattr(
        "aistack.cloud_providers.common.get_client_from_provider",
        lambda provider, credential: client,
    )
    monkeypatch.setattr(
        "aistack.schemas.clusters.Credential.one_by_id",
        AsyncMock(return_value=MagicMock(id=1, external_id="ssh-key-id")),
    )
    monkeypatch.setattr(
        "aistack.schemas.workers.Worker.one_by_id", AsyncMock(return_value=worker)
    )
    monkeypatch.setattr(
        "aistack.schemas.clusters.CloudCredential.one_by_id",
        AsyncMock(return_value=credential),
    )
    monkeypatch.setattr("aistack.server.services.WorkerService.update", AsyncMock())
...
async def test_deleting_flow(monkeypatch):
    ...
    monkeypatch.setattr("aistack.server.services.WorkerService.delete", AsyncMock())
