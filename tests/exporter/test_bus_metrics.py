...
async def test_bus_metrics_collector_reflects_subscriber_state():
    ...
    try:
        ...
        for sample in _all_samples(metrics, "aistack:bus_events"):
            if (
...
async def test_subscribers_sharing_source_collapse_to_one_series():
    ...
    try:
        ...
        for metric_name in (
            "aistack:bus_subscribers",
            "aistack:bus_queue_depth",
            "aistack:bus_queue_capacity",
            "aistack:bus_queue_full",
            "aistack:bus_queue_saturation_ratio",
            "aistack:bus_subscriber_latest_keys",
        ):
            matching = [
                s
        ...
        assert _sample_for(metrics, "aistack:bus_subscribers", label).value == 3
        # queue_depth = max across subscribers (subs[1] has 2 queued).
        assert _sample_for(metrics, "aistack:bus_queue_depth", label).value == 2
