...
from aistack.exporter.exporter import MetricExporter, _metrics_targets
...
from aistack.schemas.config import ModelInstanceProxyModeEnum
from aistack.schemas.models import ModelInstanceStateEnum
from aistack.schemas.workers import WorkerStateEnum
...
async def test_model_instance_restart_metrics_are_collected():
    ...
    with (
        patch("aistack.exporter.exporter.Cluster.all", return_value=[cluster]),
        patch(
            "aistack.exporter.exporter.CacheService.all_by_fields",
            return_value=[],
        ),
    ):
        metrics = await exporter._collect_metrics(session=SimpleNamespace())

    assert _sample_value(metrics, "aistack:model_instance_restart_count") == 3
    assert (
        _sample_value(metrics, "aistack:model_instance_latest_restart_time")
        == latest_restart_time.timestamp()
    )
    assert _sample_labels(metrics, "aistack:model_instance_restart_count") == {
        "cluster_id": "1",
...
async def test_model_info_metric_includes_category_label():
    ...
    with (
        patch(
            "aistack.exporter.exporter.Cluster.all",
    ...
    assert _sample_labels(metrics, "aistack:model") == {
        "cluster_id": "1",
        "cluster_name": "default",
        "model_id": "10",
...
async def test_model_info_metric_uses_unknown_for_uncategorized_models():
    ...
    labels = _sample_labels(metrics, "aistack:model")
...
async def test_generate_metrics_cache_survives_transient_db_error(monkeypatch):
    ...
    monkeypatch.setattr(
        "aistack.exporter.exporter.async_session", lambda: _NoopSession()
    )
    monkeypatch.setattr("aistack.exporter.exporter.asyncio.sleep", _sleep_then_stop)
...
async def test_cache_service_attached_model_relation_metric():
    ...
    from aistack.schemas.models import ExtendedKVCacheConfig, KVCacheModeEnum
    ...
    assert _sample_labels(metrics, "aistack:cache_service_attached_model") == {
        "cluster_id": "1",
        "cluster_name": "default",
        "cache_service_id": "3",
...
def _patch_target_sources(
    ...
    monkeypatch.setattr(
        "aistack.exporter.exporter.Worker.all", AsyncMock(return_value=workers)
    )
    services_mock = AsyncMock(return_value=services)
    monkeypatch.setattr(
        "aistack.exporter.exporter.CacheService.all_by_fields", services_mock
    )
    monkeypatch.setattr(
        "aistack.exporter.exporter.CacheServiceInstance.all_by_fields",
        AsyncMock(return_value=instances or []),
    )
    monkeypatch.setattr(
        "aistack.exporter.exporter.Cluster.all", AsyncMock(return_value=clusters)
    )
...
def _cache_groups(targets):
    return [
        ...
        if group["labels"].get("aistack_target_type") == "cache-service"
    ]
    ...
    assert cache_groups == [
        {
            "labels": {
                ...
                "cache_service_name": "shared-lmcache",
                "provider": "LMCache",
                "aistack_target_type": "cache-service",
                "worker_name": "worker-2",
...
async def test_l2_metrics_target_field_adds_direct_scrape_target(monkeypatch):
    ...
    monkeypatch.setattr(
        "aistack.exporter.exporter.get_cache_provider",
...
async def test_external_fields_metrics_target_field(
    ...
    monkeypatch.setattr(
        "aistack.exporter.exporter.get_cache_provider", lambda name: provider
    )
    service = _cache_service(
        provider_name="StubCache",
        mode=CacheServiceModeEnum.EXTERNAL,
