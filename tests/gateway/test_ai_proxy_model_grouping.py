...
from aistack.schemas.model_routes import ModelRoute
...
def test_provider_id_is_per_deployment():
    assert model_ai_proxy_provider_id(7) == "aistack-model-7"


def test_api_tokens_only_present_when_supplied():
    with_token = ai_proxy_openai_provider_config("aistack-model-7", ["aistack_ak_sk"])
    assert with_token["apiTokens"] == ["aistack_ak_sk"]
    ...
    assert "apiTokens" not in ai_proxy_openai_provider_config("aistack-model-7")
...
def test_plugin_spec_groups_by_deployment():
    ...
    assert [p["id"] for p in providers] == ["aistack-model-5", "aistack-model-9"]
    ...
    assert [(r.config["activeProviderId"], r.ingress, r.service) for r in rules] == [
        ("aistack-model-5", [ROUTE_A], ["model-5-1.static"]),
        ("aistack-model-5", [ROUTE_A_FALLBACK], ["model-5-1.static"]),
        (
            "aistack-model-9",
            [ROUTE_A],
...
def test_two_routes_share_one_deployment_provider():
    ...
    """
    live = _spec(
        providers=[ai_proxy_openai_provider_config("aistack-model-5", ["token"])],
        match_rules=[_rule("aistack-model-5", ROUTE_B, ["model-5-1.static"])],
    )
    providers, rules = model_ai_proxy_plugin_spec(
        [
            ModelAIProxyGroup(
    ...
    assert _provider_ids(result) == ["aistack-model-5"], "provider must not duplicate"
    assert _rule_keys(result) == [
        ("aistack-model-5", (ROUTE_A,), ("model-5-1.static",)),
        ("aistack-model-5", (ROUTE_B,), ("model-5-1.static",)),
    ]
...
def test_route_removal_keeps_provider_while_another_route_references_it():
    live = _spec(
        providers=[ai_proxy_openai_provider_config("aistack-model-5", ["token"])],
        match_rules=[
            _rule("aistack-model-5", ROUTE_A, ["model-5-1.static"]),
            _rule("aistack-model-5", ROUTE_B, ["model-5-1.static"]),
        ],
    )
    ...
    assert _provider_ids(result) == ["aistack-model-5"]


def test_last_route_removal_garbage_collects_the_provider():
    live = _spec(
        providers=[ai_proxy_openai_provider_config("aistack-model-5", ["token"])],
        match_rules=[_rule("aistack-model-5", ROUTE_A, ["model-5-1.static"])],
    )
...
def test_fallback_ingress_rule_is_dropped_when_fallback_goes_away():
    live = _spec(
        ...
        match_rules=[
            _rule("aistack-model-5", ROUTE_A, ["model-5-1.static"]),
            _rule("aistack-model-5", ROUTE_A_FALLBACK, ["model-5-1.static"]),
        ],
    )
    providers, rules = model_ai_proxy_plugin_spec(
        [
...
def test_token_change_updates_the_existing_provider_entry():
    live = _spec(
        providers=[ai_proxy_openai_provider_config("aistack-model-5", ["old-token"])],
        match_rules=[_rule("aistack-model-5", ROUTE_A, ["model-5-1.static"])],
    )
    providers, rules = model_ai_proxy_plugin_spec(
        [
...
def test_external_provider_reconcile_leaves_deployment_entries_alone():
    ...
    live = _spec(
        providers=[
            ai_proxy_openai_provider_config("aistack-model-5", ["token"]),
    ...
    assert _provider_ids(result) == ["aistack-model-5", "provider-3"]
...
async def test_startup_cleanup_drops_only_entries_nothing_will_reconcile():
    ...
    """
    live_plugin = {
        "metadata": {"name": "aistack-ai-proxy"},
        "spec": {
            "defaultConfig": {
                "providers": [
                    {"id": "ai-route-route-1", "type": "openai"},
                    {"id": "ai-route-route-404", "type": "openai"},
                    {"id": "aistack-model-5", "type": "openai"},
                    {"id": "aistack-model-404", "type": "openai"},
            ...
            "matchRules": [
                ...
                _rule("aistack-model-5", ROUTE_A, ["model-5-1.static"]).model_dump(),
                _rule(
                    "aistack-model-404", ROUTE_B, ["model-404-1.static"]
                ).model_dump(),
            ],
        },
    }
    ...
    with (
        patch("aistack.gateway.utils.ExtensionsHigressIoV1Api", return_value=api),
        patch("aistack.gateway.utils.k8s_client.ApiClient"),
    ):
...
async def test_startup_cleanup_drops_rules_of_a_deleted_route():
    ...
    live_plugin = {
        ...
        "spec": {
            ...
            "matchRules": [
                ...
                _rule("aistack-model-5", ROUTE_B, ["model-5-1.static"]).model_dump(),
                WasmPluginMatchRule(
                    config={"activeProviderId": "provider-3"},
                    configDisable=False,
                    service=["provider-3.dns"],
    ...
    assert _provider_ids(body.spec) == ["aistack-model-5", "provider-3"]
...
def test_ingress_names_for_plugins_qualify_across_namespaces():
    ...
    assert route_ingress_names_for_plugins(42, "aistack", "aistack") == (
...
def test_none_config_and_default_config_are_tolerated():
    ...
    result = ai_proxy_diff_spec(
        live,
        expected_providers=[ai_proxy_openai_provider_config("aistack-model-5", ["t"])],
        expected_match_rules=[_rule("aistack-model-5", ROUTE_A, ["model-5-1.static"])],
        owned_ingresses={ROUTE_A, ROUTE_A_FALLBACK},
    )
