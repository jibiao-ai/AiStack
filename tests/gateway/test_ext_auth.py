...
from aistack.gateway.client import WasmPluginMatchRule, WasmPluginSpec
...
from aistack.server.gateway_auth_reconciler import (
    KEY_ENTRY_BYTES,
    MATCH_RULE_BYTES,
...
def stub_reconcile_inputs(monkeypatch):
    ...
    monkeypatch.setattr(
        "aistack.server.gateway_auth_reconciler.async_session", _session
    )
    monkeypatch.setattr(
        "aistack.server.gateway_auth_reconciler.build_local_auth_tables", _tables
    )
    monkeypatch.setattr(
        "aistack.server.gateway_auth_reconciler.build_public_route_ids", _ids
    )
    monkeypatch.setattr(
        "aistack.server.gateway_auth_reconciler.ensure_wasm_plugin", _ensure
    )
    return state
...
async def test_an_unchanged_pass_says_nothing(stub_reconcile_inputs, caplog):
    ...
    with caplog.at_level(
        logging.DEBUG, logger="aistack.server.gateway_auth_reconciler"
    ):
        await reconciler.reconcile()
        first = caplog.text
        caplog.clear()
