...
from aistack.gateway import transformer_plugin
...
FALLBACK_PATH_VALUE_EXPR = f"%REQ({aistack_original_path_header.upper()})%"
...
def test_get_4xx_5xx_fallback_value_with_extra_req_headers():
    policy = _redirect_policy(
        get_4xx_5xx_fallback_value(
            ...
            extra_req_headers={aistack_fallback_path_header: FALLBACK_PATH_VALUE_EXPR},
        )
    )
    ...
    # Guard against leaking the internal x-aistack-fallback-path header back
...
def test_get_ingress_fallback_envoyfilter_shape():
    ...
    assert aistack_fallback_path_header in req_keys
...
def test_transformer_plugin_fallback_path_req_rules():
    ...
    # is not enough. We snapshot :path into x-aistack-original-path first, then on
    # redirect the EnvoyFilter injects x-aistack-fallback-path=%REQ(...)% which this
    ...
    rename_idx = next(
        ...
        if r["operate"] == "rename"
        and {"oldKey": aistack_fallback_path_header, "newKey": ":path"} in r["headers"]
    )
    map_idx = next(
        i
        for i, r in enumerate(rules)
        if r["operate"] == "map"
        and r["headers"]
        == [{"fromKey": ":path", "toKey": aistack_original_path_header}]
    )
    remove_idx = next(
        i
        for i, r in enumerate(rules)
        if r["operate"] == "remove"
        and any(h.get("key") == aistack_fallback_path_header for h in r["headers"])
    )
