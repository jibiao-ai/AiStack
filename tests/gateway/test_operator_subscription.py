"""Guards for the aistack-operator worker-gateway subscription reconciler.

G1-G3 pin aistack#5947: a cluster is subscribed only while it has a READY
...
from aistack.gpu_instances import gateway, gateway_client
...
async def test_subscription_carries_instance_and_instancetype_gvk(harness):
    ...
    assert ("worker.aistack.ai", "v1", "InstanceType") in gvk
    assert ("worker.aistack.ai", "v1", "Instance") in gvk
