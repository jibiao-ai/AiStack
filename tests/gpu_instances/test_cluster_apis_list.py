...
from aistack.gpu_instances import cluster_apis
from aistack.gpu_instances.cluster_apis import (
    ClusterOps,
    _CRDSpec,
    _Scope,
    _INSTANCE,
...
from aistack.gpu_instances.cluster_apis_util import DEFAULT_SYSTEM_NAMESPACE
...
_GROUP = "worker.aistack.ai"
_VERSION = "v1"

# The org namespace the ``ops`` fixture's principal derives.
_ORG_NAMESPACE = "aistack-default"

# A system-namespaced spec in a *different* group, standing in for the
# operator's ``aistack.ai/v1 Setting`` (owned by the task that adds it for
...
def test_system_namespace_defaults_to_aistack_system(ops):
    # A cluster whose ``k8s_options.namespace`` is unset renders its operator
    # into ``aistack-system``, so that is where its system-namespaced
    # resources are read from.
    assert DEFAULT_SYSTEM_NAMESPACE == "aistack-system"
    assert ops.system_namespace == "aistack-system"
...
async def test_create_ensures_only_the_org_namespace(monkeypatch, ops, spec, ensured):
    # The org namespace is AiStack's to create; the cluster's system namespace
