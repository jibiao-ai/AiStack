...
The operator seeds each ``Setting`` from ``AISTACK_<UPPER_SNAKE_NAME>`` on its
...
from aistack.gpu_instances import settings
...
DEFAULT_NAMESPACE = "aistack-system"
...
async def test_unmanaged_knobs_are_never_read_or_written(harness):
    """AC4.4: an unset knob is not AiStack's, so it is not even looked at —
...
async def test_model_service_cluster_is_never_touched(harness):
    ...
    Service; it has no operator settings AiStack owns, so no client is even
    built for it."""
...
async def test_switching_to_model_service_stops_convergence(harness):
    """After a purpose switch the settings stop being AiStack's, so later drift
...
async def test_external_drift_is_repaired_by_the_periodic_resync(harness):
    ...
    three settings AiStack manages would otherwise leave the UI and the cluster
...
async def test_read_setting_addresses_the_operator_group(monkeypatch, ops):
    """``Setting`` is ``aistack.ai/v1``, not the ``worker.aistack.ai/v1`` every
