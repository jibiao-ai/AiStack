...
from aistack.schemas.gpu_instances import GPUInstanceResources
...
def test_partitioned_profile_round_trips_with_camel_alias():
    payload = {"accelerator": "1", "acceleratorPartitionedProfile": "3g.40gb"}
    r = GPUInstanceResources.model_validate(payload)
    assert r.accelerator_partitioned_profile == "3g.40gb"
    assert r.model_dump(by_alias=True, exclude_none=True) == payload
