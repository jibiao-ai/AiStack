...
from aistack.policies.candidate_selectors import GGUFResourceFitSelector
...
from aistack.scheduler import scheduler
...
async def test_schedule_to_single_worker_single_gpu(config):
    ...
    with (
        patch(
            'aistack.policies.candidate_selectors.gguf_resource_fit_selector.calculate_gguf_model_resource_claim',
            side_effect=mock_calculate_model_resource_claim,
        ),
        patch(
            'aistack.scheduler.scheduler.BackendFrameworkFilter._has_supported_runners',
            return_value=True,
        ),
        patch(
            'aistack.policies.worker_filters.backend_framework_filter.async_session',
...
async def test_schedule_to_single_worker_multi_gpu_with_deepseek_r1(config):
    ...
    with (
        ...
        patch(
            'aistack.schemas.workers.Worker.all',
...
async def test_schedule_to_multi_worker_multi_gpu(config):
    ...
    with (
        ...
        patch(
            'aistack.policies.scorers.placement_scorer.async_session',
            return_value=mock_async_session(),
        ),
        patch(
            'aistack.policies.scorers.model_file_locality_scorer.async_session',
            return_value=mock_async_session(),
        ),
    ):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        #   see https://github.com/aistack/aistack/issues/2451.
        (
            new_model(
                id=1,
