...
from aistack.schemas.models import Model, BackendEnum
from aistack.schemas.inference_backend import InferenceBackend, VersionConfig
from aistack.schemas.runner_source import RunnerOverrideEntry
...
def create_inference_backend(
    ...
    from aistack.schemas.inference_backend import VersionConfigDict
...
async def test_cpu_only_worker_filtered_out():
    ...
    with patch(
        ...
        with patch(
            'aistack.policies.worker_filters.backend_framework_filter.merged_service_runners',
            return_value=[],
        ):
            filtered_workers, messages = await filter_instance.filter(workers)
...
async def test_model_spec_without_owner_does_not_crash():
    ...
    from aistack.schemas.model_sets import ModelSpec
...
async def test_a_remote_document_replaces_the_packaged_runner_catalog(source_type):
    ...
    async def filter_with(overrides):
        with (
            ...
            patch(
                'aistack.schemas.runner_source.list_runners',
