...
from aistack.routes.clusters import get_server_url
from aistack.schemas.clusters import ClusterUpdate
...
def test_get_server_url_strips_trailing_slash_from_external_url():
    ...
    with patch("aistack.routes.clusters.get_global_config", return_value=config):
...
def test_get_server_url_falls_back_to_request_host():
    ...
    with patch(
        "aistack.routes.clusters.get_global_config",
        return_value=config,
    ):
        server_url = get_server_url(
...
def test_get_trusted_hosts_resolution(tmp_path):
    ...
    derived = Config(
        data_dir=data_dir, server_external_url="https://aistack.example:30080"
    )
    assert derived.get_trusted_hosts() == ["aistack.example"]

    # An explicit empty list behaves like unset: derive from server_external_url.
    empty = Config(
        data_dir=data_dir,
        trusted_hosts=[],
        server_external_url="https://aistack.example:30080",
    )
    assert empty.get_trusted_hosts() == ["aistack.example"]
    ...
    assert blank.get_trusted_hosts() == ["aistack.example"]
