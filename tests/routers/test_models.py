...
from aistack.routes import models as models_route
from aistack.routes.models import create_model, update_model
from aistack.routes.model_common import ModelStateFilterEnum
from aistack.schemas.models import ModelCreate, ModelUpdate, SourceEnum
from aistack.schemas.principals import PrincipalType, platform_principal_id
...
async def test_create_model_rejects_default_org_cluster_for_custom_org(monkeypatch):
    ...
    monkeypatch.setattr(
        "aistack.routes.models.Cluster.one_by_id",
...
async def test_create_model_allows_own_org_cluster(monkeypatch):
    ...
    monkeypatch.setattr(
        "aistack.routes.models.Model.one_by_fields",
...
async def _run_update(monkeypatch, ctx, cluster_return):
    ...
    monkeypatch.setattr(
        "aistack.routes.models.Model.one_by_id",
        AsyncMock(return_value=model),
    )
    monkeypatch.setattr(
        "aistack.routes.models.assert_resource_visible",
        lambda *a, **k: None,
    )
    monkeypatch.setattr(
        "aistack.routes.models.Cluster.one_by_id",
...
async def test_update_model_rejects_gpu_selector_on_vgpu_model(monkeypatch):
    ...
    from aistack.api.exceptions import BadRequestException
    from aistack.schemas.models import GPUSelector, GPUTypeSelector
    ...
    monkeypatch.setattr(
        "aistack.routes.models.assert_cluster_belongs_to_org",
        AsyncMock(),
    )
