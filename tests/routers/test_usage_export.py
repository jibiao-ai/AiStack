...
from aistack.routes import usage as usage_routes
...
async def test_oversized_sheet_is_rejected_with_actionable_details(monkeypatch):
    ...
    monkeypatch.setattr("aistack.routes.usage.envs.USAGE_EXPORT_MAX_ROWS", 25)
    user = _admin()
...
async def test_xlsx_falls_back_to_csv_beyond_the_worksheet_row_limit(monkeypatch):
    ...
    """
    monkeypatch.setattr("aistack.routes.usage.envs.XLSX_MAX_ROWS_PER_SHEET", 5)
    user = _admin()
...
async def test_too_many_sheets_is_rejected(monkeypatch):
    monkeypatch.setattr("aistack.routes.usage.envs.USAGE_EXPORT_MAX_SHEETS", 1)
    user = _admin()
    session = _session(counts=[], stream_rows=[])
    request = UsageExportRequest(
        start_date=date(2026, 4, 1),
...
async def test_estimate_judges_the_limit_per_sheet_not_on_the_sum(monkeypatch):
    ...
    """
    monkeypatch.setattr("aistack.routes.usage.envs.USAGE_EXPORT_MAX_ROWS", 100000)
    monkeypatch.setattr("aistack.routes.usage.envs.USAGE_EXPORT_SOFT_ROWS", 10000)
    user = _admin()
...
async def test_existence_lookup_is_chunked_under_the_bind_parameter_cap():
    ...
    """
    from aistack.routes.usage import _existing_entity_ids, _ID_LOOKUP_CHUNK
...
async def test_existence_lookup_skips_the_query_when_there_is_nothing_to_check():
    from aistack.routes.usage import _existing_entity_ids
    from aistack.schemas.principals import Principal
