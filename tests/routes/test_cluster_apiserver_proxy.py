"""G4 (aistack#5947): a cluster without a reachable worker is an expected
condition, not a server fault.

The aistack-operator polls this proxy for worker API-service readiness, so
...
from aistack.schemas.clusters import ClusterProvider
from aistack.schemas.principals import PrincipalType

CLUSTER_ID = 1
PROXY_PATH = "apis/apiregistration.k8s.io/v1/apiservices/v1.worker.aistack.ai"
NO_WORKERS_MESSAGE = f"No reachable workers in cluster default(id: {CLUSTER_ID})"
EXCEPTIONS_LOGGER = "aistack.api.exceptions"
