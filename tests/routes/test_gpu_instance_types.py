...
from aistack.routes import gpu_instance_types as it_routes
from aistack.routes import gpu_instances_helper as helper
from aistack.schemas.clusters import ClusterProvider, GpuInstanceOptions, K8sOptions
...
from aistack.schemas.principals import OrgRole, PrincipalType
...
LIVE_ITEM = {
    "metadata": {
        "name": "a10g",
        "labels": {"schedule.aistack.ai/derived-from-node": "true"},
    },
    "spec": {
        "acceleratable": True,
...
async def test_aggregated_watch_wraps_gateway_verbs(monkeypatch):
    ...
    # AiStack event types (ADDED→1, MODIFIED→2, DELETED→3), drop BOOKMARK, and
