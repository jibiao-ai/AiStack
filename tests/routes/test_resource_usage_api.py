...
from aistack.routes.resource_usage import (
...
async def session():
    from aistack.schemas.principals import Principal, PrincipalMembership
    from aistack.schemas.gpu_instances import GPUInstance
...
async def test_breakdown_no_pagination_rejects_oversized(session, monkeypatch):
    ...
    monkeypatch.setattr(
        "aistack.routes.resource_usage.envs.USAGE_BREAKDOWN_MAX_NO_PAGINATION_ROWS",
        1,
    )
...
async def test_instance_type_rows_are_identifiable_not_just_labelled(session):
    ...
    common = dict(
        ...
        quantity=3600,
        unit="seconds",
        instance_type_name="aistack--generic--nvidia-a100-linux-amd64",
    ...
    assert {i["key"] for i in items} == {"aistack--generic--nvidia-a100-linux-amd64"}
...
async def test_gpu_instances_breakdown_by_instance_type(session):
    from aistack.schemas.metered_usage import METER_INSTANCE_UPTIME as UPTIME
    from sqlalchemy import and_
...
async def test_gpu_instances_breakdown_includes_cpu_instances(session):
    ...
    from aistack.routes.resource_usage import gpu_instances_breakdown
    from aistack.schemas.metered_usage import RESOURCE_TYPE_CPU_INSTANCE
    ...
    session.add(
        _mu(
            ...
            resource_id=601,
            resource_name="cpu-1",
            sku="aistack--generic-31-ln-a64-1c-2g",
            sku_count=1,
            quantity=7200,
            unit="seconds",
...
async def test_resource_meta_lists_creators_instances_volumes(session):
    from aistack.routes.resource_usage import resource_meta
...
async def test_resource_events_filters_by_event_type_and_name(session):
    ...
    from aistack.routes.resource_usage import resource_events
    from aistack.schemas.resource_events import (
...
async def test_resource_events_filter_uses_rollup_tz_day(session, monkeypatch):
    ...
    from aistack import envs
    from aistack.routes.resource_usage import resource_events
    from aistack.schemas.resource_events import EVENT_TYPE_CREATED, ResourceEvent
