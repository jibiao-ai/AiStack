...
def _stubbed_enrichment():
    ...
    with (
        patch("aistack.routes.resource_usage.async_session", return_value=session_ctx),
        patch("aistack.routes.resource_usage._enrich_items", new=AsyncMock()),
    ):
        yield
...
async def test_resource_export_rejects_oversized_with_details(monkeypatch):
    ...
    monkeypatch.setattr("aistack.routes.resource_usage.envs.USAGE_EXPORT_MAX_ROWS", 10)
    user = _admin()
