...
import aistack.routes.clusters as clusters
from aistack.api.exceptions import InvalidException
from aistack.routes.clusters import (
...
def server_config(monkeypatch):
    ...
    cfg = MagicMock()
    cfg.server_external_url = "https://aistack.test"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("aistack/aistack:v1", None),
        # A dot, a port or localhost in the first segment makes it a host.
        ("reg.cn/aistack:v1", "reg.cn"),
        (
            "swr.cn-south-1.myhuaweicloud.com/aistack/aistack:v1",
            "swr.cn-south-1.myhuaweicloud.com",
        ),
        ("localhost:5000/aistack:v1", "localhost:5000"),
        ("localhost/aistack:v1", "localhost"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("aistack/aistack:v1", True),
        ("aistack:v1", True),
        ("docker.io/aistack/aistack:v1", True),
        (f"{PRIVATE_REGISTRY}/aistack/aistack:v1", False),
    ],
)
