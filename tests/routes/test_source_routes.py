...
from aistack.api.auth import get_admin_user
from aistack.api.exceptions import BadRequestException, ServiceUnavailableException
from aistack.routes import inference_backend, model_sets, ota_sources
from aistack.routes.source_probe import run_source_probe, source_probe_status
...
from aistack.schemas.inference_backend import InferenceBackend
...
from aistack.schemas.models import BackendEnum, BackendSourceEnum, Model, SourceEnum
from aistack.schemas import runner_source
...
from aistack.server import controllers
from aistack.server.sources import core as core_module
from aistack.server.sources import probe as probe_module
from aistack.server.sources import routes as routes_module
from aistack.schemas.source import SourceTypeEnum
from aistack.server.sources.core import (
    OFFICIAL_SOURCE_NAME,
    gather_and_merge,
    sha256_of,
)
from aistack.server.sources.probe import (
...
from aistack.server.sources.routes import (
    CustomSourceUpsert,
...
def test_the_ota_endpoints_are_one_admin_only_family_outside_any_id_namespace():
    ...
    """
    from aistack.routes.routes import api_router
...
def _packaged_runners(*service_versions) -> list:
    """Stands in for the packaged aistack-runner catalog."""
    return [
        Runner(
            backend="cuda",
...
class TestRunnerSourceConfig:
    ...
    def _fakes(self, monkeypatch):
        ...
        aistack-runner release, and serve URL fetches from ``_REMOTE``."""
        monkeypatch.setattr(
            runner_source,
            "list_runners",
...
class TestSourceProbe:
    ...
    async def test_status_reads_the_slots_from_the_rows_and_the_round_from_memory(
        ...
        assert status.ota_server_url == "https://ota.aistack.ai/latest"
