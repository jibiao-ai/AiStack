...
from aistack.api.exceptions import UnauthorizedException
from aistack.routes.token import server_auth
from aistack.schemas.api_keys import PermissionScope
from aistack.schemas.model_routes import AccessPolicyEnum
from aistack.schemas.principals import PrincipalType
from aistack.security import AUTH_CACHE_HEADER, JWTManager
...
def route_policy(monkeypatch):
    ...
    monkeypatch.setattr("aistack.routes.token.ModelRouteService", _ModelRouteService)
    monkeypatch.setattr("aistack.routes.token.UserService", _UserService)
    return state
...
def _install_asserted_lookups(monkeypatch, api_key, user):
    ...
    monkeypatch.setattr("aistack.api.auth.UserService", _AuthUserService)
...
def _forbid_credential_auth(monkeypatch):
    ...
    monkeypatch.setattr("aistack.routes.token.authenticate_request", _fail)
...
async def test_form_b_skips_authentication_and_keeps_the_consumer(
    ...
    assert response.headers["X-Mse-Consumer"] == f"{ACCESS_KEY}.aistack-7"
...
async def test_form_a_returns_a_key_ref_and_the_same_consumer(
    ...
    monkeypatch.setattr("aistack.routes.token.authenticate_request", _authenticate)

    response = await server_auth(
        _request(
            {
                GATEWAY_AUTH_TOKEN_HEADER: GATEWAY_TOKEN,
                "authorization": f"Bearer aistack_{ACCESS_KEY}_secret",
                "x-higress-llm-model": MODEL,
            }
        ),
        session=object(),
...
async def test_a_marker_short_circuits_on_the_connection_it_was_minted_on(
    ...
    monkeypatch.setattr("aistack.routes.token.authenticate_request", _reject)
...
async def test_a_marker_is_refused_off_its_connection(
    ...
    with caplog.at_level(logging.INFO, logger="aistack.routes.token"):
        with pytest.raises(UnauthorizedException):
...
async def test_a_marker_minted_without_a_connection_never_short_circuits(
    ...
    with caplog.at_level(logging.DEBUG, logger="aistack.routes.token"):
        with pytest.raises(UnauthorizedException):
            await server_auth(
                _request(
                    {
...
async def test_a_key_with_no_access_key_does_not_leave_a_leading_dot(
    ...
    None rather than for emptiness rendered it as ``.aistack-<id>``, which the
    ...
    assert response.headers["X-Mse-Consumer"] == "aistack-7"
...
def test_consumer_golden_value():
    ...
    """
    from aistack.routes.token import _build_consumer

    assert (
        _build_consumer("3192253c1f4a9b7e", _principal())
        == "3192253c1f4a9b7e.aistack-7"
    )
