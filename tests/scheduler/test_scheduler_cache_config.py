...
def _schedule_patches(model, model_instance, candidate, resolve_mock, service_mock):
    ...
    return (
        patch(
            "aistack.scheduler.scheduler.async_session",
            lambda: _FakeSessionCtx(),
        ),
        patch(
            "aistack.scheduler.scheduler.Worker.all",
            AsyncMock(return_value=[worker]),
        ),
        patch(
            "aistack.scheduler.scheduler.Model.one_by_id",
            AsyncMock(return_value=model),
        ),
        patch(
            "aistack.scheduler.scheduler.ModelInstance.one_by_id",
            AsyncMock(return_value=model_instance),
        ),
        patch(
            "aistack.scheduler.scheduler.ModelInstance.all",
            AsyncMock(return_value=[]),
        ),
        patch(
            "aistack.scheduler.scheduler.find_candidate",
            AsyncMock(return_value=(candidate, [])),
        ),
        patch(
            "aistack.scheduler.scheduler.resolve_instance_cache_config_safe",
            resolve_mock,
        ),
        patch("aistack.scheduler.scheduler.ModelInstanceService", service_mock),
    )
