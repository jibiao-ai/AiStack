...
from aistack.schemas.source import SourceContent, SourceTypeEnum, validate_icon
...
def test_packaged_catalog_normalizes():
    ...
    raw = files("aistack.assets").joinpath("model-catalog.yaml").read_text()
...
def test_packaged_community_backends_normalize():
    ...
    raw = files("aistack.assets").joinpath("community-inference-backends.yaml")
...
# A service the packaged aistack-runner catalog never carries, so a reconcile
