...
from aistack.mixins.active_record import CommitEvent, send_post_commit_events
...
from aistack.server.coordinator.cache import clear_all_caches, get_change_detector
...
async def test_queue_full_log_includes_metadata(caplog):
    ...
    caplog.set_level(logging.WARNING, logger="aistack.server.bus")
...
async def test_publish_does_not_let_slow_subscriber_block_peers():
    ...
    from aistack.server.bus import EventBus
...
async def test_the_commit_hook_publishes_a_created_event_with_its_id():
    ...
    with patch("aistack.mixins.active_record.event_bus", _RecordingBus()):
...
async def _deliver(event_type, warm_cache):
    ...
    with (
        patch("aistack.server.bus.get_model_for_topic", return_value=_FakeRow),
        patch("aistack.server.db.async_session", _FakeSession),
    ):
        if warm_cache:
...
async def test_local_row_is_not_cached_where_nothing_could_read_it(
    ...
    with (
        patch("aistack.server.bus.get_model_for_topic", return_value=None),
        patch("aistack.server.db.async_session", _FakeSession),
    ):
        await bus._process_coordinator_event(
