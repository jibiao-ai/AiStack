...
from aistack.schemas.cache_services import CacheServiceModeEnum
from aistack.server import cache_provider_catalog
from aistack.server.cache_provider_catalog import (
    get_cache_provider,
...
def test_lmcache_provider_declaration():
    ...
    # Managed only: LMCache is the single-container engine AiStack runs
...
def test_mooncake_provider_declaration():
    ...
    assert provider is not None
    # Shipped and supported as part of the AiStack catalog, like LMCache;
    ...
    # Mooncake is a self-deployed distributed store; AiStack only references
    ...
    assert provider.dashboard_uid == "aistack-mooncake"
...
def test_mooncake_injection_renders_store_connector_env():
    ...
    config = json.loads(files["/tmp/aistack-mooncake.json"])
...
def test_mooncake_injection_backfills_optional_fields_as_empty():
    ...
    config = json.loads(files["/tmp/aistack-mooncake.json"])
    assert config["device_name"] == ""
    assert "{{" not in files["/tmp/aistack-mooncake.json"]
...
def test_lmcache_sglang_injection_renders_config_file():
    ...
    assert args == [
        "--enable-lmcache",
        "--lmcache-config-file",
        "/tmp/aistack-lmcache-sgl.yaml",
    ]
    config = files["/tmp/aistack-lmcache-sgl.yaml"]
