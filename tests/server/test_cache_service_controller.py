...
from aistack.schemas.cache_providers import CacheProvider
...
from aistack.server.controllers import CacheServiceController
...
def _patch_reconcile(
    ...
    aggregate pass)."""
    monkeypatch.setattr(
        "aistack.server.controllers.get_cache_provider", lambda name: provider
    )
    monkeypatch.setattr(
        "aistack.server.controllers.Worker.all_by_fields",
        AsyncMock(return_value=workers or []),
    )
    monkeypatch.setattr(
        "aistack.server.controllers.Worker.one_by_id",
        AsyncMock(return_value=worker),
    )
    monkeypatch.setattr(
        "aistack.server.controllers.CacheServiceInstance.all_by_fields",
        AsyncMock(side_effect=list(instance_lists or [[], []])),
    )
    create = AsyncMock()
    monkeypatch.setattr(
        "aistack.server.controllers.CacheServiceInstance.create", create
    )
    return create
...
async def _run_instance_event(monkeypatch, service, event_type):
    ...
    monkeypatch.setattr(
        "aistack.server.controllers.async_session", lambda: _FakeSessionCtx()
    )
    monkeypatch.setattr(
        "aistack.server.controllers.CacheService.one_by_id",
    ...
    with patch(
        "aistack.server.controllers.CacheServiceInstance.subscribe",
...
def _model_instance(state, injected=False, reason="not ready", worker_id=5):
    from aistack.schemas.models import CacheConfigSnapshot
...
async def test_refresh_rewrites_snapshot_before_engine_start(state):
    ...
    with (
        patch(
            "aistack.server.controllers.Model.all_by_fields",
            AsyncMock(return_value=[_attached_model()]),
        ),
        patch(
            "aistack.server.controllers.ModelInstance.all_by_fields",
        ...
        patch(
            "aistack.server.controllers.resolve_instance_cache_config_safe",
            AsyncMock(return_value=fresh),
        ),
        patch(
            "aistack.server.controllers.ModelInstanceService",
...
async def test_aggregate_flags_spec_drift():
    ...
    from aistack.schemas.cache_services import cache_service_spec_digest
