...
from aistack.schemas.cache_providers import CacheProvider, CacheProviderHealthCheck
...
async def test_check_service_single_endpoint_flips_to_unreachable(monkeypatch):
    ...
    monkeypatch.setattr(
        "aistack.server.cache_services.async_session", lambda: _FakeSessionCtx()
    )
    monkeypatch.setattr(
        "aistack.server.cache_services.get_cache_provider", lambda name: _provider()
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr("aistack.server.cache_services.probe_cache_service", probe)
