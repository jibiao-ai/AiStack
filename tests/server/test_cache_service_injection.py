...
from aistack.server.cache_services import (
    probe_cache_service,
...
def patch_lookups(service, worker=..., instances=...):
    ...
    with (
        patch(
            "aistack.server.cache_services.CacheService.one_by_id",
            AsyncMock(return_value=service),
        ),
        patch(
            "aistack.server.cache_services.Worker.one_by_id",
            AsyncMock(return_value=worker),
        ),
        patch(
            "aistack.server.cache_services.CacheServiceInstance.all_by_fields",
...
async def test_resolve_passes_worker_framework_to_injection():
    ...
    with (
        ...
        patch(
            "aistack.server.cache_services.render_injection",
...
async def test_resolve_external_mooncake_injects_store_connector():
    ...
    assert snapshot.env == {
        "MOONCAKE_CONFIG_PATH": "/tmp/aistack-mooncake.json",
    ...
    config = json.loads(snapshot.files["/tmp/aistack-mooncake.json"])
...
async def test_probe_http_success():
    ...
    with patch(
        "aistack.server.cache_services.aiohttp.ClientSession",
...
async def test_resolve_strips_password_params_from_snapshot():
    ...
    from aistack.schemas.cache_providers import CacheProviderExternalField
    ...
    with (
        ...
        patch(
            "aistack.server.cache_services.get_cache_provider",
