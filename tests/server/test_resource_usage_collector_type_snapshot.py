...
import aistack.server.resource_usage_collector as rc
...
from aistack.server.resource_usage_collector import (
...
from aistack.utils.resource_usage import volume_sku
...
def make_type(
    *,
    cluster_id: int = 1,
    name: str = "aistack--generic--nvidia-a100-linux-amd64",
...
def make_event(
    *,
    ...
    resource_id: int = 1,
    type_name: str = "aistack--generic--nvidia-a100-linux-amd64",
...
def test_null_type_snapshot_falls_back_and_is_tagged():
    ...
    w = _open_window_from_event(evt)
    assert w.sku == "aistack--generic--nvidia-a100-linux-amd64"
...
async def test_switching_instance_type_within_one_hour_splits_too(catalog_session):
    ...
    a100 = make_type()
    h100 = make_type(
        name="aistack--generic--nvidia-h100-linux-amd64",
...
def test_cpu_instance_carries_no_card_facets():
    ...
    evt = make_event(
        ...
        type_name="aistack--generic-linux-amd64",
    )
...
def _cpu_type(
    *, name: str = "aistack--generic-linux-amd64", unit_cpu: str = "1000m", **kw
) -> GPUInstanceType:
...
async def test_sealed_rows_are_never_rewritten(catalog_session):
    ...
    sealed = MeteredUsage(
        ...
        sku="aistack--generic-ln-x64-4c-16g-98g--nvidia-tesla-t4-1d",
        sku_count=4,
    ...
    assert sealed.sku == "aistack--generic-ln-x64-4c-16g-98g--nvidia-tesla-t4-1d"
