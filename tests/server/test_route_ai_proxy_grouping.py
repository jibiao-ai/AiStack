...
from aistack.gateway.utils import lora_registry_name_suffix
from aistack.schemas.clusters import Cluster
from aistack.schemas.model_routes import ModelRoute, ModelRouteTarget, TargetStateEnum
...
async def test_lora_targets_of_one_deployment_fold_into_a_single_group():
    ...
    with (
        patch(
            "aistack.server.controllers.ModelRouteTarget.all_by_field",
            AsyncMock(return_value=targets),
        ),
        patch(
            "aistack.server.controllers.Model.one_by_id",
            AsyncMock(return_value=model),
        ),
        patch(
            "aistack.server.controllers.Cluster.one_by_id",
            AsyncMock(return_value=_cluster()),
        ),
        patch(
            "aistack.server.controllers.ModelInstance.all_by_field",
    ...
    assert len(groups) == 1
    group = groups[0]
    assert group.provider_id() == "aistack-model-5"
...
async def test_cluster_token_is_resolved_once_per_cluster():
    ...
    with patch("aistack.server.controllers.Cluster.one_by_id", one_by_id):
