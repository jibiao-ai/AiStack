...
from aistack.utils.resource_usage import (
    SLICE_MODE_PROFILE,
    SLICE_MODE_RATIO,
    SLICE_MODE_WHOLE,
...
def test_non_metered_phases_match_schema():
    ...
    from aistack.schemas.gpu_instances import GPUInstancePhase
    from aistack.utils.resource_usage import _NON_METERED_PHASES
...
def test_parse_gpu_type():
    assert parse_gpu_type("aistack-nvidia-geforce-rtx-4090-c9bjn") == (
        "nvidia-geforce-rtx-4090",
        "nvidia",
    )
    assert parse_gpu_type("aistack-amd-mi300x-ab12c") == ("amd-mi300x", "amd")
...
def test_instance_sku():
    ...
    assert (
        instance_sku("aistack-nvidia-h100-ab12c", "nvidia-h100", 2, 8000, 128000)
        == "aistack-nvidia-h100-ab12c"
    )
    assert (
        instance_sku("aistack--generic-31-ln-a64-1c-2g", None, 0, 1000, 2048)
        == "aistack--generic-31-ln-a64-1c-2g"
    )
...
def test_parse_gpu_descriptor():
    desc = (
        '{"name":"aistack-nvidia-geforce-rtx-5090-d-18c-54gi",'
