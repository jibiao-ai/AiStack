...
from aistack.schemas.cache_services import CacheConfigSnapshot
...
from aistack.envs import ENABLE_CUDA_MINOR_VERSION_COMPATIBILITY_ENV as _KNOB
from aistack.worker.backends.base import (
    InferenceServer,
    is_ascend,
    is_ascend_310p,
    read_lora_max_rank,
...
from aistack.worker.backends.custom import CustomServer
from aistack.worker.backends.sglang import (
    SGLangServer,
...
from aistack.worker.backends.vllm import (
    VLLMServer,
...
from aistack.worker.backends.vox_box import VoxBoxServer
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "aistack/runner:cuda12.8-vllm0.10.2",
            "test-registry.io",
            "test-registry.io/aistack/runner:cuda12.8-vllm0.10.2",
            None,
        ),
        (
            "foo/bar",
        ...
        (
            "aistack/runner:cuda12.8-vllm0.10.2",
            None,
            "quay.io/aistack/runner:cuda12.8-vllm0.10.2",
            "quay.io",
        ),
        (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Prefix-caching flags are not AiStack's responsibility — left to the user.
...
def test_vox_box_command_args_return_injected_parameters():
    ...
    backend._config = types.SimpleNamespace(data_dir="/var/lib/aistack")
    backend._worker = types.SimpleNamespace(ip="192.168.50.10")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("12.8", "aistack/runner:cuda12.8-vllm0.10.2"),
...
def test_resolve_image_fallback_matches_host_major(
    ...
    # aistack-runner returns backend versions newest-first.
    runner = types.SimpleNamespace(
        versions=[
            _make_versioned_runner("13.0", "aistack/runner:cuda13.0-vllm0.10.2"),
            _make_versioned_runner("12.9", "aistack/runner:cuda12.9-vllm0.10.2"),
            _make_versioned_runner("12.8", "aistack/runner:cuda12.8-vllm0.10.2"),
        ]
    )
...
def test_resolve_image_treats_blank_backend_version_as_auto(
            ...
            types.SimpleNamespace(
                versions=[
                    _make_versioned_runner(
                        "13.0", "aistack/runner:cuda13.0-vllm0.10.2"
                    )
                ]
            )
        ]
    ...
    assert image_name == "aistack/runner:cuda13.0-vllm0.10.2"
...
@pytest.mark.parametrize(
    "image,expected",
    [
        ("aistack/runner:cuda12.9-vllm0.10.2", "12.9"),
        (
            "registry.example.com/aistack/runner:cuda12.8-vllm0.10.2-linux-amd64",
            "12.8",
        ),
        (
            "registry.example.com/cuda11.8/aistack/runner:cuda12.8-vllm0.10.2-linux-amd64",
            "12.8",
        ),
        ("aistack/runner:cuda13.0-sglang0.4", "13.0"),
        ("aistack/runner:rocm6.2-vllm0.10.2", None),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("12.8", "aistack/runner:cuda12.9-vllm0.10.2", "cuda", True),
        # Equal minor: cuda-compat is never activated, nothing to do.
        ("12.8", "aistack/runner:cuda12.8-vllm0.10.2", "cuda", False),
        # Host newer than image: plain backward compatibility, nothing to do.
        ("12.8", "aistack/runner:cuda12.6-vllm0.10.2", "cuda", False),
        # Cross major is not MVC-safe; must not trigger.
        ("12.8", "aistack/runner:cuda13.0-vllm0.10.2", "cuda", False),
        # Non-CUDA vendor.
        ("6.2", "aistack/runner:rocm6.3-vllm0.10.2", "rocm", False),
        # Host CUDA version undetected.
        (None, "aistack/runner:cuda12.9-vllm0.10.2", "cuda", False),
...
def test_should_disable_cuda_compat(monkeypatch, host_cuda, image, backend, expected):
    ...
    monkeypatch.setattr("aistack.envs.ENABLE_CUDA_MINOR_VERSION_COMPATIBILITY", True)
...
def test_cuda_compat_switch_is_multi_level(
    ...
    monkeypatch.setattr(
        "aistack.envs.ENABLE_CUDA_MINOR_VERSION_COMPATIBILITY", global_enabled
    )
    server = _make_cuda_compat_server(
        "12.8",
        "aistack/runner:cuda12.9-vllm0.10.2",
        model_env=model_env,
    )
    assert server._should_disable_cuda_compat() is expected
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"AISTACK_HOST_IPC": "false"},
            None,
            False,
        ),
        (None, {"AISTACK_HOST_IPC": "true"}, None, True),
...
def test_serving_command_script_writes_cache_injection_files():
    ...
    files = {"/tmp/aistack-mooncake.json": '{"mode": "standalone-store"}'}
    script = _script_server(False, cache_files=files)._get_serving_command_script({})
    assert script is not None
    assert "cat > '/tmp/aistack-mooncake.json' <<'AISTACK_CACHE_FILE_EOF'" in script
...
def test_serving_command_script_none_without_triggers():
    ...
    assert (
        _script_server(True)._get_serving_command_script(
            {"AISTACK_MODEL_SERVING_COMMAND_SCRIPT_DISABLED": "1"}
        )
        is None
    )
...
def test_configured_env_injects_disable_require_only(monkeypatch):
    monkeypatch.setattr(
        "aistack.worker.backends.base.filter_env_vars", lambda _env: {}
    )
    server = _StubServer.__new__(_StubServer)
    ...
    assert "AISTACK_DISABLE_CUDA_COMPAT" not in env
