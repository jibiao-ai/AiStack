...
from aistack.policies.candidate_selectors.vllm_resource_fit_selector import (
...
def test_validate_returns_tp_for_inference_by_caller():
    """selector can read out.tp to learn what AiStack inferred."""
...
def test_create_candidate_cross_node_tp_allowed():
    """Cluster=[1,1] + tp=2 → cross-node TP is now allowed; AiStack builds the
    candidate and leaves the launch-time go/no-go to vLLM.

    Previously AiStack pre-emptively rejected TP that spans nodes; that guard
