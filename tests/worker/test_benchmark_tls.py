...
The benchmark container is a separate ``aistack/benchmark-runner`` image whose
entrypoint is the runner CLI, so it never runs ``aistack-prerun.sh`` and never
...
import aistack.worker.benchmark.runner as bm_runner
from aistack.worker.benchmark.runner import BenchmarkRunner
...
class TestProgressCAFile:
    ...
    def test_https_hands_over_the_worker_bundle(self, tmp_path, monkeypatch):
        ...
        ca_file = BenchmarkRunner._progress_ca_file(
            self._runner("https://aistack.internal/v2/benchmarks/1/state")
        )
    ...
    def test_insecure_mode_hands_over_nothing(self, tmp_path, monkeypatch):
        ...
        assert (
            BenchmarkRunner._progress_ca_file(
                self._runner(
                    "https://aistack.internal/v2/benchmarks/1/state", insecure=True
                )
            )
            is None
        )
...
class TestInsecureFlagForwarding:
    ...
    def _runner(self, api_url, *, insecure):
        ...
        return SimpleNamespace(
            ...
            _benchmark_dir="/var/lib/aistack/benchmarks",
            _api_url=api_url,
            _api_key="token",
            _progress_insecure_skip_tls_verify=insecure,
            _progress_is_https=urlparse(api_url).scheme == "https",
    ...
    def test_the_ca_cert_points_at_the_injected_file(self):
        ...
        args = BenchmarkRunner._build_command_args(
            self._runner(
                "https://aistack.internal/v2/benchmarks/1/state", insecure=False
            ),
            with_ca_cert=True,
        )
    ...
    def test_the_warning_travels_with_the_flag(self, caplog):
        ...
        assert "AISTACK_INSECURE_TLS is set" in caplog.text
...
class TestResolveProgressInsecureTLS:
    ...
    Only AISTACK_INSECURE_TLS, which a worker started with the enterprise
