...
from aistack.schemas.cache_providers import (
    CacheProvider,
...
from aistack.worker.cache_service_manager import (
...
def _run_start(
    ...
    with (
        patch(
            "aistack.worker.cache_service_manager.get_cache_provider",
            return_value=provider,
        ),
        patch(
            "aistack.worker.cache_service_manager.network.get_free_port",
            side_effect=lambda **kwargs: next(ports),
        ),
        patch(
            "aistack.worker.cache_service_manager.registration.determine_default_registry",
            return_value=None,
        ),
        patch(
            "aistack.worker.cache_service_manager.transform_workload_plan",
            side_effect=lambda cfg, plan, fallback: plan,
        ),
        patch("aistack.worker.cache_service_manager.delete_workload"),
        patch("aistack.worker.cache_service_manager.create_workload") as create,
        patch.object(
...
def test_gpu_resources_attaches_all_devices_on_gpu_backend():
    ...
    manager, _ = _build_manager(worker_id=1)
    with patch(
        "aistack.worker.cache_service_manager.detect_backend",
        return_value="cuda",
    ):
...
def test_start_instance_removes_stale_workload_first():
    ...
    with (
        ...
        patch(
            "aistack.worker.cache_service_manager.delete_workload",
            side_effect=lambda name: call_order.append(("delete", name)),
        ),
        patch(
            "aistack.worker.cache_service_manager.create_workload",
...
def test_start_instance_resolves_runtime_image():
    ...
    with (
        ...
        patch(
            "aistack.worker.cache_service_manager.detect_devices",
...
def _field_provider(**overrides):
    from aistack.schemas.cache_providers import CacheProviderField
...
def test_sync_workload_failed_restarts_with_incremented_count():
    ...
    with (
        patch(
            "aistack.worker.cache_service_manager.get_workload",
            return_value=SimpleNamespace(state=WorkloadStatusStateEnum.FAILED),
        ),
        patch("aistack.worker.cache_service_manager.delete_workload") as delete,
        patch.object(manager, "_update_cache_service_instance") as update,
    ):
...
def test_sync_ready_probe_resets_restart_count_after_stable_window():
    ...
    with (
        ...
        patch("aistack.worker.cache_service_manager.socket.create_connection"),
        patch.object(manager, "_update_cache_service_instance") as update,
    ):
        manager.sync_cache_service_instances_state()
...
def test_sync_ready_tcp_probe_marks_running_healthy():
    ...
    with (
        ...
        patch(
            "aistack.worker.cache_service_manager.socket.create_connection"
        ) as connect,
        patch.object(manager, "_update_cache_service_instance") as update,
    ):
        manager.sync_cache_service_instances_state()
...
def test_sync_skips_instances_of_other_workers_and_states():
    ...
    with (
        patch("aistack.worker.cache_service_manager.get_workload") as get_wl,
        patch.object(manager, "_update_cache_service_instance") as update,
        patch.object(manager, "_start_cache_service_instance") as start,
    ):
        manager.sync_cache_service_instances_state()
...
def test_stop_instance_deletes_workload_and_frees_port():
    ...
    with patch("aistack.worker.cache_service_manager.delete_workload") as delete:
...
def test_start_instance_without_run_command_runs_image_entrypoint():
    ...
    the entrypoint with the parameters — and everything AiStack adds
...
def test_start_instance_host_ipc_override(monkeypatch):
    ...
    it) but honors the service env and the worker-global AISTACK_HOST_IPC
    ...
    cache_service = _new_cache_service(
        config=CacheServiceConfig(ram_size=8, env={"AISTACK_HOST_IPC": "0"})
    )
    create, _ = _run_start(manager, clientset, cache_service, _new_provider())
    assert create.call_args[0][0].host_ipc is False

    from aistack import envs as aistack_envs

    monkeypatch.setattr(aistack_envs, "HOST_IPC", "false")
    cache_service = _new_cache_service(config=CacheServiceConfig(ram_size=8))
    create, _ = _run_start(manager, clientset, cache_service, _new_provider())
    assert create.call_args[0][0].host_ipc is False
...
def test_start_instance_reuses_recorded_ports():
    ...
    with (
        ...
        patch(
            "aistack.worker.cache_service_manager.network.is_port_available",
            return_value=True,
        ),
        patch(
            "aistack.worker.cache_service_manager.network.get_free_port"
        ...
        patch("aistack.worker.cache_service_manager.delete_workload"),
        patch("aistack.worker.cache_service_manager.create_workload"),
...
def test_probe_targets_metrics_port_for_http_health_check(monkeypatch):
    ...
    with (
        ...
        patch("aistack.worker.cache_service_manager.httpx.get") as http_get,
    ):
